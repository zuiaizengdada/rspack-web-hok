  ...
  <div class="action-bar-container flex items-center justify-between">
    <div class="bar-left flex">
      <div class="icon-list flex">
        <el-tooltip v-for="item in iconList" :key="item.src">
...
export default {
  ...
  methods: {
    ...
    async initHotKeys() {
      ...
      const { adsorbent, contentMark, preview, trackLarge, trackSmall, videoCut, videoExport: exportVideo } = data
      // hotkeys.unbind()
      // 打开标记
      if (contentMark) {
      ...
      // const mm = 'Ctrl + M'
      // console.log(exportVideo, 4444444444)
      // 快速导出
      if (exportVideo) {
        hotkeys(exportVideo, { keyup: true }, (e) => {
          e.preventDefault()
          !videoExport.getVisible() && videoExport.open({
