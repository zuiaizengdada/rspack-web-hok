...
import { keycodes } from '@/views/oaMannagement/signFileManagement/components/utils/shortcutKey'
...
export default {
  ...
  computed: {
    ...mapState({
      editMode: state => state.oaManage.editMode,
  ...
  methods: {
    ...
    handleInput(e) {
      console.log(e.target.innerHTML, '输入值+++++')
      this.$emit(
        'input',
        this.element,
    ...
    handleBlur(e) {
      this.element.propValue = e.target.innerHTML.replace(/&nbsp;/g, ' ') || ' '
