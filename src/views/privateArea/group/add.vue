<template>
  <div class="table_wrap">
      ...
      <el-form ref="form" :model="form" label-width="100px" :rules="rules">
        ...
        <el-form-item label="发送类型" required>
          ...
          <div v-if="myArray.length" class="form-list-box">
            <!-- <el-form-item prop="listStatusInfo"> -->
            <el-form-item>
              <draggable
                v-model="myArray"
                chosen-class="chosen"
                  ...
                  <div v-for="(item, index) in myArray" :key="item.msgId" class="draggable-item">
                    <div class="draggable-item-child">
                      ...
                      <div class="m-l20 draggable-item-child-info">
                        ...
                        <div v-else-if="item.msgType == 2" class="draggable-item-child-info-txt">
                          {{ item.content }}
                        </div>
    ...
    <el-button type="primary" class="foot-btn-box" size="small" @click="save()">保存</el-button>
    <AppDialog
    ...
    >
      ...
      <div slot="footer" class="footer">
        <el-button type="primary" size="small" @click="onPopSave()">保存</el-button>
      </div>
    </AppDialog>
  </div>
</template>
...
import { wechatCoursePage, msgAdd, msgSend, wechatMsgDetail } from '@/api/privateArea/CWMassSending'
export default {
  name: 'CWeChat',
  components: {
    AppDialog,
  ...
  data() {
    return {
      ...
      rules: {
        ...
        listStatus: [{ required: true, message: '请选择发送类型', trigger: 'change' }],
        listStatusInfo: [{ required: true, message: '请编辑类型内容', trigger: 'change', type: 'array' }]
      },
      rulesT1: {
      ...
      this.$SendObj({
        ...
        success: (res) => {
          ...
          if (res.type === 1) {
            ...
            console.log('汇总数据', that.objSelect)
          } else if (res.type === 2) {
            const uids = []
            res.value.forEach((e) => {
              uids.push(e.uid)
            ...
            that.objSelect.push({
              ...
              title: `查询条件${res.value.user.label === 'name' ? '昵称' : '真实姓名'}:${
                res.value.user.value ? res.value.user.value : '无'
              };${labelName}`
            })
          }
          this.form.sObj = this.objSelect
