<template>
  <div class="kanbanlist" :class="{ active: active }" @click="onhandleClick">
    <div class="kanbanContent">
      ...
      <!-- <div v-if="item.cardNums" class="kanbanlistNumber">
        <div>
          <div>{{ item.value }}</div>
          <div>总数</div>
        </div>
        <div>
          <div class="textOne">{{ item.valueOne }}</div>
      ...
      <div
        v-if="item.basisRatio==='0' || item.basisRatio===0 || item.basisRatio"
        class="kanbanListFloat"
      >
        <div class="footerViewText">环比上期</div>
        <div v-if="!item.isCarousel">
          <i style="font-size: 12px" :class="cssFullClass(item.basisRatio)" />
          <span
            :style="cssBasisRatioClass(item.basisRatio)"
          >{{ Math.abs(item.basisRatio) }}%</span>
          <span v-if="item.previousValue !== undefined" class="footerViewTexts">
            ({{
        ...
        <el-carousel
          ...
          @click.native="disableClickEvents"
        >
          <el-carousel-item v-for="(n, index) in item.basisRatio" :key="index">
            <i style="font-size: 12px" :class="cssFullClass(n)" />
            <span
              class="medium"
              :style="cssBasisRatioClass(n)"
            >{{ Math.abs(n) }}%</span>
            <span class="footerViewTexts">
              ({{
                Math.abs(
                  item.ismoney
                    ? item.previousValue[index]
                    : Math.abs(item.previousValue[index])
                )
              }})
            </span>
          </el-carousel-item>
      ...
      <div class="kanbanListFloatFot">
        <div v-if="item.deliveryRate">
          送达率 <span>{{ item.deliveryRate }}</span>
        </div>
        <div v-if="item.convertedRatio !== undefined">
          线索转化率 <span>{{ item.convertedRatio }}</span>
        </div>
        <div v-if="item.addWxRate" class="deliveryRateBox">
          <div>加微率</div>
          <span v-if="!item.isCarousel" style="font-weight: bold">
            {{ item.addWxRate }}
          </span>
          <el-carousel
            v-else
        ...
        <div v-if="item.completingRate">
          接通率 <span>{{ item.completingRate }}</span>
        </div>
        <div v-if="item.completingTime">
          通话时长<span>{{ item.completingTime }}</span>分钟
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    cssBasisRatioClass(val) {
      ...
      if (numl < 0) {
        return 'color:#D43030;font-weight:bold'
      } else if (numl > 0) {
        return 'color:#43CF7C;font-weight:bold'
      } else {
