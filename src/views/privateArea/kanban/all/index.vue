...
export default {
  ...
  methods: {
    ...
    async getClueStatHourlineChart() {
      ...
      this.$nextTick(() => {
        this.$refs.histogramRef.getData(prama, !this.search.followDeptId)
      })
    },
    // 获取数据总览
    getDetail() {
    ...
    async getMakeSelectData() {
      ...
      if (code === 1 && data) {
        ...
        this.listDataOne[5].previousValue = (
          previousPerCapitaConversion || 0
        ).toFixed(2)
      ...
      getSelectData(data)
        .then(res => {
          if (res.code === 1) {
            ...
            this.listData[10].previousValue = (
              previousPerCapitaConversion || 0
            ).toFixed(2)
    ...
    getUserClueList() {
      ...
      if (!this.search.type) {
        data = {
          ...data,
          sort: this.labelSearch?.sort || 'DESC',
          column: this.labelSearch?.sort ? this.labelSearch?.column : 'addWxNum'
        }
      }
      ...
      query(data)
        .then(res => {
          if (res.code === 1) {
            this.UserClueStat = res.data.items
