    ...
    >
      <template slot="search">
        <filter-top
          ...
          type="all"
          :permission-key="[
            'web:allClue:activateTask',
            'web:allClue:followUp',
            'web:allClue:taskReport',
            'web:allClue:taskReport'
          ]"
      ...
      <template slot="footer">
        <bottomBtns
          :permission-key="[
            'web:allClue:intention',
            'web:allClue:changePerson',
            'web:allClue:partExport',
            'web:allClue:allExport',
            'web:allClue:massTexting'
          ]"
      ...
      <template slot="action" slot-scope="scope">
        <el-button
          v-permission="['web:allClue:details', permsList]"
          type="text"
        ...
        <el-button
          ...
          "
          v-permission="['web:allClue:sendCode', permsList]"
          :disabled="
            scope.row.isAddQw === 1 ||
...
export default {
  ...
  data() {
    return {
      ...
      config: {
        tableData: [],
        tableColumns: getColumns.call(this, 'web:allClue:setPhone'), // , 'web:allClue:setPhone'
  ...
  computed: {
    isShowAvailable() {
      return this.permsList.includes('web:allClue:isAvailable')
    }
  },
  mounted() {
    this.init()
    ...
    getList() {
      ...
      getCluePageList(params)
        .then(res => {
          ...
          this.config.totalPages = res?.data?.totalPages
