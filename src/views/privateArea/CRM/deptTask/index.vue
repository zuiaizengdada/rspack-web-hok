    ...
    >
      <template slot="search">
        <filterTop
          ...
          :search="search"
          type="dept"
          :permission-key="['web:deptTask:add']"
      ...
      <template slot="action" slot-scope="{ row }">
        <el-button
          v-permission="['web:deptTask:edit', permsList]"
          type="text"
          @click="openEdit(row)"
        >编辑</el-button>
        <el-button
          v-permission="['web:deptTask:clue', permsList]"
          type="text"
          @click="gotoClue(row)"
        >线索</el-button>
...
export default {
  ...
  methods: {
    ...
    getDropdownList(row) {
      const dropdownList = []
      if (this.permsList.includes('web:deptTask:detail')) {
        dropdownList.push({
          command: '8',
          label: '详情'
        })
      ...
      if (
        this.permsList.includes('web:deptTask:copy') &&
      ...
      if (
        this.permsList.includes('web:deptTask:close') &&
      ...
      if (
        this.permsList.includes('web:deptTask:stop') &&
      ...
      if (
        this.permsList.includes('web:deptTask:restart') &&
      ...
      if (
        this.permsList.includes('web:deptTask:live') &&
        row.isLive
      ) {
        dropdownList.push({
          command: '6',
      ...
      if (this.permsList.includes('web:deptTask:kanban') && row.dataType !== 4) {
        dropdownList.push({
          command: '7',
          label: '看板'
        })
      }
      if (this.permsList.includes('web:deptTask:log')) {
        dropdownList.push({
          command: '9',
          label: '日志'
        })
      ...
      if (
        this.permsList.includes('web:deptTask:delete') &&
        row.taskStatus === '4'
    ...
    gotoClue(row) {
      ...
      if (this?.active?.value === 1) {
        this.$router.push({
          path: '/privateArea/clue/clueDepartment',
          query: { taskId: row.id, taskName: row.taskName }
        })
      } else {
        this.$router.push({
          path: '/privateArea/clue/clueAll',
          query: { taskId: row.id, taskName: row.taskName }
        })
      }
    },
