...
<template>
  ...
  >
    <el-form ref="post" :model="post" :rules="rules" label-width="120px">
      <el-form-item label="人员" class="form-item" prop="name">
        <span>{{ post.name }}</span>
      </el-form-item>
      <el-form-item
        label="人员信息移入"
        class="form-item"
        prop="candidateMoveIn"
      >
        <el-select
          v-model="post.candidateMoveIn"
          placeholder="请选择"
          style="width: 100%"
        >
      ...
      <el-form-item label="原因类型" class="form-item" prop="reasonType">
        <el-select
          v-model="post.reasonType"
          placeholder="请选择"
          style="width: 100%"
        >
