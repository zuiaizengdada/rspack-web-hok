  ...
  <div class="upload">
    ...
    <template v-for="(item, index) in fileList">
      <uploadItem
        ...
        @upload="upload"
        @replace="replaceUpload($event, index)"
      />
    </template>

    <div
      v-if="tips"
      class="tips overflowOuto"
      v-html="tips ? tips.replace(/\n/g, '<br />') : ''"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async alignUpload(item, index) {
      const self = this.fileList[index]
      console.log('点击重新上传', self, index)
      await this.resumeUpload(self)
    },
    async replaceUpload(e, oldIndex) {
      this.$refs.upload && this.$refs.upload.clearFiles()
      console.log('上传文件', e)
      if (!e) {
        return
      ...
      this.fileList[oldIndex] = obj
      // this.$emit('change', this.fileList)
      const self = this.fileList[oldIndex]
      console.log(self)
      try {
        const options = {
          // 获取分片上传进度、断点和返回值。
