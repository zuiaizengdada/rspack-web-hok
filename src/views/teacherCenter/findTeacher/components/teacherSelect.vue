...
<script>
import { teacherQueryAllByName } from '@/api/tearchCenter/index'
export default {
  name: 'TeacherSelect',
  props: {
    defaultTearchId: {
  ...
  methods: {
    ...
    // 点击添加老师
    gotoAddTeacher() {
      console.log('searchTearch', this.searchTearch)
      console.log('searchVideo', this.searchVideo)
      const query = {
        urls: this.searchVideo,
        name: this.searchTearch
      }
      const routeData = this.$router.resolve({ path: '/teacherCenter/teachFile/add', query })
