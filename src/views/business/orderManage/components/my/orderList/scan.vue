<template>
  <div v-loading="loading" class="page_wrap">
    <div class="content">
      <div class="table-module" :class="showBorder?'table-module-border':''">
        <!-- <div class="m-b-20">
            <el-button
        ...
        <div class="main-table">
          <div class="table-header clearfix" :style="{borderRadius: showBorder ?'6px 6px 0 0':'' }">
            ...
            <div v-for="(item, itemIndex) in tableData" :key="itemIndex">
              <div class="table-title-row">
                <div class="table-title-row-l">
                  <div class="order-box">
                    <div v-if="item.orderNo" class="title-item title-item-font">
                      ...
                      <AppIconClipboard :src="item.orderNo">
                        <span class="a_link" @click="gotoDetail(item)">{{ item.orderNo }}</span>
                      </AppIconClipboard>
                    </div>
                    <div v-if="item.createTime" class="title-item title-item-font">下单时间: <span>{{ item.createTime }}</span></div>
                    <!-- <div v-if="[1, 2, 3, 4, 5].includes(item.paymentType)" class="title-item">
                        渠道来源: {{ { 1: '微信', 2: '支付宝', 3: '赠送', 4: '抖店支付', 5: '快手支付' }[item.paymentType] }}
                      </div> -->
                    <div v-if="item.cid && item.cid > 0" class="title-item title-item-font">付费推广</div>
                    <!-- <div class="title-item">收款方式</div> -->
                    <!-- <div class="title-item">普通订单</div> -->
                  </div>
                  <div class="pay-box">
                    <AppStatus v-if="item.orderStatus" :status="orderStatus[item.orderStatus].type">
                  ...
                  <div class="title-item">
                    招生老师：{{ item.enrollmentTeacherName }}
                  </div>
                  <div v-if="item.orderConfigId===6" class="title-item">
                    备注：{{ item.depositGoodsName }}
                  </div>
              ...
              <div class="table-body">
                <div v-for="(listItem, index) in item.goodsList" :key="index" class="table-body-row" :class="index === item.goodsList.length - 1?'table-body-row-box':''" :style="{marginBottom: showBorder && item.list.length - 1 == index ?'16px':'' }">
                  <div class="body-item">
                    <div class="goods-img-info">
                      <div class="goods-img">
                        <ImagePreviewer :src="listItem.coverUrl" fit="contain" style="width: 100%; height: 100%;border-radius: 6px;" />
                      </div>
                  ...
                  <div v-if="operationColumn" class="body-item">
                    <el-button v-if="item.confirmUser !== 0" type="text" @click="gotoDetail(item)">订单详情</el-button>
                    <el-button
                      v-if="transferEnableFlag ===1 && item.transferEnableFlag === 1"
                      type="text"
                      @click="toTransfer(item)"
                    >订单转让</el-button>
                    <el-button
                      v-if="item.confirmUser === 0"
                      type="text"
                    ...
                    >
                      ...
                      <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item
                          v-if="item.examineStatus === 0 && [8,9,17].includes(item.channelSource) && item.orderConfigId != 5"
                          type="text"
                          style="color: #F53F3F"
                          @click.native="deleteOrderMethod(item)"
                        >删除</el-dropdown-item>
                        <el-dropdown-item
                        ...
                        <el-dropdown-item
                          v-if="[1].includes(listItem.orderStatus) && item.channelSource === 3"
                        ...
                        <div v-if="item.orderConfigId != 6">
                          <el-dropdown-item
                            v-if="search.activeName === '全部订单' && (item.orderStatus === 2 || item.orderStatus === 6) && item.isTransfer === 0 && item.orderConfigId === 1 && (item.channelSource === 8 || item.channelSource ===9 || item.channelSource ===17)"
                          ...
                          <el-dropdown-item
                            v-if="search.activeName === '全部订单' && (item.orderStatus === 2 || item.orderStatus === 6) && item.isTransfer === 0 && item.orderConfigId === 1 && (item.channelSource === 8 || item.channelSource ===9 || item.channelSource ===17)"
                            v-permission="['system:order:partRefund', permsList]"
                            type="text"
    ...
    >
      <div v-loading="changePriceModel.loading" class="change-price-model">
        <!-- <div>改价</div> -->
        <el-table :data="changePriceModel.data" style="width: 100%;max-height: 400px;overflow: auto;" border>
          ...
          <el-table-column prop="name" label="改价(元)" width="190">
            <template slot-scope="{ row }">
              ...
              <el-input-number v-model="row.payerTotal" controls-position="right" style="width:120px" :precision="2" :step="0.01" :min="0.01" :max="row.goodsTotalFeeNum" @change="changePrice" />
            </template>
          </el-table-column>
        </el-table>
    ...
    <!-- 退款弹框 -->
    <AppDialog v-model="refundModel.visible" :title="refundModel.title" width="700px" height="100px">
      <div v-if="refundModel.title === '全额退款'" class="full-price">
        <div>请确认该操作将变更订单的退款状态为：<span class="full-price-ps">已退款</span></div>
        <div class="full-price-box">退款金额：<span class="refund-prcie">￥{{ refundModel.prcie }}</span></div>
      </div>
      <div v-if="refundModel.title === '部分退款'" class="partial-price">
        <div class="partial-price-in-box">
          ...
          />
          <span>元</span>
          <span class="partial-price-in-ps">注：部分退款金额不得大于等于订单金额</span>
        </div>
        <div v-if="refundModel.refundPrice > 0" class="partial-price-in-toast">
          <div class="partial-price-in-toast-l">请确认该操作将变更订单的退款状态为：<span>部分退款</span></div>
          <div class="partial-price-in-toast-r"> 退款金额：<span class="refund-prcie">￥{{ refundModel.refundPrice }}</span></div></div>
      </div>
      <div slot="footer">
        <div class="footer-box">
          <el-button v-if="refundModel.title === '全额退款'" type="primary" size="small" @click="onClickFullRefund">确定</el-button>
          <el-button v-if="refundModel.title === '部分退款'" type="primary" size="small" @click="onClickPartialRefund">确定</el-button>
    ...
    <TransferModal ref="transferModal" @formLastLoadData="formLastLoadData" />
  </div>
</template>
...
export default {
  ...
  created() {
    this.getTransferCfonfig()
    // this.$store.dispatch('option/getChannelsourceType')
  },
  activated() {
  },
  methods: {
    async getTransferCfonfig() {
      const { data } = await transferCfonfig()
      this.transferEnableFlag = data.transferEnableFlag
    },
    toComplete(row) {
      this.row = row
    ...
    toTransfer(row) {
      this.row = row
      this.$refs.transferModal.show(row)
    },
    deleteOrderMethod(item) {
      const test = '是否要删除当前订单号为【' + item.orderNo + '】的订单号？'
    ...
    getThirdShopName(item, list) {
      if (item.channelSource === 3 && item.userTerminal) {
        const userTerminalName = getOptionsValue(item.userTerminal, userTerminal)
        return `| ${userTerminalName}`
      }
      if (item.channelSource && item.channelSource !== 3 && list.thirdShopName) {
        return `| ${list.thirdShopName}`
      }
      return ''
    },
    setSearchOption(e) {
      console.log('列表查询111111', e)
      this.search = e
      this.getList()
    },
    tabChange(i) {
    ...
    getList() {
      const that = this
      this.loading = true
      const type = 2 // 1、全部订单 2、我的订单 3、我的审核
      ...
      const params = {
        ...
        liveRoomCode: this.search.liveRoomCode, // 直播间ID
        type,
        confirmUser: this.search.confirmUser
      }
      getQrcodeOrderPage(params)
        .then((res) => {
          that.loading = false
          that.tableData = (res.data?.records || res.data?.items || res.data) ?? []
    ...
    // 跳转订单详情
    gotoDetail(row) {
      if (row.confirmUser === 0) {
        return
      }
      console.log('row', row.examineStatus)
      if (this.seeDetailModel) {
        this.$router.push({ path: `/business/orderManage/orderDetail/${row.orderNo}/${row.examineStatus}/我的扫码录单` })
      } else {
        const date = new Date().getTime()
        const routeUrl = this.$router.resolve(`/business/orderManage/orderDetail/${row.orderNo}/${row.examineStatus}/${date}`)
    ...
    changePrice() {
      ...
      this.changePriceModel.changePrice = Number(this.changePriceModel.changePrice).toFixed(2)
    },
    // 改价弹框确认按钮
    handleChangePrice() {
    ...
    openFullRefund(row, e) {
      ...
      this.refundModel.prcie = e.balanceFee > 0 ? Number(e.balanceFee / 100).toFixed(2) : '0.00'
    ...
    setRefundPrice(val) {
      if (this.refundModel.refundPrice > 0) {
        this.refundModel.refundPrice = Number(this.refundModel.refundPrice).toFixed(2)
      }
    },
    // 关闭退款弹窗
    onClickOffRefunds() {
