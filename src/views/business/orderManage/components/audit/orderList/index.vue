<template>
  <div v-loading="loading" class="page_wrap">
                ...
                <div class="table-title-row-l">
                  <div class="order-box">
                    ...
                    <div class="title-item title-item-font">
                      渠道平台: <span>{{ item.channelSource | getOptionsValue(channelSourceType) }}</span>
                    </div>
                  ...
                  <div v-if="item.auditType" class="audit-list">审核类型：{{ item.auditType | getOptionsValue(auditTypeType) }}</div>
                  <!-- <el-button type="text" @click="gotoDetail(item)">订单详情</el-button> -->
                </div>
                <div class="table-title-row-r" />
              </div>
              <div class="table-body">
                <div v-for="(listItem, index) in item.list" :key="index" class="table-body-row" :class="index === item.list.length - 1?'table-body-row-box':''" :style="{marginBottom: showBorder && item.list.length - 1 == index ?'16px':'' }">
                  <div class="body-item">
                  ...
                  <div v-if="operationColumn" class="body-item">
                    ...
                    <el-button
                      v-if="search.activeName === '我的审批' && operation && item.auditType===2 && [0,1].includes(item.examineStatus)"
                      v-permission="['system:order:myreview', permsList]"
                      type="text"
                      class="el-dropdown-link"
                      @click="gotoClaimOrder(item)"
                    >审核</el-button>
                    <el-button
                      v-if="search.activeName === '我的审批' && operation && item.auditType===2 && [2,3].includes(item.examineStatus)"
                      v-permission="['system:order:myreview', permsList]"
                      type="text"
                      class="el-dropdown-link"
                      @click="gotoReceiveDetail(item)"
                    >审核记录</el-button>
                    ...
                    >
                      ...
                      <el-dropdown-menu slot="dropdown">
                        ...
                        <el-dropdown-item
                          v-if="search.activeName === '全部订单' && (item.orderStatus === 2 || item.orderStatus === 6) && item.isTransfer === 0 && item.orderConfigId === 1 && (item.channelSource === 8 || item.channelSource ===9)"
    ...
    <!-- 订单认领 审核弹框 -->
    <receiveDetail ref="receiveDetailRef" :review="review" @refresh="getList" />
  </div>
</template>
...
import receiveDetail from '@/views/business/orderManage/components/search/receiveDetail.vue'
export default {
  name: 'OrderList',
  components: {
    AppStatus,
  ...
  methods: {
    ...
    getList() {
      ...
      const params = {
        ...
        type,
        auditType: this.search.auditType
      }
      getOrderPage(params)
        .then((res) => {
          that.loading = false
    ...
    gotoDetail(row) {
      ...
      if (this.seeDetailModel) {
        this.$router.push({ path: `/business/orderManage/orderDetail/${row.orderNo}/${row.examineStatus}/${this.search.activeName}` })
      } else {
    ...
    gotoClaimOrder(item) {
      this.review = true
      this.$refs.receiveDetailRef.open(item.claimId, item.examineStatus)
    },
    // 我的审核 审核
