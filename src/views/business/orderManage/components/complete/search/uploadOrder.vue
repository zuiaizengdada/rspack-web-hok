<template>
  <el-dialog v-if="visible" class="dialog-container" :close-on-click-modal="false" title="批量补齐导入" width="600px" :visible.sync="visible" append-to-body>
    <el-upload
      ref="upload"
      width="30%"
    ...
    >
      ...
      <p class="pt-3" style="font-size:12px;color: #9c9c9c;">一次最多导入1000条补齐订单</p>
    </el-upload>
...
import { getModalOrderExport, importComOrder } from '@/api/business'
...
export default {
  ...
  methods: {
    ...
    async fileRequest(e) {
      ...
      const { data } = await importComOrder(formData)
      if (data && data.errorNumber !== 0) {
        this.errorMsg = true
        this.link = data.errUrl
        this.successNum = data.sucessNumber
        this.errorNum = data.errorNumber
        return
      }
      this.errorMsg = false
      this.visible = false
