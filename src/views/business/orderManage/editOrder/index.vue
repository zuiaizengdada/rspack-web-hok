  ...
  <div class="entry-box">
    <div v-if="confirm" class="add-order-warp">
      <div class="crumbs-box">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item class="nav-add">修改订单</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!-- crumbs end -->
      <el-form ref="form" :model="form" label-width="80px" :rules="rules" class="form-box">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="成交来源" prop="channel">
              <el-select v-model="form.channel" placeholder="请选择" disabled class="w-296">
                <el-option
          ...
          <el-col :span="16">
            <el-form-item label="关联订单" prop="orderNo">
              <div class="search-order">
                <el-input
                  v-model="form.giveParentOrderNo"
                  maxlength="50"
                  class="w-296 search-in"
                />
                <el-button class="button-h32" type="primary" size="small" @click="selectThirdShop(form.giveParentOrderNo)">查询</el-button>
                <!-- <el-button v-permission="['system:order:findOrderNo', permsList]" class="button-h32" type="primary" size="small" @click="selectThirdShop(form.giveParentOrderNo)">查询</el-button> -->
        ...
        <el-row v-if="selectGiftType === '关联订单赠送' && selectOrderType === '赠送'">
          <div class="order-list">
            <order-list ref="orderList" :operation="false" @sizeChange="receiveSizeChange" @tableDataNum="receiveTableDataNum" @tableDataPrecisen="tableDataPrecisen" />
          </div>
        ...
        <div v-else>
          <div v-for="(orderObject,index) in form.materialGoodsOrderVoList" :key="index">
                ...
                <div class="table-box">
                  ...
                  >
                    ...
                    <span class="table-txt">- {{ scope.row.goodsPreferentialPrice }}</span>
                      ...
                      <template slot-scope="scope">
                        <div v-if="orderObject.goodsData" class="table-in">
            ...
            <el-row v-if="orderObject.optionsSpecs.length > 0">
              <el-form-item
                label="商品规格"
            ...
            <el-row v-if="selectOrderType === '新购' && orderObject.goodsData">
                ...
                <div class="upload-img">
                  <div v-for="(items, i) in form.imageList" :key="i">
                    <uploadItem
                      ...
                      :url="items.url"
                      :auth-progress="items.authProgress"
                      @del="delFileForText(i)"
                      @again="alignUploadForText(item, i,index)"
                    />
                  </div>
                  <el-upload
                    v-if="form.imageList && form.imageList.length < 6"
        ...
        <el-row>
          ...
          <el-col :span="12">
            ...
            <el-form-item label="用户账号" :prop="form.userType === 'phone' ? 'user.phone' : 'user.email'" class="user-account">
              ...
              <el-select v-model="form.userType" placeholder="请选择" style="width: 90px;" @change="changeUserType">
                <el-option label="手机号" value="phone" />
                <el-option label="邮箱" value="email" />
              </el-select>
              <el-input v-if="form.userType === 'phone'" v-model="form.user.phone" style="margin-top: 2px;" class="w-350" maxlength="11" placeholder="请输入用户手机号" rule>
                <el-select slot="prepend" v-model="form.user.phoneAreaCode" placeholder="请选择" @change="changeAreaCodes">
          ...
          <el-col :span="12">
            <el-form-item label="所在行业">
              <el-cascader
                ...
                clearable
              />
              <el-select v-model="form.firm.industry" placeholder="请选择" class="w-296">
                <el-option
                  v-for="item in optionsType"
                  :key="item.id"
                  :label="item.type"
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="公司地址">
              <AppChinaArea v-if="showAddress" v-model="form.firm.firmAddress" propsvalue="name" class="w-296" />
              <el-select v-model="form.firm.firmAddress" placeholder="请选择" class="w-296">
                <el-option
                  v-for="item in optionsType"
                  :key="item.id"
                  :label="item.type"
      ...
      <div v-if="selectOrderType==='' || selectOrderType === '新购' || selectOrderType === '预存'" class="water-header">
        <div class="water-title common-title-width">交易信息</div>
                ...
                <el-col :span="12">
                  <el-form-item
                    ...
                    :rules="[{ required: true, trigger: 'blur' ,validator:
                      validatePass}]"
                  >
                    <el-input-number v-model="item.price" controls-position="right" :min="0" :max="999999" :precision="2" class="w-296" />
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row class="m-t20">
      ...
      <div v-else-if="selectOrderType==='' || selectOrderType === '新购'" class="water-list">
              ...
              <el-row class="m-t20">
                <el-col :span="12">
                  <div class="flex-box">
                    ...
                    <el-form-item
                      v-if="item.optionsUnit.length && item.channel
                        !=optionsChannelValue"
                      :prop="'list.' + index + '.unit'"
                      :rules="[{ required: true, message: '请选择收款单位', trigger: 'change' }]"
                    >
                ...
                <el-col v-if="item.channel != optionsChannelValue" :span="12">
                  <el-form-item label="收款账号">{{ item.optionsUnit.length ? item.account : '--' }}</el-form-item>
                </el-col>
                <el-col v-if="item.channel === optionsChannelValue" :span="12">
                  <el-form-item
                ...
                <el-col v-if="item.channel === optionsChannelValue" :span="12">
                  <el-form-item
                    ...
                    :rules="[{ required: true, trigger: 'blur' ,validator:
                      validatePass}]"
                  >
                    <el-input-number v-model="item.price" :disabled="!moneyShowFlag || !moneyThetue" controls-position="right" :min="0" :max="999999" :precision="2" class="w-296" />
                  </el-form-item>
                </el-col>
              </el-row>
              <div v-if="item.channel === optionsChannelValue">
                <el-row class="m-t20">
                  <el-col :span="12">
                    <el-form-item
                      label="交易凭证"
      ...
      <div v-if="selectOrderType==='' || selectOrderType === '新购'" class="pay-box">
        <div v-if="form.materialGoodsOrderVoList && goodsAllPrice > 0" class="price-num" :class="goodsAllPrice > 0 ? '':'m-b16'">商品金额：¥{{ goodsAllPrice }}</div>
        <!-- <div class="price-num m-b16">商品金额：¥{{ goodsAllPrice }}</div> -->
        <div v-if="form.materialGoodsOrderVoList && allYouhuiPay > 0" class="price-num m-b16">优惠金额：-¥{{ allYouhuiPay }}</div>
        ...
        <div v-if="Number(allShouldPay) > Number(paidIn)" class="price-tomast">实付金额不能少于应付金额</div>
      </div>
    ...
    <div v-else class="confirm-box">
      <confirm-order
        ...
        :paid-money-number="Number(paidMoneyNumber)"
        :paid-in="paidIn"
        @submitOK="submitOK"
        @submitNO="submitNO"
      />
...
import { getPhoneByOrderEmail } from '@/api/user/member'
import {
  getPhoneAreaCodes
...
import { getDepositGoods, getQueryMoneyByPhone, taskSelect } from '@/api/business/addOrder'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      channelSource: [{ id: 8, label: '博商销售成交', value: 8 }, { id: 9, label: '获客线下收款', value: 9 }],
      form: {
        orderType: { // 订单类型
          o_one: '',
          o_two: ''
  ...
  computed: {
    ...
    // watchInputData() { return this.form.goods.goodsData.length ? this.form.goods.goodsData[0].goodsPreferentialPrice : '' }
  },
  watch: {
    'formWater.list': {
      handler(val) {
        ...
        // console.log(this.amountPay, '监听这个this.amountPay')
        if (Number(this.amountPay) > 0) { paidIn >= Number(this.amountPay) ? this.paidInStatus = false : this.paidInStatus = true }
        // if (this.form.goods.goodsData.length) {
        //   if (this.form.goods.goodsData[0].goodsPreferentialPrice > 0) {
        //     paidIn > Number(this.amountPay) ? this.paidInStatus = false : this.paidInStatus = true
        //   }
        // }
      },
      deep: true,
  ...
  mounted() {
    ...
    getPhoneByOrderEmail(this.$route.params.id).then(res => {
      if (res.code === 1) {
        this.phoneNumber = res.data.userPhone ? res.data.userPhone : res.data.email
      }
    })
    this.getDetail()
  ...
  methods: {
    ...
    getDetail() {
      ...
      getOrderDetail(this.$route.params.id)
        .then((res) => {
          ...
          if (res.data.type === undefined || res.data.type === null || res.data.type === '') {
            res.data['type'] = ''
            this.orderType = ''
          }
          ...
          const form = {
            ...
            channel: detail.channelSource !== undefined ? detail.channelSource : '', // 成交来源
            orderNo: detail.orderNo !== undefined ? detail.orderNo : '', // 订单编号
            teacherName: {
              name: detail.enrollmentTeacherName !== undefined ? detail.enrollmentTeacherName : '',
              content: detail.enrollmentTeacherId !== undefined ? detail.enrollmentTeacherId : ''
            }, // 招生老师
            materialGoodsOrderVoList: [
              {
                id: '',
            ...
            giveDesc: detail.content !== undefined ? detail.content : '', // 赠送理由
            givePhoto: [],
            preferentialDesc: detail.discountContent !== undefined ? detail.discountContent : '', // 优惠原因
            sixp: detail.largeP !== undefined ? detail.largeP : '', // 6p
            sessions: detail.privateVenues !== undefined ? detail.privateVenues : '', // 场次
            user: { // 用户信息
              name: detail.addUserVo.realName,
              email: this.phoneNumber?.indexOf('@') > 0 ? this.phoneNumber : '',
              phoneAreaCode: this.phoneNumber?.indexOf('@') > 0 ? '' : detail.addUserVo.phoneAreaCode,
              phone: this.phoneNumber?.indexOf('@') > 0 ? '' : this.phoneNumber
            },
            userType: detail.addUserVo.email ? 'email' : 'phone',
            firm: { // 公司信息
              name: detail.addUserVo.company,
              job: detail.addUserVo.position,
              industry: detail.addUserVo.industry ? [detail.addUserVo.industry] : '',
              turnover: detail.addUserVo.turnover,
              business: detail.addUserVo.introduce,
              firmAddress: [],
              detailAddress: ''
            },
            giveParentOrderNo: detail.giveParentOrderNo ? detail.giveParentOrderNo : '',
            taskId: detail.taskId
          }
          if (form.channel === 17) {
            form.channel = detail.transactionSource
          }
          this.paidMoneyNumber = Number(detail.payerTotal)
          form.materialGoodsOrderVoList = []
          // 商品信息及规格
          if (detail.list.length) {
            detail.list.map(async (v, i) => {
              let goodsType = ''
              if (v.goodsType === 0) {
                goodsType = '图片'
              } else if (v.goodsType === 1) {
                goodsType = '音频'
              } else if (v.goodsType === 2) {
                goodsType = '视频'
              } else if (v.goodsType === 3) {
                goodsType = '专栏'
              } else if (v.goodsType === 4) {
                goodsType = '大专栏'
              } else if (v.goodsType === 5) {
                goodsType = '线下课'
              } else if (v.goodsType === 6) {
                goodsType = '直播'
              }
              const optionsSpecsArray = []
              if (v.materialSpecVo) {
                const searchGoods = await findGoodsIdByList({ goodsId: v.goodsId })
                searchGoods.data.some(obj => {
                  if (obj.specId === v.materialSpecVo.specId) {
                    const materObj = {
                      id: obj.specId,
                      value: obj.specId,
                      label: obj.sort + '-' + obj.specName + '-' + Number(obj.salePrice / 100).toFixed(2),
                      specId: obj.specId,
                      content: obj
                    }
                    optionsSpecsArray.push(materObj)
                  }
                })
              }
              const obj = {
                id: v.goodsId,
                contentName: v.goodsName,
                goodsData: [{
                  id: v.goodsId,
                  goodsName: v.goodsName,
                  goodsType,
                  goodsPrice: Number(v.goodsPrices).toFixed(2),
                  goodsPreferentialPrice: detail.type === '赠送' ? 0 : v.discountsFee, // 优惠金额
                  goodsNum: v.goodsNum,
                  image: v.coverUrl,
                  goodsTypeNum: v.goodsType,
                  coverVerticalUrl: v.coverVerticalUrl,
                  preferentialDesc: v.discountContent
                }],
                tableShow: true, // tab默认关闭
                specsId: v.materialSpecVo !== undefined ? v.materialSpecVo.specId : '',
                specs: v.materialSpecVo !== undefined ? v.materialSpecVo : '',
                optionsSpecs: optionsSpecsArray
              }
              this.allShouldPay += Number(v.goodsPrices)
              if (detail.type !== '预存') {
                form.materialGoodsOrderVoList.push(obj)
              } else {
                form.materialGoodsOrderVoList.push({
                  id: '',
                  contentName: '',
                  goodsData: [
                    {
          ...
          // 公司地址解析
          console.log('detail.addUserVo !== undefined', detail.addUserVo !== undefined)
          if (detail.addUserVo !== undefined) {
            if (detail.addUserVo.areaName) {
              console.log('detail.addUserVo.areaName', detail.addUserVo.areaName)
              const areaName = detail.addUserVo.areaName.split(',')
              form.firm.firmAddress = [areaName[0], areaName[1], areaName[2]]
              form.firm.detailAddress = detail.addUserVo.address
            } else {
              form.firm.detailAddress = detail.addUserVo.address
            }
          }
          // this.goodsAllPrice = detail.goodsTotalFee
          this.$nextTick(() => {
            setTimeout(() => {
              // 流水
              if (detail.paymentInfoVoList !== undefined && detail.paymentInfoVoList.length) {
                detail.paymentInfoVoList.map((v, i) => {
                  if (v.channel === '钱包支付') {
                    this.moneyBagLocked = true
                    console.log('钱包支付的方式', v)
                    if (v.accountType === 1) {
                      this.moneyBag.userType = 'phone'
                      this.moneyBag.user.phone = this.moneyBag.userType === 'phone' ? v.paymentUserPhone : '',
                      this.moneyBag.user.phoneAreaCode = this.moneyBag.userType === 'phone' ? v.phoneAreaCode + '' : ''
                    } else if (v.accountType === 2) {
                      this.moneyBag.userType = 'email'
                      this.moneyBag.user.email = v.email
                    }
                  }
                  const imageUrl = []
                  if (v.voucherList !== undefined && v.voucherList !== null) {
                    if (v.voucherList?.length > 0) { // 交易凭证
                      v.voucherList.map(v => {
                        imageUrl.push({
                          file: '', // 文件对象
                          name: '', // 文件名
                  ...
                  this.optionsChannel.map(e => {
                    if (e.label === v.channel) {
                      ...
                      e.companyVo.map(q => {
                        optionsUnit.push({
                          label: q.company, value: q.id, id: q.id, account: q.account
                        })
                      ...
                      listWater.push({
                        ...
                        time: v.payTime ? parseTime(new Date(v.payTime).getTime(), '{y}-{m}-{d} {h}:{i}:{s}') : '--', // 支付时间
                        price: v.payerTotal, // 金额
                        optionsUnit, // 单位可选
                        channelLabel: e.label, // 渠道名称
                        paymentUserPhone: this.moneyBag.userType === 'phone' ? v.paymentUserPhone : v.email,
                        imageUrl // 交易凭证
                      })
                    }
                  })
              ...
              // 订单类型
              if (detail.type === '新购') {
                this.selectOrderType = '新购'
                this.optionsType.map(v => {
                  if (v.type === detail.type) {
                    form.orderType.o_one = v.id
                    // this.selectOrderType = v.type
                  }
                })
              } else if (detail.type === '预存') {
                this.selectOrderType = '预存'
                this.optionsType.map(v => {
                  if (v.type === detail.type) {
                    form.orderType.o_one = v.id
                    // this.selectOrderType = v.type
                ...
                if (detail.giveVoucherList.length > 0) { // 赠送处理
                  detail.giveVoucherList.map(v => {
                    givePhoto.push({
                      file: '', // 文件对象
                      name: '', // 文件名
                      status: 3, // 状态
                  ...
                  form.givePhoto = givePhoto
                }
                this.selectOrderType = '赠送'
                this.selectGiftType = detail.type
                this.optionsType.map(v => {
                  if (v.type === '赠送') {
                    this.optionsGiftType = v.childrenList
                    v.childrenList.map(e => {
                      console.log(e, 'e')
                      if (e.type === detail.type) {
                        form.orderType.o_one = v.id
                        form.orderType.o_two = e.id
                        // this.selectOrderType = '赠送'
                        // this.selectGiftType = e.type
                      }
                    })
                  }
                  // 关联订单赠送需要判断 自动查询
                ...
                this.$nextTick(() => {
                  detail.type === '关联订单赠送' ? this.selectThirdShop(detail.giveParentOrderNo) : ''
                })
              }

              this.form = form
              // this.moneyBag = deepClone({ ... this.form })
              this.formWater.list = listWater
              this.formWater.list.some((item, i) => {
                if (item.channel === this.optionsChannelValue) {
                  this.toSearchMoney(i)
                }
              })
    ...
    selectThirdShop(orderNo) {
      ...
      orderNo === '-' ? this.form.giveParentOrderNo = '' : ''
    ...
    onChangeGoodsSpecs(e, index) {
      ...
      console.log(index)
      // this.form.materialGoodsOrderVoList[index].specId = e
      // this.$refs['form'].clearValidate(['materialGoodsOrderVoList.' + index + '.specsId'])
      this.form.materialGoodsOrderVoList[index].optionsSpecs.map((v, i) => {
        if (v.id === e) {
          ...
          this.form.materialGoodsOrderVoList[index].goodsData[0].goodsPreferentialPrice = Number(0).toFixed(2)
    ...
    onClickTabDel(index) {
      ...
      /* this.form.materialGoodsOrderVoList[index] = {
        id: '',
        contentName: '',
        goodsData: [
          {
        ...
        if (v.id === e) {
          ...
          if (v.type === '新购') {
            ...
            this.form.materialGoodsOrderVoList?.some(item => {
              if (item.goodsData && item.goodsData.length > 0) {
                item.goodsData[0].goodsPreferentialPrice = 0
              }
            })
            this.countAllData()
            this.formWater.list?.some(item => {
              this.paidIn = Number(this.paidIn) + Number(item.price)
            })
            this.paidIn = this.paidIn.toFixed(2)
          } else if (v.type === '预存') {
            this.$refs.formWater.clearValidate()
            this.formWater.list.some((item, j) => {
              if (item.channel === this.optionsChannelValue) {
                this.formWater.list.splice(j, 1)
              }
            })
            console.log(this.$refs.formWater)
    ...
    getOptionsUnitChange(e, index) {
      ...
      this.optionsChannel.map((v, i) => {
        if (v.id === e) {
          ...
          console.log('this.optionsChannelValue: ', this.optionsChannelValue)
          ...
          if (v.id === this.optionsChannelValue) {
            ...
            this.$set(this.formWater.list[index], 'price', '')
          } else {
            this.formWater.list.some(item => {
              if (item.channel === this.optionsChannelValue) {
                this.moneyBagLocked = true
    ...
    changeUx(item, index) {
      if (this.moneyBag.userType === 'phone') {
        ...
        this.moneyBag.user.phoneAreaCode = this.moneyBag.user.phoneAreaCode ? this.moneyBag.user.phoneAreaCode : '86'
        this.$nextTick(() => {
          setTimeout(() => {
        ...
        item.paymentUserPhone = this.moneyBag.user.email ? this.moneyBag.user.email : ''
        this.$nextTick(() => {
          setTimeout(() => {
            const str = 'list.' + index + '.paymentUserPhone'
            this.$refs.formWater.clearValidate(str)
    ...
    getIGoodsPreferentialPrice(index) {
      if (this.form.materialGoodsOrderVoList[index].goodsData.length) {
        ...
        if (Number(this.form.materialGoodsOrderVoList[index].goodsData[0].goodsPreferentialPrice) > 0 && Number(this.form.materialGoodsOrderVoList[index].goodsData[0].goodsPreferentialPrice) >= price) {
      ...
      console.log('this.form.materialGoodsOrderVoList[index].goodsData[0].amountPay:', this.form.materialGoodsOrderVoList[index].goodsData[0])
      this.countAllData()
    },
    countAllData() {
      console.log('this.form.materialGoodsOrderVoList:', this.form.materialGoodsOrderVoList)
    ...
    changeUserType() {
      if (this.form.userType === 'phone') {
        this.form.user.phoneAreaCode = this.form.user.phoneAreaCode ? this.form.user.phoneAreaCode : '86'
      }
    },
    async uploadGive(e) {
      // this.$refs.uploadGive[0].clearFiles()
    ...
    submitOK() {
      ...
      const addUserVo = {
        areaName: form.firm.firmAddress !== undefined && form.firm.firmAddress.length ? form.firm.firmAddress.join(',') : '', // 公司所在地
        address: form.firm.detailAddress !== undefined && form.firm.detailAddress !== undefined ? form.firm.detailAddress : '',
        company: form.firm.name !== undefined ? form.firm.name : '', // 公司名称
        industry: form.firm.industry !== undefined ? form.firm.industry.length ? form.firm.industry[0] : '' : '', // 行业
        introduce: form.firm.business !== undefined ? form.firm.business : '', // 主营业务
        ...
        position: form.firm.job !== undefined ? form.firm.job : '', // 职位
        realName: form.user.name !== undefined ? form.user.name : '', // 用户名称
        turnover: form.firm.turnover !== undefined ? form.firm.turnover : '' // 年营业额
      }

      // 处理流水
      const addPaymentInfoList = []
      if (this.formWater.list.length) {
      ...
      this.selectOrderType === '赠送' ? params['content'] = form.giveDesc : '' // 赠单原因
      ...
      this.selectOrderType === '赠送' && this.selectGiftType === '关联订单赠送' ? params['giveParentOrderNo'] = form.giveParentOrderNo : '' // 订单号
