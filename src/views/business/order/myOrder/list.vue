  ...
  <div v-loading="loading" class="page_wrap">
    <div class="content">
      <div class="filter_top">
        <myFilterTop :search="search" @search="searchFn" />
      </div>

      <div class="table-module">
        <div class="m-b-20">
          <el-button v-permission="['system:order:export', permsList]" :loading="exportLoading" @click="goodsOrderExport">导出</el-button>
        </div>
        <div class="main-table">
          ...
          <div v-else class="table-body">
            <div v-for="item in tableData" :key="item.orderNo">
              <div class="table-title-row">
                ...
                <div v-if="item.paymentType >=0 " class="title-item">
                  支付方式: {{ item.paymentType | getOptionsValue(paymentType_options) }}
                </div>
                <div v-if="item.cid && item.cid > 0" class="title-item">付费推广</div>
                <!-- <div class="title-item">收款方式</div> -->
                <!-- <div class="title-item">普通订单</div> -->
              </div>
              <div v-for="(list, index) in item.list" :key="index" class="table-body-row">
                <div class="body-item">
                  <div class="goods-img-info">
                    <div class="goods-img">
                      <el-image :src="list.coverUrl" fit="contain" style="width: 100%; height: 100%" />
                    </div>
                    <div class="goods-info">
                      <div class="EC3" :title="list.goodsName">{{ list.goodsName }}</div>
                      <div class="goods-type">
                        {{ goodsType_obj[item.goodsType] }}
                      </div>
                      <div class="goods-period">
                        获得有效期时长:
                        <span v-if="list.saleValidity === 0">长期有效</span>
                        <span v-if="list.saleValidity === 1">{{ list.validityDays }}</span>
                        <span v-if="list.saleValidity === 2">{{
                          listItem.fixedTime
                        }}</span>
                      </div>
                    </div>
                ...
                <div class="body-item">
                  <AppStatus v-if="list.orderStatus" :status="orderStatus[list.orderStatus].type">
                    <span>{{ orderStatus[list.orderStatus].name }}</span>
                  </AppStatus>
                </div>
                <div class="body-item">
                  <template v-if="[5, 6, 7].includes(list.orderStatus)">
                    <AppStatus v-if="list.orderStatus" :status="orderStatus[list.orderStatus].type">
                      <span>{{ orderStatus[list.orderStatus].name }}</span>
                    </AppStatus>
                  </template>
                </div>
                <div class="body-item">{{ (list.goodsPrices || 0) | filtersMoney }}</div>
                <div class="body-item">{{ list.goodsNum }}</div>
                <div class="body-item">
                  {{ list.payerTotal | filterPayerTotal }} 元
                  <div v-if="list.payerBeforeTotal" class="c_red m-t-10">
                    改价前: {{ list.payerBeforeTotal | filtersMoney }} 元
                  </div>
                </div>
                <div class="body-item">{{ item.userName }}</div>
                <div class="body-item">
                  {{ item.channelSource | getOptionsValue(channelSourceType) }}
                  <span v-if="item.channelSource && (item.userTerminal || list.thirdShopName)">|</span>
                  <template v-if="item.channelSource === 3">
                    {{ item.userTerminal | getOptionsValue(userTerminal) }}
                  </template>
                  <template v-if="item.channelSource && item.channelSource !== 3">
                    {{ list.thirdShopName }}
                  </template>
                </div>
                <div class="body-item">
                  <el-button type="text" @click="gotoDetail(list)">订单详情</el-button>

                  <el-dropdown
                    v-if="
                      [5].includes(list.goodsType) ||
                        [1].includes(list.orderStatus) ||
                        ([1, 2, 4].includes(item.channelSource) && list.orderStatus === 2)
                    "
                  >
                    ...
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item
                        v-if="[5].includes(list.goodsType)"
                        v-permission="['system:student:query', permsList]"
                        type="text"
                        @click.native="openMemberModel(list)"
                      >成员信息</el-dropdown-item>
                      <el-dropdown-item
                        v-if="[1].includes(list.orderStatus)"
                        v-permission="['system:order:price', permsList]"
                        type="text"
                        @click.native="openChangePriceModelModel(item)"
                      >改价</el-dropdown-item>
                      <el-dropdown-item
                        v-if="[1, 2, 4].includes(item.channelSource) && list.orderStatus === 2"
                        v-permission="['system:order:send', permsList]"
                        type="text"
                        @click.native="sendActivationSMS(list)"
                      >激活短信</el-dropdown-item>
                      <el-dropdown-item
                        v-if="[1, 2, 4].includes(item.channelSource) && list.orderStatus === 2"
                        v-activation="list.orderNo"
            ...
            <template slot-scope="{ row }">
              <div class="fl">
                <div class="goods_picture m-r-10">
                  <el-image fit="contain" :src="row.coverUrl" />
                </div>
...
<script>
import myFilterTop from '../myFilterTop.vue'
import { getMyOrderPage, getStudentList, goodsOrderPrice, goodsOrderExport, goodActivesSendSMS } from '@/api/business'
...
export default {
  ...
  computed: {
    ...mapGetters(['goodsType_obj', 'channelSourceType', 'paymentType_options'])
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      const params = {
        ...
        afterStatus: this.search.afterStatus || '',
    ...
    gotoDetail(row) {
      console.log(row)
      this.$router.push({ path: `/business/myOrder/myOrder/${row.orderNo}` })
    },
    // 打开成员信息弹框
    openMemberModel(row) {
      this.memberList = []
