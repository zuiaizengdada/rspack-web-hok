<template>
  <div class="salesChannels">
    ...
    <div class="sales-content flexbox">
        ...
        >
          <div
            v-if="
              [1, 2, 4, 10, 11, 12, 13, 15, 14, 16, 17, 20].includes(
                ...
                (selectIndex == item.orderChannel || selectIndex == -1)
            "
            :style="{
              width:
                selectIndex == -1
                  ? 587 - 150 * (index / sellTotalData.length) + 'px'
                  : '587px',
              marginBottom:
                index === sellTotalData.length - 1 ||
                selectIndex == item.orderChannel
                  ? '0px'
                  : '20px'
            }"
            :class="[
          ...
          >
            <div class="flexbox">
              ...
              >￥ {{ filterPrice(item.totalCount) }}</span>
            </div>
            <span class="flexbox fl_ai_c">
              <span style="font-size: 18px">{{
                fillDataString(item.totalRatio)
              }}</span>
              <span
                class="right-enter"
                :style="
          ...
          <div
            v-if="selectIndex == item.orderChannel"
            class="child-box overflowOuto"
          >
            <div
              v-for="ele in shopData"
              :key="ele.shopId"
              class="childList fl fl_ai_c"
              :style="{ width: '587px' }"
              @click="goShop(item, ele)"
            >
              <div>
                <span>{{ ele.shopName }}</span>
              </div>
              <div class="fl fl_ai_c">
                <div class="childPrice fl">
                  <span class="childList-price">¥{{ ele.totalCount }}</span>
                  <div class="fl fl_ai_c childListRatio">
                    <span> {{ fillDataString(ele.totalRatio) }} </span>
                  </div>
                </div>
                <span class="child-right-enter" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div
        class="sales-broken"
        :style="selectIndex == -1 ? 'height:340px' : 'height:395px'"
      >
...
export default {
  ...
  created() {
    console.log(
      'slsleidjkfsdljekwljfkdlsajfklweujjklfdsa: ',
      this.sellTotalData
    )
  },
  methods: {
    ...
    // 售卖渠道跳转
    goDoudian(item, index) {
      const setBuObj = JSON.parse(
        sessionStorage.getItem('departmentDataSet') || '{}'
      )
      setBuObj.qudaoItem = item
      setBuObj.qudaoItemIndex = index
      setBuObj.qudaoSelectIndex = this.selectIndex
      sessionStorage.setItem('departmentDataSet', JSON.stringify(setBuObj))
      this.$emit('goPage', item, index)
    },
    goShop(item, ele) {
      this.$emit('goShop', item, ele)
    },
    // 初始化echarts
    initChart() {
      console.log(this.shopDataDayTotal, this.sellTotalDayData, '列表数据')
      ...
      const option = {
        ...
        series:
          this.selectIndex === -1
            ? this.sellTotalDayData
            : this.shopDataDayTotal
      }
      option.legend = {
        data:
          this.selectIndex === -1
            ? this.sellTotalDayData.map(e => e.name)
            : this.shopDataDayTotal.map(e => e.name)
      }
