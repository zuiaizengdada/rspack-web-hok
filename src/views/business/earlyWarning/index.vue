  ...
  <div class="page-wrap">
    ...
    >
      <div slot="action" slot-scope="scope">
        <el-button
          v-permission="['system:warning:edit', permsList]"
          type="text"
        ...
        <el-dropdown>
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-if="hideSource(scope.row)" v-permission="['system:order:active:send', permsList]" command="1">
              <el-button type="text" @click.native="sendActivationSMS(scope.row)">激活短信</el-button>
            </el-dropdown-item>
            <el-dropdown-item v-if="hideSource(scope.row)" v-permission="['web:order:link', permsList]" command="2">
              <el-button v-activation="scope.row.orderNo" type="text">激活链接</el-button>
            </el-dropdown-item>
            <el-dropdown-item v-if="hideManual(scope.row)" v-permission="['system:order:manual', permsList]" command="3">
      ...
      <template slot="channelSource" slot-scope="scope">
        {{ scope.row.channelSource | getOptionsValue(channelSourceType) }}
      </template>
    </MPageLayout>
    <!-- 跟进人员弹框 -->
...
<script>
import { orderWarning, orderWarningClose, goodActivesSendSMS, addFollowSave, orderManual } from '@/api/business'
...
export default {
  ...
  data() {
    return {
      ...
      slotArr: ['channelSource', 'headerRight', 'headerLeft', 'orderNo', 'action', 'userPhone', 'footer', 'followers'],
      config: {
        tableData: [],
  ...
  methods: {
    ...
    async getList() {
      ...
      const { data } = await orderWarning(params)
    ...
    goOrderDetails(row) {
      const routeUrl = this.$router.resolve({
        path: `/business/order/order/${row.orderNo}`
      })
