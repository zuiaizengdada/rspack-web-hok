<template>
  <div class="big-wrapper">
    <div v-loading="loading" class="wrapper-online-gmv-show" :class="nowItem.bizType === activeBiz?'active-show':''" @click="changeTheBizType(nowItem)">
      ...
      <div class="gmv-center">
        ...
        <div class="gmv-center-detail">
          ...
          <div v-if="nowItem.bizType === activeBiz" class="huanbi-cls">
            <span>{{ nowItem.amountLinkRatio>1000?'1000%+':nowItem.amountLinkRatio+'%' }}</span>
          ...
          <div v-else class="huanbi-cls">
            <span :class="nowItem.amountLinkRatio>0?'green-span':'red-span'">{{ nowItem.amountLinkRatio>1000?'1000%+':nowItem.amountLinkRatio+'%' }}</span>
      ...
      <div class="gmv-right">
        <span :class="(formGmvShow.employeeDeptId && formGmvShow.chooseType === 2)?'hidden-cls':''" @click.stop="toListData(nowItem)">详情&nbsp;&nbsp;></span>
      </div>
    </div>
    <dataRow ref="dataRowRef" :loadding="loading" :contrast-string="contrastString" :form-gmv-show="formGmvShow" :biz-type-active="activeBiz" @changeTheBizType="changeTheBizType" @changeToList="changeToList" />
  </div>
</template>
  ...
  methods: {
    ...
    init(item) {
      ...
      if (item.chooseType === 1) {
        ...
        const paramsReve = this.returnParams(1)
        revenueGroupByBizType(paramsReve).then(res => {
          if (res.code === 1 && res.data) {
            this.dataArray = res.data
            this.$emit('setBizArray', this.dataArray)
            this.dataArray.some((obj, index) => {
              if (obj.bizType === item.bizTypes[0]) {
                this.nowItem = obj
