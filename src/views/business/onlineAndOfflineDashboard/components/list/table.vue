...
export default {
  ...
  methods: {
    ...
    clearSort() {
      this.$refs.mobantable.clearSort()
    },
    getSummaryEmployee(params) {
      const { columns, data } = params
      const sums = []
      if (this.employeeObject?.employeeName && this.chooseType === 1) {
        columns.forEach((column, index) => {
          if (index === 0) {
            sums[index] = '总计'
            return
          }
          if (column.property === 'employeeDeptIdChainName') {
            sums[index] = this.employeeObject.employeeDeptIdChainName
            return
          }
          if (column.property === 'gmvAmount') {
            sums[index] = this.filterPrice(this.employeeObject.gmvAmount) + '(' + this.employeeObject.gmvOrderCount + '单)'
            return
          }
          if (column.property === 'revenueAmount') {
            sums[index] = this.filterPrice(this.employeeObject.revenueAmount) + '(' + this.employeeObject.revenueOrderCount + '单)'
            return
          }
          ...
          if (column.property === 'contemporaneousAmount') {
            sums[index] = this.filterPrice(this.employeeObject.contemporaneousAmount)
            return
          }
          if (column.property === 'increaseRate') {
            sums[index] = this.employeeObject.increaseRate > 1000 ? '1000%+' : this.employeeObject.increaseRate + '%'
            return
          }
        })
        return sums
      } else if (this.employeeObject?.expertName && this.chooseType === 2) {
        columns.forEach((column, index) => {
          if (index === 0) {
            sums[index] = '总计'
            return
          }
          if (column.property === 'expertName') {
            sums[index] = this.employeeObject.expertName
            return
          }
          if (column.property === 'gmvAmount') {
            sums[index] = this.filterPrice(this.employeeObject.gmvAmount) + '(' + this.employeeObject.gmvOrderCount + '单)'
        ...
        return sums
      } else if (this.employeeObject?.employeeDeptName && this.chooseType === 3) {
        columns.forEach((column, index) => {
          if (index === 0) {
            sums[index] = '总计'
            return
        ...
        return sums
      } else if (this.employeeObject?.itemName && this.chooseType === 4) {
        columns.forEach((column, index) => {
          if (index === 0) {
            sums[index] = '总计'
            return
    ...
    headerCellStyleFunc({ row, column, rowIndex, columnIndex }) {
      ...
      } else if ((this.formForList.bizTypeActive !== 3 && this.formForList.bizTypeActive !== 2) && (column.label === '总营收（单量）' || column.label === '总营收(单量)')) {
        return {
          background: '#EFF2F6',
          padding: '9px 0',
          color: '#0091ff',
    ...
    init(isResetPageIndex = true) {
      ...
      this.getList()
        .then(res => {
          ...
          if (this.formForList.bizTypeActive === 3) {
            this.tableData.sort((a, b) => b.gmvAmount - a.gmvAmount)
          } else if (this.formForList.bizTypeActive === 2) {
