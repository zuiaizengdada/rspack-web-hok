...
export default {
  ...
  data() {
    return {
      ...
      configList: [
        {
          name: '基本信息',
          approveStatus: 'basicInfo'
        },
        {
          name: '店铺信息',
          approveStatus: 'shopInfo'
        }
      ]
    }
  },
  ...
  methods: {
    ...
    initRouter() {
      try {
        // 获取物流设置相关的路由权限列表
        const routerList = getRoutesPermsList('/afterSale/generalSetting/logistics')
        ...
        // 处理默认路由跳转逻辑
        if (this.$route.name === 'generalSettingLogistics') {
