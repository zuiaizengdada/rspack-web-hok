<template>
  <div class="adSetting">
    <div class="adSettingLeft">
      <list ref="list" :title="title" :loading="loading" :current="current" :data="listData" :if-add="ifAdd" @onhandleAdd="onHandleAdd" @onClick="onHandleSwitch" @dragged="onHandleDragged" />
    </div>
...
export default {
  ...
  computed: {
    ...mapState({
      bannerList: state => state.homePageSetting.bannerList,
      navViewList: state => state.homePageSetting.navViewList
    })
  },
  watch: {
    currentType: {
  ...
  methods: {
    ...
    getDetail() {
      this.detail = {
        ...
        fileFormat: [],
        fileSize: 5 * 1024,
        remark: this.currentType === 'banner' ? '1、上传jpg,png图片，只能上传1张 \n 2、单张图片不能大于5M \n 3、建议尺寸：1029*288' : '1、上传jpg,png图片，只能上传1张 \n 2、单张图片不能大于5M \n 3、建议尺寸：108*108'
      }
      // const params = {
    ...
    getList() {
      if (this.currentType === 'banner') {
        this.listData = JSON.parse(JSON.stringify(this.bannerList))
      } else if (this.currentType === 'navView') {
        this.listData = JSON.parse(JSON.stringify(this.navViewList))
      } else {
        this.listData = []
      }
        ...
        if (obj.longTerm === 0) {
          ...
          if (terminalArr.length > 0) {
            this.$delModal({
              ...
              tips: `
                <div style="font-size: 16px;color: #333333;line-height: 24px;">显示终端（${terminalArr.join('/')}）不含长期有效的设置</div>
