...
// import { adInfoCreate, adInfoUpdate, adInfoRemove } from '@/api/shop/advertising.js'
import { saveHomepage, delHomepage, getHomepageDetailById } from '@/api/business/homeSetting/index.js'
...
export default {
  ...
  methods: {
    beforeUpload(file) {
      ...
      if (!isLtX) {
        this.$message.error(`上传文件大小不能超过 ${this.filterSize(this.detail.fileSize)}!`)
      }
      return isLtX
    },
    // 点击保存
    ...
    onhandleRemove() {
      this.$delModal({
        ...
        success: () => {
          this.myloading = true
          delHomepage(this.form.id).then(res => {
            this.myloading = false
    ...
    getAdDetail(id) {
      ...
      // const result = this.listData.find(v => v.id === id)
      // console.log(result, 'result')
      // if (!result) return
      return getHomepageDetailById({ id: id }).then(res => {
    ...
    onhandleAdd(data) {
      ...
      const params = {
        ...data,
        configType: { banner: 1, navView: 2 }[this.currentType]
      }
      saveHomepage(params).then(res => {
        this.myloading = false
    ...
    // 判断能否显示终端
    ifShowTerminal(key) {
      console.log(this.form.skipType, 'this.form.skipType')
