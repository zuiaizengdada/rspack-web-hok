<template>
  <div class="content">
    <div class="navList">
      <div class="navhead">
        <span>二级分组</span>
        <el-button icon="el-icon-plus" size="mini" type="text" @click="handleAddGroup">新增</el-button>
      </div>
      <draggable v-model="groupingList" class="navcontent" @end="handleDragEnd">
        <div v-for="item in groupingList" :key="item.id" :class="{ active: active === item.groupingId }" @click="handleItem(item)">
          <span>{{ item.groupingName }}</span>
          <div>
            <el-button size="mini" type="text" @click.stop="handleEditGroup(item)">编辑</el-button>
    ...
    <div class="contentList">
      <div class="content_header">
        <div class="header_left">
          <el-button v-if="active" type="primary" @click="selectGoods()">添加商品</el-button>
        </div>
      </div>

      <div class="content_table">
        <el-table ref="myTable" border :data="tableData" height="100%" style="width: 100%" :loading="loading" @selection-change="handleSelectionChange">
...
export default {
  ...
  data() {
    return {
      ...
      active: undefined,
      detailsData: { groupingName: '', type: 1 },
      // id: '17',
      groupingList: [],
      dialogVisible: false,
      loading: false,
  ...
  methods: {
    handleAddGroup() {
      if (this.groupingList.length >= 7) { return this.$message.warning('最多添加七个二级分组') }
      this.dialogVisible = true
      this.detailsData = { groupingName: '', type: 1, parentId: this.id }
    },
    handleEditGroup(data) {
      this.dialogVisible = true
      this.detailsData = { ...data, type: 2 }
    },
    handleItem(data) {
    ...
    async handleAddSubmit() {
      ...
      this.detailsData.type === 1
        ? await addGrouping(this.detailsData)
        : await groupingEdit(this.detailsData)
    ...
    getGroupingList() {
      ...
      return new Promise(async resolve => {
        const res = await groupingList({ parentId: this.id })
        if (res.code === 1) {
    ...
    _disassociateBatch(row) {
      this.$delModal({
        ...
        success: () => {
          ...
          groupingRelationDisassociate(data)
            .then(res => {
              this.getList()
              this.refresh()
            })
