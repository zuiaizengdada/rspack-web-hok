      ...
      <template slot="action" slot-scope="scope">
        <div>
          <el-button
            ...
            size="medium"
            @click="
              goPage('CouponCenterDetails', { couponNo: scope.row.couponNo })
            "
          >
            详情
          </el-button>
          <el-button
            ...
            size="medium"
            @click="
              goPage('CouponCenterAdd', {
                couponType: scope.row.couponType,
                couponNo: scope.row.couponNo
              })
            "
          >
            复制
...
export default {
  ...
  data() {
    return {
      ...
      config: {
        ...
        total: 0,
        tableData: [],
        basics: cloneDeep(couponMgt.table)
      }
    }
  },
  created() {
  ...
  methods: {
    async initOption() {
      try {
        const { data: couponTypesData } = await couponTypesApi()
        this.formOpts.optionsObj.couponType = [
          { message: '全部', code: '' },
          ...(couponTypesData || [])
        ]
        const { data: disData } = await discountMethodsApi()
    ...
    validateForm() {
      const req = {}
      for (const key in this.formOpts.formData) {
        if (this.formOpts.formData[key] !== '') {
          req[key] = this.formOpts.formData[key]
        }
      }
      return req
    },
    ...
    clearFn() {
      ...
      this.formOpts.formData = cloneDeep(couponMgt.form.formData)
      this.getTableInfo()
    },
    goPage(routerName, query = {}) {
      this.$router.push({
        name: routerName,
        query: {
          prevPath: this.$route.path + '?active=mgt',
          ...query
        }
      })
    },
