...
export default {
  ...
  methods: {
    ...
    handleCheckAll(val) {
      ...
      if (val) {
        this.$nextTick(() => {
          ...
          if (this.$refs[`FragmentList${this.checkIndex}`][0] && this.$refs[`FragmentList${this.checkIndex}`][0].geSelctAllStatus()) {
            // 获取是否全选中了  全选中直接到下一个
            this.checkIndex++
    ...
    check(res, val, isAll) {
      ...
      console.log(this.selectLength, this.checkedArray, '选中的数量')
