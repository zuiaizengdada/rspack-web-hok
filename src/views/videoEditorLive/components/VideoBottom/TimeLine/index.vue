<template>
  <div class="timeline-panel flex">
    ...
    <div v-for="item in linesRenderList" :key="item.t" class="scale" :class="{isWidth:isWidth}" :index="item.index">
      <div class="scalse">
...
export default {
  ...
  computed: {
    ...
    getTotalVideoDuration: function () {
      return this.video.totalVideoDuration
    }
  },
  watch: {
    // 缩放监听
    ...
    getTotalVideoDuration(val) {
      this.initTimeLine(val)
    }
  },
  mounted() {
    ...
    scrollDom.addEventListener('scroll', (e) => {
      ...
      console.log(Math.abs(scrollLeft - this.lastScrollPx), 222)
      // 滚动条拖到
      if (Math.abs(scrollLeft - this.lastScrollPx) > 8) {
        console.log('拖动滚动条', scrollLeftTime, scrollLeftIndex)
        ...
        console.log('自动播放', scrollLeftTime, scrollLeftIndex)
        // 自动播放
        // 当前渲染列表第20个刻度所代表的时间
        ...
        if (currentTimeS > current20S) {
          ...
          console.log('自动播放', this.linesRenderList, list)
  ...
  methods: {
    // 初始化刻度线
    initTimeLine(totalDuration) {
      const { isVideoScreenPortait, currentZoomEachSectionTime, totalVideoDuration, trueWidth } = this.video
      const width = trueWidth
      this.isWidth = !isVideoScreenPortait
      const duration = totalDuration || totalVideoDuration
      this.linesList = []
      if (duration > 0) {
        let n = Math.ceil(duration / currentZoomEachSectionTime) + 1 // n总节数，根据视频时长除以每节代表的秒数，得出总节数，小于24节,统一为24节
        n = n < 24 ? 24 : n
        for (let i = 0; i < n; i++) {
          const obj = {
            index: i,
