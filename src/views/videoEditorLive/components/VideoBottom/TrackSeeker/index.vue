...
export default {
  ...
  watch: {
    ...
    getCurrentTime(val) {
      ...
      if (val >= totalVideoDuration) {
        this.pausedTrackSeeker()
        this.updateVideoSeekTime(totalVideoDuration)
      }
    }
  },
  mounted() {
  ...
  methods: {
    ...
    initMove() {
      ...
      const isMoveMapsItem = this.video.isMoveMapsItem
      // 鼠标键按下，表示鼠标操作中
      $seekerContainer.addEventListener('mousedown', () => {
        if (isMoveMark || isMoveMapsItem) return false
        isMove = true
        isMoveComplete = true
      ...
      $tracksBox.addEventListener('mouseup', (e) => {
        isMove = false
        console.log(isMove, '鼠标抬起。。。')
      ...
      $tracksBox.addEventListener('click', (e) => {
        ...
        console.log(isMoveMark, isMoveMapsItem, '点击事件。。。')
        if (isMoveMark || isMoveMapsItem) return false
        const currentZoomSectionWidth = this.video.currentZommEachSectionWidth
        const durationTime = this.video.totalVideoDuration
