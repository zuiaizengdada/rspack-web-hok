<template>
  <div class="video-right-container relative">
    <!-- <p class="move-line" @mousedown="mousedownFun" /> -->
    <div v-if="!visibleMapsPanel" class="sub-content">
      ...
      <div class="content">
        <mark-list v-show="activeNavIndex === 0" />
        <subtitle-list v-show="activeNavIndex === 1" />
        <full-subtitle v-if="activeNavIndex === 2" />
      </div>
    </div>
    <div v-else class="maps-panel-content">
      <maps-panel :panel="activeMapsPanelData" />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    getCurrentMoveMapsItemData() {
      return this.video.currentMoveMapsItemData
    }
  },
  watch: {
    ...
    getCurrentMoveMapsItemData(data) {
      // const data = this.video.mapsList[this.video.currentDragMapsItemIndex]
      if (Object.keys(data).length > 0) {
        if (data.stickerType === '1') {
          this.visibleMapsPanel = true
          this.activeMapsPanelData = data
          this.activeNavIndex = -1
        } else {
          this.visibleMapsPanel = false
          this.activeNavIndex = 2
          this.activeMapsPanelData = {}
        }
      } else {
        this.visibleMapsPanel = false
        this.activeNavIndex = 2
  ...
  methods: {
    ...
    mousemoveFun(e) {
      if (this.isMove) {
        ...
        const maxWidth = documentWidth - videoListWidth - 600
