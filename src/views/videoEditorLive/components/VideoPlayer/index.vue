<template>
  <div class="play-container" style="height: 100%">
    ...
    <div class="player-content flex justify-center items-center">
      <div id="player" :style="{width:isWidth?'100%':'190px'}" />
      <canvas-fabric id="fabric" class="fabric-content" />
...
export default {
  ...
  computed: {
    ...
    getPlayVideoSeekTime() {
      return this.video.playSeekTime
    },
    getVideoDetail() {
    ...
    getCurrentPlayCutLineItem() {
      return this.video.currentPlayCutLineItem
    }
  ...
  watch: {
    ...
    getPlayVideoSeekTime(data) {
      const isPause = data.isPause ? 'pause' : 'play'
     this.playerSdk?.seek && this.playerSdk.seek(data.time, isPause)
    },
    getVideoDetail(val) {
      this.initPlay()
      this.videoName = val.name
      this.isWidth = val.width === 1920
    },
    ...
    getCurrentPlayCutLineItem(val) {
      if (Object.keys(val).length > 0) {
        this.videoName = val.videoData.name
      }
    }
  ...
  methods: {
    ...
    initPlay() {
      ...
      playerSdk.on('seeked', (player) => {
        ...
        // this.updateVideoSeekTime(player.currentTime, '播放器跳帧播放2.。。。')
      })
