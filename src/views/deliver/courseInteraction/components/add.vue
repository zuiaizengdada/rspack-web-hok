<template>
  <div class="groupDrawer">
      ...
      <div v-loading="loading" class="page_wrap">
        ...
        >
          <div class="m-t-20">
            ...
            <el-form-item label="参与条件" prop="partCond">
              <el-radio-group v-model="form.partCond">
                <el-radio :label="0" style="width: 100%" class="m-t-10"
                  >任何人可参与
                </el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="可补打卡总次数">
              <el-input-number
                v-model="form.allowMakeupTimes"
                ...
                step-strictly
              />
              <span class="tips1 tips">0表示不允许补打卡</span>
            </el-form-item>
            <el-form-item label="打卡可见范围" prop="visibility">
              <el-radio-group v-model="form.visibility">
                <el-radio :label="0" class="m-t-10">学员自主选择 </el-radio>
                <el-radio :label="1" class="m-t-10">所有人可见 </el-radio>
                <el-radio :label="2" class="m-t-10">仅管理员可见 </el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="其他设置">
              <div>
                <el-checkbox
                  v-model="form.antiCheat"
                  :true-label="1"
                  :false-label="0"
                  >防作弊 </el-checkbox
                ><span class="tips1 tips"
                  >勾选后，学员完成打卡才能查看其他学员内容</span
                >
              </div>
              <el-checkbox 
                ...
                >重新编辑 </el-checkbox
              ><span class="tips1 tips">
                勾选后，允许学员对打卡内容重新编辑</span
              >
            </el-form-item>
            <el-form-item label="打卡封面图">
              <uploadView
                key="MUploadImg"
                ref="MUploadImg"
                upload-text="点击上传封面"
              ...
              <div class="m-t-20 text-sm" style="color: #777777">
                <div class="tips1">
                  750*420px或16:9，小于5M，支持jpg、png、jpeg格式，单张
                </div>
              </div>
            </el-form-item>
            <el-form-item label="打卡详情" prop="content">
              <AppEditor
                ref="AppUEditor"
    ...
    <div class="demo-drawer__footer">
      <el-button size="small" @click="save(0)">暂不发布</el-button>
      <el-button type="primary" size="small" @click="save(1)"
        >保存并发布</el-button
      >
    </div>
    <CopyDialog :config="config" @success="success" />
  </div>
</template>
...
import { punch, getPunchDetail, taskCopy } from '@/api/deliver/punch.js'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        visibility: [
          { required: true, message: '请选择打卡可见范围', trigger: 'change' }
        ]
      },
      isCopy: false,
      config: {
  ...
  methods: {
    ...
    async getDetail() {
      const { data } = await getPunchDetail(this.data.id)
    ...
    save(isPublish) {
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          if (this.type === 'copy') {
            if (this.form.taskCount > 0) {
              this.config = {
                visible: true,
                data: {
                  taskCount: this.form.taskCount,
                  copyPunchId: this.data.id,
                  punchId: res.data
                }
              }
            } else {
    ...
    fileWarmChange(res) {
      this.form.coverImage = res[0].url
    },
