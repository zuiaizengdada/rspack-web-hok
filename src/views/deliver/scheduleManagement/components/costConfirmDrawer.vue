  ...
  >
    <div v-loading="loading" class="drawerBody">
      ...
      <div class="ss-material-box-footer">
        <el-button :loading="sureLoading" size="small" type="primary" @click="sure">确定成本</el-button>
        <el-button :loading="sureLoading" size="small" @click="close()">取消</el-button>
      </div>
    </div>
  </el-drawer>
</template>
...
import { confirmCost, getDeliveryScheduleCost } from '@/api/deliver/scheduleManagement'
export default {
  components: {
    courseDeliveryCost,
    studentTuition
    ...
    sure() {
      this.$delModal({
        ...
        success: () => {
          ...
          confirmCost(this.scheduleId).then(res => {
            this.$emit('sure', this.scheduleId)
