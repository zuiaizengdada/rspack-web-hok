  ...
  >
    <div v-if="visibleDialog" class="drawerBody">
      <div class="p-20 border-b border-#EFF2F6" style="height: 60px">
        <p class="font-bold text-16">详情</p>
      </div>
        ...
        if (i.children && i.children.length) {
          i.children.forEach(j => {
            if (j.path === 'scheduleManagement') {
              this.permsList = []
              const data = j.children.find(x => x.name === 'scheduleLive')
              const childrenData = data?.children.find(item => item.name === '/scheduleOfflineMy')
              if (childrenData) {
                this.permsList = childrenData.meta.permsList
              }
