  ...
  <div class="page_wrap">
    ...
    >
      ...
      <el-table-column label="交付状态">
        ...
        <el-table-column prop="unDeliveredNum" label="未交付学员数">
          <template slot-scope="{ row }">
            <el-button
              type="text"
              size="small"
              :disabled="!row.unDeliveredNum"
              @click.native.prevent="handleRow(row, '未交付')"
            >
              {{ row.unDeliveredNum }}
            </el-button>
          </template>
        ...
        <el-table-column prop="enrollmentRatio" label="入班率" sortable>
          <template slot-scope="{ row }"> {{ row.enrollmentRatio }}% </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleRow(row, name, notInScheduleStatus) {
      ...
      // 获取search缓存 数据
      const initSearchData = this.search
      this.param = {
        deliveryStatus: name !== '未交付' ? deliveryStatus : undefined,
        deliveryStatusList: name === '未交付' ? [1, 3] : undefined,
