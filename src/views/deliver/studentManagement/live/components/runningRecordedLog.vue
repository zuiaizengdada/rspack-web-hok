<template>
  <div v-loading="loading">
    ...
    <el-table :data="data" :header-cell-style="{ background: '#f5f5f5' }">
      ...
      <el-table-column prop="operateTypeName" label="动作">
        <template slot-scope="{ row }">
          {{ row.operateTypeName }}
        </template>
      </el-table-column>
...
<script>
import { getOperationRecordPage } from '@/api/deliver/studentManagement'
export default {
  components: {},
  props: {
    deliveryInfoId: {
  ...
  methods: {
    ...
    getList() {
      ...
      const params = {
        ...
        size: this.pageSize,
        deliveryInfoId: this.deliveryNo
      }
