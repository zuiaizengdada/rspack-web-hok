            ...
            <span v-if="item.type === 4">
              <div class="flex flex-middle">
                ...
                <span
                  class="name-staff"
                >
                  呼叫员工：{{ item.clueCallRecordVo.callerName }}
                </span>
              </div>
            </span>
          </div>
...
<script>
import { userDynamics } from '@/api/deliver/studentManagement'
...
export default {
  ...
  methods: {
    getDetail() {
      if (!this.studentDynamicId) return
      this.loading = true
      const data = {
        deliveryInfoId: this.studentDynamicId,
