<template>
  <div>
    ...
    <el-table
      ...
      :row-key="record => record.goodsId + record.smallMaterialId"
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          {{ row.contentName || row.materialName || '' }}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="accumulatedDuration" label="学习时长">
        <template slot-scope="{ row }">
...
<script>
import {
  getUserProgressDeliveryPage,
  getUserProgressDeliveryTotal,
  progressDeliveryList,
  progressDeliveryPageTotal,
  progressAllTotal
...
export default {
  ...
  methods: {
    ...
    async getUserAllTotal() {
      const { data } = await progressAllTotal({
        subOrderId: this.subOrderId,
        goodsId: this.goodsId
      })
      this.allTotal = data
    ...
    getList() {
      this.loading = true
      const api =
        this.active === 1 ? getUserProgressDeliveryPage : progressDeliveryList
      const param = {
