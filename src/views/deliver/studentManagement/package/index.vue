...
export default {
  ...
  methods: {
    onClickPath(item) {
      ...
      this.$store.dispatch('table/setInitPage', { '/deliver/studentManagement/package': item.path })
    ...
    pageInit() {
      ...
      if (currentRouter.length && currentRouter[0].children) {
        ...
        if (liveListArr.length) {
          this.pathList = liveListArr[0].children.filter(i => i.path === '/deliver/studentManagement/package')
          ...
          if (this.$route.path === '/deliver/studentManagement/package') {
            if (this.tabs.length > 0) {
              this.$uweb?.setAction('Event_Play_page')
              const isPath = this.tabs?.find(item => item.path === this.initPage['/deliver/studentManagement/package']) || this.tabs[0]
