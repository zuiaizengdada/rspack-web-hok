<template>
  ...
  <div class="page_wrap">
    ...
    >
      ...
      <template slot="action" slot-scope="{ row }">
        ...
        <el-button
          ...
          :disabled="
            row.isGive || ['已关闭', '锁定交付'].includes(row.deliveryStatusName)
          "
          type="text"
        ...
        >解除锁单</el-button>
        <el-button
          v-if="row.shiftFlag && !row.isInclude"
        ...
        <el-button v-permission="['web:allStudent:call', permsList]" type="text" @click="call(row)">呼叫</el-button>
      </template>
    ...
    <!-- 换人就读弹窗 -->
    <changeReadDialog ref="changeReadDialog" :is-package="true" />

    <!-- 延期弹窗 -->
    <delayPeriodsDialog ref="delayPeriodsDialog" @success="getList" />

    <!-- 关闭交付弹窗 -->
    <closeDeliverDialog ref="closeDeliverDialog" :is-package="true" @success="getList" />
...
import { deliveryPackagePage, packageUserSalesChange } from '@/api/deliver/studentManagement'
...
export default {
  ...
  mounted() {
    this.$store.dispatch('user/viewingFlag', 'studentManagement')
    this.$uweb && this.$uweb.setAction('Event_AllPlay_page')
    this.getLastMonthDateRange()
    this.getList()
  },
  methods: {
    ...
    getList() {
      ...
      const params = {
        ...
        deliveryStatusList: this.search.deliveryStatusList || [],
        orderStartDate:
