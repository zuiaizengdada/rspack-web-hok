...
export default {
  ...
  methods: {
    onClickPath(item) {
      ...
      this.$store.dispatch('table/setInitPage', { '/deliver/studentManagement/offline': item.path })
    ...
    pageInit() {
      ...
      if (currentRouter.length && currentRouter[0].children) {
        ...
        if (liveListArr.length) {
          this.pathList = liveListArr[0].children.filter(i => i.path === '/deliver/studentManagement/offline')
          ...
          if (this.$route.path === '/deliver/studentManagement/offline') {
            if (this.tabs.length > 0) {
              const isPath = this.tabs?.find(item => item.path === this.initPage['/deliver/studentManagement/offline']) || this.tabs[0]
