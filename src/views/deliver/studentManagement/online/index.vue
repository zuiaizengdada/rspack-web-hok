...
export default {
  ...
  methods: {
    onClickPath(item) {
      ...
      this.$store.dispatch('table/setInitPage', { '/deliver/studentManagement/online': item.path })
    ...
    pageInit() {
      ...
      if (currentRouter.length && currentRouter[0].children) {
        ...
        if (liveListArr.length) {
          this.pathList = liveListArr[0].children.filter(i => i.path === '/deliver/studentManagement/online')
          ...
          if (this.$route.path === '/deliver/studentManagement/online') {
            if (this.tabs.length > 0) {
              const isPath = this.tabs?.find(item => item.path === this.initPage['/deliver/studentManagement/online']) || this.tabs[0]
