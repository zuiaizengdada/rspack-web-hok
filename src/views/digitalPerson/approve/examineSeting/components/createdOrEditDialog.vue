<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="部门范围" prop="approveDeptIds">
      ...
      <el-form-item label="IP范围" prop="IPTags">
        <el-tag
          v-for="tag in IPTags"
          :key="tag.id"
          closable
        ...
        <el-select
          ...
          placeholder=""
          :popper-append-to-body="false"
          :remote-method="queryIPSearch"
          :loading="loading"
          class="input-new-tag"
          size="mini"
          @blur="blurInputIPValue"
          @change="handleIPInputConfirm"
        >
          <el-option
...
export default {
  ...
  methods: {
    ...
    queryIPSearch(query) {
      if (query === '') return
      const data = this.allTeacherList.filter(item =>
        item.teacherName.includes(query)
      )

      this.IPOptions = data.map(item => ({
        id: item.teacherId,
        name: item.teacherName
      }))
    },
    blurInputIPValue() {},
    handleIPClose(record) {
      this.IPTags = this.IPTags.filter(item => item.id !== record.id)
    },
    handleIPInputConfirm() {
      const inputValue = this.inputIPValue
      if (inputValue) {
        const info = this.IPOptions.find(item => item.id === inputValue)
        // 判断是否已经存在
        const isExist = this.IPTags.find(item => item.id === info.id)
        if (!isExist) {
          this.IPTags.push(info)
        }
      }
    ...
    async querySearch(query) {
      ...
      console.log('this.loginType3---------', this.loginType)
    ...
    open(type, row) {
      ...
      if (type === 'edit') {
        ...
        this.IPTags = row.approveTeachers.map(m => ({
          id: m.teacherId,
          name: m.teacherName
        }))
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          const params = {
            ...
            teacherIds: this.IPTags.map(item => item.id)
          }
          editApprover(params)
            .then(res => {
