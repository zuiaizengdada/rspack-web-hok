...
import { getSensitiveList, deleteSensitiveList, batchDeleteSensitiveList, updateSensitiveList, addSensitiveList, importSensitiveList } from '@/api/liveRoom/setting'
export default {
  components: {
  ...
  methods: {
    // 导入
    getUploadApi(data) {
      return importSensitiveList(data)
    },
    jtkey(e) {
      e.preventDefault()
      if (e.keyCode === 13) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.insertOrEdit === '新增敏感词') {
            ...
            addSensitiveList(param).then(res => {
              if (res.success) {
            ...
            updateSensitiveList(param).then(res => {
              if (res.success) {
    ...
    toDelete(row) {
      ...
      this.$confirm(string, {
        ...
        let res = null
        if (this.isObject(row)) {
          res = await deleteSensitiveList(row.id)
        } else {
          const ids = []
          row.forEach(item => {
            ids.push(item.id)
          })
          res = await batchDeleteSensitiveList(ids)
        }
        if (res.success) {
          this.$message({
            type: 'success',
