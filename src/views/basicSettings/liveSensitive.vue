<template>
  <div class="sensitive-wrapper">
    ...
    <el-dialog class="insert-sen-word" :title="insertOrEdit" :visible.sync="dialogFormVisible" :close-on-click-modal="false" append-to-body>
      <el-form ref="caseWordForm" :rules="rules" label-width="95px" :model="caseWordForm">
    ...
    <uploadTongyongCaseWords ref="update" down-template="https://oss.hokkj.cn/hok_product/liveSensitiveWordTemplate.xls" :get-api="getUploadApi" @loadDataUploadSuccess="searchList" />
  </div>
</template>

<script>
import uploadTongyongCaseWords from '@/views/basicSettings/components/uploadTongyongCaseWords'
import { getLiveSensitivePageAjax, deleteLiveSensitiveAjax, batchDeleteLiveSensitiveAjax, upDateLiveSensitiveAjax, addLiveSensitiveAjax, importLiveSensitiveAjax } from '@/api/liveRoom/setting'
export default {
  components: {
  ...
  data() {
    return {
      typeArray: [
        { typeId: 1, typeName: '涉黄' }, { typeId: 2, typeName: '违禁' }, { typeId: 3, typeName: '谩骂' }, { typeId: 4, typeName: '广告' }, { typeId: 5, typeName: '诈骗' }, { typeId: 6, typeName: '涉政' }, { typeId: 7, typeName: '联系方式' }, { typeId: 8, typeName: '其他' }
      ],
      typeArr: ['', '涉黄', '违禁', '谩骂', '广告', '诈骗', '涉政', '联系方式', '其他'],
      teacherArray: [],
      caseWordForm: {
        type: '',
        content: [],
      ...
      config: {
        ...
        tableColumns: [
          { prop: 'type', label: '分类', type: 'slot', visible: true, disable: false },
          { prop: 'content', label: '敏感词', type: 'text', visible: true, disable: false },
          { prop: 'createBy', label: '上传者', type: 'text', visible: true, disable: false },
          { prop: 'createTime', label: '上传时间', width: 180, type: 'text', visible: true, disable: false },
          { prop: 'action', label: '操作', width: 130, type: 'slot', visible: true, disable: true, fixed: 'right' }
        ],
  ...
  methods: {
    // 导入
    getUploadApi(data) {
      return importLiveSensitiveAjax(data)
    },
    jtkey(e) {
      e.preventDefault()
      if (e.keyCode === 13) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.insertOrEdit === '新增敏感词') {
            ...
            addLiveSensitiveAjax(param).then(res => {
              if (res.success) {
            ...
            upDateLiveSensitiveAjax(param).then(res => {
              if (res.success) {
    ...
    toDelete(row) {
      ...
      this.$confirm(string, {
        ...
        let res = null
        if (this.isObject(row)) {
          res = await deleteLiveSensitiveAjax(row.id)
        } else {
          const ids = []
          row.forEach(item => {
            ids.push(item.id)
          })
          res = await batchDeleteLiveSensitiveAjax(ids)
        }
        if (res.success) {
          this.$message({
            type: 'success',
    ...
    getList() {
      ...
      getLiveSensitivePageAjax(params).then(res => {
        if (res.success) {
          this.loading = false
