...
<script>
import { caseWordsImport } from '@/api/product/videoManager'
...
export default {
  ...
  methods: {
    ...
    downloadFile() {
      window.open('https://oss.hokkj.cn/hok_product/Sensitivewordimporttemplate.xls')
    },
    onSubmit() {
    ...
    async fileRequest(e) {
      ...
      const { data } = await caseWordsImport({ data: formData })
