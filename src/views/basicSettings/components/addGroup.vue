  ...
  methods: {
    ...
    delAssessUsers(tag, index) {
      this.$delModal({
        ...
        success: () => {
          const paramsArr = []
          this.shopFeishuGroupList.some((item, i) => {
            if (i === index) {
          ...
          if (this.setType === 5) {
            const params = {
              status: this.shopNoticeFlag ? 1 : 0,
              bizId: this.bizId,
            ...
            setMediaRemind(params).then(res => {
              ...
              if (res.code === 1) {
                ...
                this.keywordsObject = {}
                this.$emit('getComplement', this.setType)
              }
            })
          } else if (this.setType === 1) {
            const params = {
              onOff: this.shopNoticeFlag ? 1 : 0,
              remindCrowdVos: paramsArr.map(v => ({
                crowdId: v.chatId,
                crowdName: v.name
              })),
              sysUserIds,
            ...
            setFeishuremind(params).then(res => {
              if (res.code === 1) {
                this.$notify.success({ title: '提示', message: '操作成功' })
                this.keywordsObject = {}
                this.$emit('getComplement', this.setType)
            ...
            setDeliveryInfoList(params).then(res => {
              if (res.code === 1) {
                this.$notify.success({ title: '提示', message: '操作成功' })
                this.keywordsObject = {}
                this.$emit('getComplement', this.setType)
          ...
          } else if (this.setType === 10) {
            console.log('this.setType10:', this.setType)
            const params = {
              status: this.shopNoticeFlag ? 1 : 0,
              bizId: this.bizId,
              noticeCrowd: paramsArr,
          ...
          } else if (this.setType === 19) {
            this.$emit('setRemindInfo', paramsArr)
          } else if (this.setType === 16) {
            this.$emit('del', paramsArr)
            this.addFlag = !this.addFlag
          } else {
            ...
            setComplementRemindParam(params).then(res => {
              if (res.code === 1) {
                this.$notify.success({ title: '提示', message: '操作成功' })
    ...
    sureToAdd() {
      ...
      const objForKey = {
        chatId: this.keywordsObject.chat_id,
        name: this.keywordsObject.name
      }
      paramsArr.push(objForKey)
      const sysUserIds = this.dianpuDelivered.reduce((pre, next) => {
        pre.push(next.userId)
        return pre
      }, [])
