    ...
    >
      ...
      <div class="m-t-20">
        ...
        <el-form-item label="封面图" prop="coverUrlList">
          ...
          />
          <div class="text-sm m-t-20" style="color: #777777">
            <div>
              尺寸750*750或比例1:1,小于5M（图片会自动进行压缩）的jpg,png,jpeg格式的图片，最多上传7张
            </div>
          </div>
        </el-form-item>
        <!--        <el-form-item label="封面图" prop="patchUrl">-->
        <!--          <div class="flex">-->
        <!--            <AppMyImage-->
        <!--              v-for="(item, index) in form.patchUrl"-->
        <!--              :key="index"-->
        <!--              class="mr-5"-->
        ...
        <!--                }-->
        <!--              }"-->
        <!--              @close="delImage(form.patchUrl, index)"-->
        <!--              @success="res => changeImage(form.patchUrl, index, res)"-->
        <!--            />-->
        <!--            <div-->
        <!--              v-if="form.patchUrl.length < 7"-->
        <!--              class="image_poster"-->
        <!--              style="width: 112px; height: 112px"-->
        <!--              @click="getImgUrl(form.patchUrl)"-->
        <!--            >-->
        ...
        <!--        </el-form-item>-->
        <!-- <div class="m-t-20">
          <el-form-item label="是否包含实物" prop="goodsForm">
            <el-radio-group
          ...
          <el-form-item>
            <template v-if="form.goodsForm === 2">
              ...
              <el-form-item label="图片" prop="giftsImgList" class="mt-6">
                <ImageUpload
        ...
        <el-form-item label="相关资料">
          <div class="btnfl">
            ...
            <div class="tips">
              最多添加100个文件(后缀为pdf),单个文件不能大于100M。
            </div>
          </div>
      ...
      <div class="m-t-20">
        <el-form-item label="商品价格" prop="salePrice" class="mt-6">
          <el-input
        ...
        <el-form-item label="划线价格" prop="marketPrice" class="mt-6">
          <el-input
        ...
        <el-form-item
          label="有效日期"
          prop="validityDays"
          class="mt-6 validity-days is-required"
        >
          <div>
            <el-radio
              ...
              @change="
                () => {
                  form.materialSalePicReq.fixedTime = ''
                  form.validityDays = ''
                  $refs.form.clearValidate('validityDays')
                }
              "
            >
              购买后
          ...
          <div style="margin-top: 20px">
            ...
            <el-date-picker
              ...
              :picker-options="fixedTimePickerOptions"
            />
          </div>
        </el-form-item>
      </div>
      <div class="header_tips">服务与履约</div>
      <div class="m-t-20">
      ...
      <div class="header_tips">上架设置</div>
...
import ImageUpload from '@/views/course/components/ImageUpload.vue'
...
export default {
  ...
  data() {
    ...
    var checkSalePrice = (rule, value, callback) => {
      ...
      const salePriceNumber = parseFloat(this.form.salePrice)
      const marketPriceNumber = parseFloat(this.form.marketPrice)
    ...
    const validateValidity = (rule, value, callback) => {
      if (this.form.materialSalePicReq.saleValidity === 1) {
        // 购买后有效期
        if (!this.form.validityDays) {
          callback(new Error('请填写有效日期'))
        } else {
          callback()
        }
      } else if (this.form.materialSalePicReq.saleValidity === 2) {
        // 固定截止日期
        if (!this.form.materialSalePicReq.fixedTime) {
    ...
    return {
      ...
      form: {
        ...
        goodsId: '', // 图文音频图文id
        goodsType: 8, // 0:图片,1:音频，2视频,3专栏，4大专栏,5线下课程,6,直播专栏 8 直播课
      ...
      rules: {
        ...
        giftsName: [{ required: true, message: '请输入名称', trigger: 'blur' }],
        giftsNum: [{ required: true, message: '请输入数量', trigger: 'blur' }],
        policyType: [
        ...
        lecturerIdList: [
          { required: true, message: '请选择IP老师', trigger: 'blur' }
        ],
        catalogueReqList: [
          {
            required: true,
        ...
        validityDays: [{ validator: validateValidity, trigger: 'change' }],
        goodsType: [
  ...
  mounted() {
    this.hasPrudectFlag = this.permsList.includes(
      'messageManager:packageColumn:addGoods'
    )
    this.hasGoodsFlag = this.permsList.includes(
      'messageManager:packageColumn:addGoodsFlag'
    )
  },
  methods: {
    getMaterialTagList() {
    ...
    handleSelectGiftsCode(item) {
      ...
      this.form.giftsType = item.giftsType || 10086
    ...
    // 裁剪图片
    changeImage(obj, index, res) {
      obj.splice(index, 1, res)
    },
    // 选择IP
    teacherChange(e) {
      console.log(e, 'IP')
    ...
    getGiftsListChange(giftsImgList) {
      this.form.materialGiftsReq.giftsImgList = giftsImgList
      this.$refs.form.clearValidate('materialGiftsReq.giftsImgList')
    },
    // 选择相关资料
    getInforMationPic() {
      this.$SelectMaterialMultiple({
    ...
    _addGoods() {
      ...
      const materialSalePicReq = {
        ...
        salePrice: this.form.salePrice || 0, // 价格
        marketPrice: this.form.marketPrice || 0, // 划线价格
        saleType: 1, // 0 免费 1付费 2 加密 3 指定学员
        fixedTime: this.form.materialSalePicReq.fixedTime, // 固定截止日期
        saleValidity: this.form.materialSalePicReq.saleValidity // 0 长期有效 1 购买后 2 固定时间
      }
      if (this.form.validityType === 2) {
        // 年
        materialSalePicReq.validityYears = this.form.validityDays
      } else {
        // 天
        materialSalePicReq.validityDays = this.form.validityDays
      }
      ...
      const params = {
        ...
        coverUrl: this.form.coverUrlList[0].coverUrl,
      ...
      // 格式化价格
      params.salePrice = Math.round((params.salePrice || 0) * 100)
      params.materialSalePicReq.salePrice = Math.round(
        (params.materialSalePicReq.salePrice || 0) * 100
      )
      params.marketPrice = Math.round((params.marketPrice || 0) * 100)
      params.materialSalePicReq.marketPrice = Math.round(
        (params.materialSalePicReq.marketPrice || 0) * 100
      )
