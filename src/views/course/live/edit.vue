          ...
          <el-form-item label="相关资料">
            ...
            <div class="ZiliaoTable">
              ...
              >
                ...
                <el-table-column prop="materialName" label="文件" width="150" />
            ...
            >
              <div>
                <el-radio
                  ...
                  :label="1"
                  style="width: 100px"
                  @change="handleSaleValidityChange"
                >
                  购买后
                </el-radio>
                <el-input
              ...
              <div style="margin-top: 20px">
                <el-radio
                  ...
                  :label="2"
                  style="width: 100px"
                  @change="handleFixedTimeValidityChange"
                >
                  固定截止日期
                </el-radio>
                <el-date-picker
                  ...
                  @change="
                    val => (form.materialSalePicReq.fixedTime = val || null)
                  "
                />
              </div>
            </el-form-item>
...
export default {
  ...
  data() {
    ...
    var checkSalePrice = (rule, value, callback) => {
      ...
      const marketPriceNumber = parseFloat(
    ...
    const validateValidity = (rule, value, callback) => {
      // 如果是购买后模式，检查 validityDays
      if (
        this.form.materialSalePicReq.saleValidity === 1 &&
        !this.form.materialSalePicReq.validityDays
      ) {
        callback(new Error('请填写有效日期'))
        return
      }

      // 如果是固定截止日期模式，检查 fixedTime
      if (
        this.form.materialSalePicReq.saleValidity === 2 &&
        !this.form.materialSalePicReq.fixedTime
      ) {
        callback(new Error('请选择截止日期'))
        return
      }

      // 只要不是上面两种情况（即:要么没选择模式，要么选择了模式并且填了对应的值），就通过验证
      callback()
    }
    // 添加课时校验规则
    const validateClassHour = (rule, value, callback) => {
      if (this.type !== 'copy' && value < this.originalForm.classHour) {
        callback(new Error(`上架保存之后只能增加不能减少，请按实际情况填写`))
      } else {
        callback()
      }
    }
    return {
      ...
      form: {
        ...
        materialSalePicReq: {
          // 售卖方式关联商品集合
          fixedTime: null, // 课程有效期-固定时间 【sale_validity =2时生效】
          goodsId: '', // 商品信息关联id
        ...
        shelveTime: null, // 定时上架时间shelveFlag==2是生效
  ...
  mounted() {
    this.hasPrudectFlag = this.permsList.includes(
      'messageManager:packageColumn:editGoodsFlag'
    )
    this.hasGoodsFlag = this.permsList.includes(
      'messageManager:packageColumn:editGoods'
    )
    this.init()
  ...
  methods: {
      ...
      axios
        ...
        .then(res => {
          ...
          if (res[2].data.materialSaleVo) {
            const materialSaleVo = res[2].data.materialSaleVo

            // 处理价格
            materialSaleVo.salePrice = (materialSaleVo.salePrice || 0) / 100
            materialSaleVo.marketPrice =
              Number(materialSaleVo.marketPrice) === 0
                ? ''
                : (materialSaleVo.marketPrice || 0) / 100

            // 处理固定截止日期 - 始终确保是字符串格式
            materialSaleVo.fixedTime = materialSaleVo.fixedTime
              ? typeof materialSaleVo.fixedTime === 'string'
                ? materialSaleVo.fixedTime
                : new Date(materialSaleVo.fixedTime)
                  .toISOString()
                  .slice(0, 16)
                  .replace('T', ' ')
              : null
          ...
          const giftsMediumImg =
            materialGiftsVo.giftsMediumImg?.split(',') ?? []
          ...
          this.form = {
            ...
            shelveTime: res[2].data.shelveTimingTime
              ? new Date(res[2].data.shelveTimingTime)
              : null,
            countdown: this.countdownType === 'h' ? countdown / 60 : countdown,
            materialSalePicReq: res[2].data.materialSaleVo
              ? {
                ...res[2].data.materialSaleVo,
                fixedTime: res[2].data.materialSaleVo.fixedTime || null
              }
            ...
            coverUrl: res[2].data.coverUrlList[0].coverUrl,
            coverVerticalUrl: res[2].data.coverUrlList[0].coverUrl
          }
          ...
          if (res[2].data.materialSaleVo.validityYears > 0) {
            ...
            this.form.materialSalePicReq.validityDays =
              this.form.materialSalePicReq.validityYears
          } else {
    ...
    _addGoods() {
      ...
      if (
        ...
        marketPrice = Math.round(
          (this.form.materialSalePicReq.marketPrice || 0) * 100
        )
      } else {
        marketPrice = ''
      }
      ...
      console.log(
        this.form.labelCode,
        'this.form.labelCodethis.form.labelCode++'
      )
      ...
      if (this.form.validityType === 2) {
        // 年
        materialSalePicReq.validityYears =
          this.form.materialSalePicReq.validityDays
        materialSalePicReq.validityDays = 0
      } else {
        // 天
        materialSalePicReq.validityDays =
          this.form.materialSalePicReq.validityDays
    ...
    // 处理购买后有效期切换
    handleSaleValidityChange() {
      // 清空固定截止日期，设置为 null 而不是空字符串
      this.form.materialSalePicReq.fixedTime = null

      // 如果有原始值，则恢复购买后天数的值
      if (this.originalForm.materialSalePicReq) {
        this.form.materialSalePicReq.validityDays =
          this.form.materialSalePicReq.validityDays ||
          this.originalForm.materialSalePicReq.validityDays ||
          ''
      }

      // 清除验证信息
      this.$refs.form.clearValidate('materialSalePicReq.validityDays')
    },
    // 处理固定截止日期切换
    handleFixedTimeValidityChange() {
      // 清空购买后天数，设置为空字符串
      this.form.materialSalePicReq.validityDays = ''

      // 始终设置为 null，让日期选择器重新初始化
      this.form.materialSalePicReq.fixedTime = null
