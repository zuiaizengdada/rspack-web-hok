  ...
  <div>
    <div class="page_header">
      <div class="page_header_title">
        <span
          class="pointer"
          @click="$router.push({ path: '/shop/livecourse/list' })"
        >直播课</span>
        /
        <span>直播课详情</span>
      </div>
      ...
      <div v-loading="loading" class="page_header_detail">
        ...
        <div class="page_header_detail_content">
          ...
          <div class="price-wrapper">
            <span
              v-if="detail.materialSaleVo.saleType === 0"
              class="c_777"
            >免费</span>
            <span
              v-if="detail.materialSaleVo.saleType === 1"
              class="c_777"
            >￥{{ detail.materialSaleVo.salePrice | filtersMoney }}</span>
            <span
              v-if="
                [0, 1].includes(detail.materialSaleVo.saleType) &&
                  [0, 1].includes(detail.materialSaleVo.saleValidity)
              "
              class="c_777"
            >
              /
            ...
            <span
              v-if="detail.materialSaleVo.saleValidity === 0"
              class="c_777"
            >长期有效</span>
            <span
              v-if="
                detail.materialSaleVo.saleValidity === 1 &&
                  detail.materialSaleVo.validityYears === 0
              "
              class="c_777"
            >有效期{{ detail.materialSaleVo.validityDays }}天</span>
            <span
              v-if="
                detail.materialSaleVo.saleValidity === 1 &&
                  detail.materialSaleVo.validityYears !== 0
              "
              class="c_777"
            >有效期{{ detail.materialSaleVo.validityYears }}年</span>
            <span
              v-if="detail.materialSaleVo.saleValidity === 2"
              class="c_777"
            >/有效期{{ detail.materialSaleVo.fixedTime }}</span>
          </div>
          <div class="operate">
            <el-button @click="gotoEdit(detail)">编辑</el-button>
            <el-button
              v-share="
                `pages/subpage/share/index?id=${$route.params.id}&type=1`
              "
            >分享</el-button>
          </div>
        </div>
    ...
    <div class="page_content">
      <el-tabs v-model="activeName" @tab-click="tabsChange">
        <!-- <el-tab-pane label="直播课程" name="contentManage" />
...
export default {
  ...
  created() {
    this.activeName = this.$route.query.type || 'tiktok'
