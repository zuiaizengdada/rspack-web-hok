  ...
  <div class="content">
    ...
    <div class="content_table">
      <el-table
        ...
        @row-dblclick="handleCellMouseEneter"
          ...
          <template slot-scope="{ row }">
            <div class="fl goods-info-wrapper">
              ...
              <div class="goods-info-detail">
                <div
                  v-show="row.goodsId !== isCell.goodsId"
                  class="titleOptions"
                >
                  {{ row.contentName
                  }}<i
                ...
                <div v-show="row.goodsId === isCell.goodsId" class="InputBox">
                  <el-input
                    v-show="row.goodsId === isCell.goodsId"
                    v-model="row.contentName"
                  />
            ...
            <template v-if="permsList.includes('web:content:watch')">
              ...
              >
                ...
                <div style="text-align: right; margin: 0">
                  <el-button
                    type="text"
                    @click="showtrialMinute(row)"
                  >取消</el-button>
                  <el-button
                    type="primary"
                    @click="sortshowtrialMinute(row)"
                  >确定</el-button>
                </div>
                <el-button
...
export default {
  ...
  methods: {
    ...
    rowDrop() {
      ...
      const tableIds = this.tableData.map(i => i.columnId)
      const _this = this
        ...
        this.sortable = Sortable.create(tbody, {
          ...
          async onEnd({ newIndex, oldIndex }) {
            ...
            const dropList = tableIds.map((i, index) => {
              const dIndex = (_this.current - 1) * _this.size
              return {
                id: i,
                sort: index + dIndex
              }
    ...
    async handleClickStart(val) {
      ...
      if (!val.contentName) {
        val.contentName = this.isCell.contentName
        this.$message.warning('请填写商品名称')
        this.isCell = {}
        return
      }
      const res = await updateContentName({
