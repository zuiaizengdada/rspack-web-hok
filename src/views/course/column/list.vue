    ...
    >
      ...
      <div slot="action" slot-scope="{ row }">
        ...
        <el-button
          ...
          @click="gotoEdit(row)"
        >编辑</el-button>
        <el-button type="text" @click="handleCopyGoods(row)">复制</el-button>
        <el-button
        ...
        <el-button
          ...
          v-share="`pages/subpage/share/index?id=${row.goodsId}&type=1`"
          type="text"
        >分享</el-button>
        <el-button
...
export default {
  ...
  methods: {
    ...
    // 跳转到新建图文页面
    gotoAddVideo() {
      this.$router.push({ path: 'columnAdd' })
    },
    ...
    gotoDetail(row) {
      if (row.goodsType === 6) {
        this.$router.push({
          path: `/shop/packageColumn/liveColumnDetail/${row.goodsId}`
        })
      } else {
        this.$router.push({
