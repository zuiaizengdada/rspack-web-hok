    ...
    >
      ...
      <div class="m-t-20">
        ...
        >
          ...
          <el-button
            v-if="form.overAllocationNotice.length < 3"
            size="mini"
            @click="selectOverallocation"
          >新增人员</el-button>
        </el-form-item>
        ...
        <el-form-item v-if="hasPrudectFlag" label="商品分类标签" prop="productCategory">
          <el-select v-model="form.productCategory" placeholder="请选择分类标签">
  ...
  created() {
    if (this.$route.query.productId) {
      productDetailByIdAjax({
        ...
        if (res.code === 1) {
          this.form.contentName = JSON.parse(JSON.stringify(res.data.productName))
          this.$set(this.form, 'columnContent', JSON.parse(JSON.stringify(res.data.courseIntroduction)))
