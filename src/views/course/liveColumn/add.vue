<template>
  <div v-loading="loading" class="page_wrap">
    <!-- <div class="header">
      <span class="pointer" @click="$router.push({ path: '/course/packageColumn/list' })">专栏</span>
      > 新建专栏
    </div> -->
    <el-form ref="form" :model="form" label-width="120px" label-position="left" :rules="rules">
      ...
      <div class="m-t-20">
        ...
        <el-form-item label="专栏类型" prop="goodsType">
          <el-select v-model="form.goodsType" placeholder="请选择专栏类型">
            <el-option label="普通专栏" :value="3" />
            <el-option label="直播专栏" :value="6" />
          </el-select>
        </el-form-item>

        <!-- <el-form-item v-if="form.goodsType === 6" label="预警通知" prop="warningNotice">
          <el-tag
            v-for="(tag) in form.warningNotice"
            :key="tag.userId"
            class="m-r-10"
            closable
            size="small"
            @close="handleNoticeClose('warningNotice', tag)"
          >
            {{ tag.nickName || tag.userName }}
          </el-tag>
          <el-button size="mini" @click="selectEarlyWarning">新增人员</el-button>
        </el-form-item> -->

        <el-form-item v-if="form.goodsType === 6" label="报满通知" prop="fullNotice">
          <el-tag
            v-for="tag in form.fullNotice"
            :key="tag.userId"
            class="m-r-10"
            closable
            size="small"
            @close="handleNoticeClose('fullNotice', tag)"
          >
            {{ tag.nickName || tag.userName }}
          </el-tag>
          <el-button size="mini" @click="selectFull">新增人员</el-button>
        </el-form-item>

        <el-form-item v-if="form.goodsType === 6" label="超额分配通知" prop="overAllocationNotice">
          <el-tag
            v-for="tag in form.overAllocationNotice"
            :key="tag.userId"
            class="m-r-10"
            closable
            size="small"
            @close="handleNoticeClose('overAllocationNotice', tag)"
          >
            {{ tag.nickName || tag.userName }}
          </el-tag>
          <el-button size="mini" @click="selectOverallocation">新增人员</el-button>
        </el-form-item>

        <el-form-item label="专栏简介" prop="columnContent">
          <el-input v-model.trim="form.columnContent" maxlength="200" type="textarea" :autosize="{ minRows: 4}" show-word-limit />
        </el-form-item>

        <el-form-item label="选择IP" prop="lecturerIdList">
          <el-select v-model="form.lecturerIdList" :multiple-limit="20" placeholder="请选择IP" multiple>
            <el-option
        ...
        <el-form-item label="人气值" prop="goodsPopularity">
          <el-input-number v-model.number="form.goodsPopularity" :min="0" controls-position="right" style="width: 200px;" />
        </el-form-item>

        <el-form-item label="专栏封面" prop="patchUrl">
          <div>
            <div v-if="!form.patchUrl" class="image_poster" style="width: 200px;height: 112px" @click="getImgUrl('patchUrl')">
            ...
            <div class="tips m-t-20">
              <div>专栏封面是指专栏展示的图片，建议尺寸750*422或比例16：9，</div>
        ...
        <el-form-item label="方形缩略图" prop="coverVerticalUrl">
          <div>
            <div v-if="!form.coverVerticalUrl" class="image_poster" style="width: 138px;height: 138px" @click="getImgUrl('coverVerticalUrl')">
              <i class="el-icon-plus" />
              <span>点击上传图片</span>
            </div>
            <div v-else>
        ...
        <el-form-item label="横版缩略图" prop="coverUrl">
          <div>
            <div v-if="!form.coverUrl" class="image_poster" style="width: 200px;height: 150px" @click="getImgUrl('coverUrl')">
              <i class="el-icon-plus" />
              <span>点击上传图片</span>
            </div>
            <div v-else>
        ...
        <el-form-item label="学习说明">
          <el-row v-for="(item, index) in learnExplain" :key="index" class="m-b-10">
            <el-input v-model="learnExplain[index]" type="textarea" :row="5" show-word-limit style="width: 500px" class="m-r-10" />
            <el-button v-if="learnExplain.length === index + 1" type="primary" icon="el-icon-plus" @click="addLearnExplain(item)">添加</el-button>
            <el-button v-if="learnExplain.length !== 1" @click="learnExplain.splice(index, 1)">删除</el-button>
          </el-row>
        </el-form-item>

        <el-form-item label="专栏详情" prop="content">
          <AppUEditor ref="AppUEditor" v-model="form.content" @change="change" />
        </el-form-item>

        <el-form-item label="金句" prop="goldenSentence">
          <el-row v-for="(item, index) in goldenSentence" :key="index" class="m-b-10">
            <el-input v-model="goldenSentence[index]" type="textarea" :row="5" show-word-limit style="width: 500px" class="m-r-10" />
            <el-button v-if="goldenSentence.length === index + 1" type="primary" icon="el-icon-plus" @click="addGoldenSentence(item)">添加</el-button>
            <el-button v-if="goldenSentence.length !== 1" @click="goldenSentence.splice(index, 1)">删除</el-button>
          </el-row>
        </el-form-item>
        ...
        <el-form-item label="内容更新顺序" prop="sortFlag">
          <el-radio-group v-model="form.sortFlag">
            <el-radio :label="0" style="width: 100%" class="m-t-10">
              <span>最新添加在前</span>
              <span class="tips m-l-10">适用于资讯类等时效性较强的专栏</span>
            </el-radio>
            <el-radio :label="1" style="width: 100%" class="m-t-10">
              <span>最新添加在后</span>
              <span class="tips m-l-10">适用于有声书等上下文关系较强的专栏</span>
            </el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="相关资料">
          <div class="btnfl">
            <div class="add-course-ware-btn m-r-20" @click="getInforMationPic()"><i class="el-icon-plus" />添加资料</div>
            <div class="tips">最多添加100个文件(后缀为pdf),单个文件不能大于100M;专栏和视频中均存在资料,只显示专栏中资料。</div>
          </div>
              ...
              <el-table-column label="操作" width="240">
                <template slot-scope="scope">
                  <el-button
                    type="text"
                    size="small"
                    @click.native.prevent="deleteRow(scope.$index, form.informationPicReqList)"
                  >
                    移除
                  </el-button>
                </template>
          ...
          <div class="btnfl  m-t-20">
            <div class="add-course-ware-btn m-r-20" @click="selectPPT"><i class="el-icon-plus" />添加PPT</div>
            <div class="tips">最多添加200张jpg/png图片,单张图片不能大于2M;专栏和视频中均存在图片,只显示视频中图片。</div>
          </div>
              ...
              <el-table-column prop="materialName" label="文件">
                <template slot-scope="{ row }">
                  <div class="fl fl_ai_c">
                    ...
                    />
                    <div class="m-l-20">
                      <div class="m-picture-title" :title="row.materialName">{{ row.materialName }}</div>
                      <div class="m-picture-property">{{ row.materialDes | filterDes }}</div>
                    </div>
                  </div>
                </template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="80">
                <template slot-scope="scope">
                  <el-button
                    type="text"
                    size="small"
                    @click.native.prevent="deleteRow(scope.$index, form.informationPptReqList)"
                  >
                    移除
                  </el-button>
                </template>
      ...
      <div class="m-t-20">
        <el-form-item label="售卖方式" prop="sale">
          ...
          <!-- 关联专栏列表 -->
          <el-table
            v-if="form.goodsReqList.length > 0 && form.saleFlag === 0"
            :header-cell-style="{background: '#f5f5f5', padding: '0'}"
            style="width: 800px; margin-top: 20px"
          ...
          >
            ...
            <el-table-column prop="materialSize" label="类型" width="120">
              <template slot-scope="{ row }">
                {{
                  {0: '图片', 1: '音频', 2: '视频', 3: '专栏', 4: '大专栏'}[row.goodsType]
                }}
              </template>
            </el-table-column>
            <el-table-column label="操作" width="80">
              <template slot-scope="scope">
                <el-button
                  type="text"
                  size="small"
                  @click.native.prevent="deleteRow(scope.$index, form.goodsReqList)"
                >
                  取消关联
                </el-button>
              </template>
...
import { addGoods } from '@/api/course'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      form: {
        ...
        goodsId: '', // 图文音频图文id
        goodsType: 3, // 0:图片,1:音频，2视频,3专栏，4大专栏,5线下课程,6,直播专栏
      ...
      rules: {
        ...
        lecturerIdList: [
          { type: 'array', required: true, message: '请至少选择一个IP', trigger: 'change' }
        ],
        patchUrl: [
        ...
        fullNotice: [
          { required: true, message: '请选择报满通知人员', trigger: 'change', type: 'array' }
        ],
        overAllocationNotice: [
          { required: true, message: '请选择超额分配通知人员', trigger: 'change', type: 'array' }
        ]
      },
      srcList: [],
      pickerOptions: {
  ...
  methods: {
    ...
    init() {
      this.loading = true
      axios.all([getTeacherList(), getCatalogueList()]).then(res => {
        console.log(res, '数据')
    ...
    _addGoods() {
      ...
      this.form.informationPicReqList && this.form.informationPicReqList.map((v, i) => {
        informationPicReqList.push({
          informationId: v.materialId,
          sort: i
        })
      })
      this.form.informationPptReqList && this.form.informationPptReqList.map((v, i) => {
        informationPptReqList.push({
          informationId: v.materialId,
          sort: i
        })
      })
      this.form.goodsReqList && this.form.goodsReqList.map(v => {
        goodsReqList.push(v.goodsId)
      })
      let marketPrice
      if (typeof this.form.materialSalePicReq.marketPrice === 'number' || typeof this.form.materialSalePicReq.marketPrice === 'string') {
      ...
      const warningReqList = [...this.form.fullNotice, ...this.form.overAllocationNotice]
      ...
      if (tbody) {
        Sortable.create(tbody, {
          onEnd({ newIndex, oldIndex }) {
            ...
            const currRow = _this.form.informationPicReqList.splice(oldIndex, 1)[0]
    ...
    rowPPTDrop() {
      ...
      if (tbody) {
        Sortable.create(tbody, {
          onEnd({ newIndex, oldIndex }) {
            console.log('拖动了行', '当前序号: ' + newIndex)
            const currRow = _this.form.informationPptReqList.splice(oldIndex, 1)[0]
            currRow.sort = newIndex
