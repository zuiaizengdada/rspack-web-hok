      ...
      <div v-loading="loading" class="page_wrap">
        ...
        >
            ...
            <template v-if="form.goodsForm === 2">
              ...
              <template v-if="form.giftsType === 1">
                ...
                <el-form-item label="赠品退还策略" prop="policyType">
                  <el-select v-model="form.policyType" placeholder="请选择赠品退还策略">
                    <el-option
                      label="退款不需要退回"
                      :value="1"
                    />
          ...
          <div class="m-t-20">
                    ...
                    <div class="validTime">
                      ...
                      >
                        ...
                        <el-radio
                          :label="1"
                          class="w-500 m-b-20"
                          :disabled="form.materialSalePicReq.saleType === 0 && !isCopy"
                        >
                          购买后
                          <el-input-number
                            v-model.number="
                            ...
                            "
                            :disabled="
                              form.materialSalePicReq.saleValidity !== 1 && !isCopy
                            "
                            :min="1"
                            :max="9999"
                            controls-position="right"
            ...
            <div v-if="form.saleFlag === 0" class="add-course-ware-btn m-l-20" style="width: 195px"><i class="el-icon-plus" />选择专栏/训练营</div> -->
              <!-- <div
...
import { editGoods, getGoodsDetail, getPlatformCatalogueList, materialTagList } from '@/api/course'
...
export default {
  ...
  created() {},
  mounted() {
    this.hasPrudectFlag = this.permsList.includes('messageManager:packageCollegeColumn:editGoods')
    this.hasGoodsFlag = this.permsList.includes('messageManager:packageCollegeColumn:editGoodsFlag')
    this.init()
  ...
  methods: {
    ...
    getImage() {
      this.$SelectMaterial({
        ...
        types: [0],
        // disabledFn: (row) => {
        //   return row.materialSize > (500 * 1024) || !(getFileType(row.materialUrl).suffix === 'jpg' || getFileType(row.materialUrl).suffix === 'png')
        // },
        success: res => {
    ...
    _addGoods() {
      ...
      api(params).then(res => {
        ...
        // this.$router.push({ path: '/course/package/packageCollegeColumn' })
        // this.visible = false
