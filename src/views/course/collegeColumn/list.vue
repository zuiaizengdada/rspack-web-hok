  ...
  <div class="page_wrap">
    ...
    >
      ...
      <div slot="headerRight">
        <el-form class="header-form" :inline="true" size="small">
...
export default {
  ...
  data() {
    return {
      ...
      config: {
        tableData: [],
        tableColumns: [
          { prop: 'contentName', width: 380, label: '课程名称', type: 'slot' },
          {
            prop: 'lecturerName',
  ...
  methods: {
    ...
    // 跳转到新建图文页面
    gotoAddVideo() {
      this.$router.push({ path: 'collegeColumnAdd' })
    },
    ...
    gotoDetail(row) {
      console.log(row, 'row')
      this.$router.push({ path: `/shop/packageCollegeColumn/collegeColumnDetail/${row.goodsId}` })
    },
    handleCopyGoods(row) {
