    ...
    >
      ...
      <div slot="headerRight">
        ...
        <el-input
          ...
          placeholder="输入名称"
          clearable
          class="w-150 m-r-10 my-search-input"
        ...
        <div class="fl goods-info-wrapper">
          <ImagePreviewer
            ...
            style="
              ...
              background: #f5f7fa;
            "
            :src="row.contentUrl ? row.contentUrl.split(',')[0] : ''"
            fit="contain"
...
export default {
  ...
  data() {
    return {
      ...
      config: {
        tableData: [],
        tableColumns: [
          { prop: 'contentName', label: '图文名称', type: 'slot' },
          {
            prop: 'relationSaleFlag',
            label: '订阅量',
            type: 'slot',
            width: '120'
          },
  ...
  methods: {
    ...
    // 跳转到新建图文页面
    gotoAddVideo() {
      this.$router.push({ path: 'imageAdd' })
    },
    ...
    gotoDetail(row) {
      console.log(row, 'row')
      this.$router.push({ path: `/shop/image/imageDetail/${row.goodsId}` })
    },
    // 编辑
    gotoEdit(row) {
      this.$AppEditGoods({
