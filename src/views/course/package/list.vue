...
export default {
  ...
  data() {
    return {
      ...
      config: {
        tableData: [],
        tableColumns: [
          { prop: 'contentName', width: 380, label: '课程名称', type: 'slot', visible: true },
          {
            prop: 'lecturerName',
          ...
          //   width: '140'
          // },
          { prop: 'shelveFlag', label: '上架状态', type: 'slot', width: '100', visible: true },
          {
            prop: 'shelveTime',
            label: '上架时间',
            sortable: 'custom',
          ...
          //   width: '200'
          // },
          { prop: 'action', label: '操作', type: 'slot', width: '400', disable: true }
        ],
  ...
  methods: {
    ...
    gotoDetail(row) {
      // if (row.goodsType === 6) {
      //   this.$router.push({ path: `/shop/packageColumn/liveColumnDetail/${row.goodsId}` })
      // } else {
      //   this.$router.push({ path: `/shop/packageColumn/columnDetail/${row.goodsId}` })
      // }
      this.$router.push({ path: `/shop/package/detail/${row.goodsId}` })
    },
    gotoDouEdit(row) {
      this.$AppEditGoods({
        type: 'editColumn',
