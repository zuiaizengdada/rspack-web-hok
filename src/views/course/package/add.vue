        ...
        <div class="m-t-20">
          ...
          <el-form-item>
            <template v-if="form.goodsForm === 2">
              ...
              <!--              <el-form-item label="商品图片" prop="materialGiftsReq.patchUrl" class="mt-6">-->
              <!--                <div class="flex">-->
              <!--                  <AppMyImage-->
              <!--                    v-for="(item, index) in form.materialGiftsReq.patchUrl"-->
              <!--                    :key="index"-->
              <!--                    class="mr-5"-->
              ...
              <!--                      }-->
              <!--                    }"-->
              <!--                    @close="delImage(form.materialGiftsReq.patchUrl, index)"-->
              <!--                    @success="res => changeImage(form.materialGiftsReq.patchUrl, index, res)"-->
              <!--                  />-->
              <!--                  <div-->
              <!--                    v-if="form.materialGiftsReq.patchUrl.length < 7"-->
              <!--                    class="image_poster"-->
              <!--                    style="width: 90px; height: 90px"-->
              <!--                    @click="getImgUrl(form.materialGiftsReq.patchUrl)"-->
              <!--                  >-->
              <!--                    <i class="el-icon-plus" />-->
              <!--                    <span>点击上传图片</span>-->
              <!--                  </div>-->
...
export default {
  ...
  data() {
    ...
    return {
      ...
      form: {
        ...
        goodsId: '', // 图文音频图文id
        goodsType: 7, // 0:图片,1:音频，2视频,3专栏，4大专栏,5线下课程,6,直播专栏
  ...
  mounted() {
    this.hasPrudectFlag = this.permsList.includes('messageManager:packageColumn:addGoods')
    this.hasGoodsFlag = this.permsList.includes('messageManager:packageColumn:addGoodsFlag')
  },
  methods: {
    getMaterialTagList() {
    ...
    _addGoods() {
      ...
      const materialSalePicReq = {
        ...
        saleValidity: 1 // 0 长期有效 1 购买后 2 固定时间
      }
      ...
      const params = {
        ...
        coverVerticalUrl: this.form?.patchUrl[0],
      ...
      params.materialSalePicReq.salePrice = Math.round((params.materialSalePicReq.salePrice || 0) * 100)
      params.marketPrice = Math.round((params.marketPrice || 0) * 100)
      params.materialSalePicReq.marketPrice = Math.round((params.materialSalePicReq.marketPrice || 0) * 100)
