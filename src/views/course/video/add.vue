<template>
  <div v-loading="loading" class="page_wrap">
    <div class="header">
      <span class="pointer" @click="$router.push({ path: 'list' })">视频</span>
      > 新建视频
    </div>
    <el-form
      ref="form"
...
import { addGoods, getPlatformCatalogueList } from '@/api/course'
...
export default {
  ...
  methods: {
    ...
    init() {
      this.loading = true
      axios.all([getTeacherList(), getCatalogueList(), getPlatformCatalogueList()]).then(res => {
        console.log(res, '数据')
