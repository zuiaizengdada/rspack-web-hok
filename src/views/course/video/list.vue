  ...
  data() {
    return {
      ...
      config: {
        tableData: [],
        tableColumns: [
          { prop: 'contentName', width: 280, label: '视频名称', type: 'slot' },
          {
            prop: 'lecturerName',
            slotFn: row => row.lecturerVos?.map(i => i.lecturerName)?.join('/') || '--',
            label: 'IP',
            type: 'text'
          },
          {
  ...
  methods: {
    ...
    getList() {
      ...
      const params = {
        ...
        goodsType: this.goodsType, // 0:图片,1:音频，2视频,3专栏，4大专栏
      ...
      getGoodsList(params)
        .then(res => {
          ...
          // ?.map(i => {
          //   const douRejectReason = i.douRejectReason ? JSON.parse(i.douRejectReason) : {}
          //   let douReason = ''
          //   for (const key in douRejectReason) {
          //     douReason += (`${key}：${douRejectReason[key].join('、')};`)
          //   }
          //   return {
          //     ...i,
          //     douReason
    ...
    gotoDetail(row) {
      console.log(row, 'row')
      this.$router.push({ path: `/shop/video/videoDetail/${row.goodsId}` })
    },
    // 编辑
    gotoEdit(row) {
      // this.$router.push({ path: `/course/videoEdit/${row.goodsId}` })
      this.$AppEditGoods({
        type: 'editVideo',
        goodsId: row.goodsId,
        visible: true,
...
<style lang="scss">
.el-button--text.douExamineStatus {
  span {
