    ...
    <div class="add-order-warp">
      ...
      >
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="优惠券名称" prop="couponName">
              ...
              />
              <p v-if="$route.query.type === 3" class="tips-form">
                该名称仅用于管理，不对用户展示
              </p>
              <p v-else class="tips-form">
                优惠券名称会显示到用户端，请认真填写
              </p>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          ...
          <el-col v-if="form.couponType === 1" :span="8">
            <el-form-item label="员工可用范围">
              <el-cascader
                v-model="form.depts"
                class="m-r-10 m-l-10"
                size="small"
  ...
  methods: {
    ...
    loadDetailData() {
      ...
      getCouponInfoDetail(params).then(res => {
        ...
        if (res.code === 1) {
          if (res.data.depts) {
            const mapDp = res.data.depts.map(m =>
              m
                .split('-')
                .map(n => Number(n))
            )
            this.form.depts = mapDp
          }
          this.form.couponType = res.data.couponType
      ...
      this.$refs[formName].validate((valid, fields) => {
        if (valid) {
          ...
          if (this.form.depts.length > 0) {
            depts = this.form.depts.map(item => item[item.length - 1])
          }
          const params = {
          ...
          addOrUpdateCouponInfo(params).then(res => {
            if (res.code === 1) {
              ...
              this.$router.push({
                path: '/yxgj/couponManager?fromTab=couponManager'
              })
            } else {
