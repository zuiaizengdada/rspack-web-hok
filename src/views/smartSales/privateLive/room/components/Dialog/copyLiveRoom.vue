<template>
  ...
  >
    <div v-loading="loading" class="copyLiveRoom overflowOuto">
      ...
      <template v-if="copyLive.status === 0">
        ...
        >
          ...
          <el-form-item
            v-else-if="form.liveModel === 'AI_LIVE'"
            label="视频讲师"
          >
          ...
          <el-form-item label="直播类型">
            <div
              :style="{
                color: form.liveModel === 'AI_LIVE' ? '#00B42A' : '#0C6FFF'
              }"
            >
          ...
          <template v-else-if="form.liveModel === 'AI_LIVE'">
            ...
            <el-form-item label="视频名称">
              <span class="formValue a_link" @click="gotoVideo(form)">{{
                form.videoName
              }}</span>
              <el-button
                style="margin-left: 10px"
                @click="openChangeVideoConfig(form)"
              >
                更换视频
              </el-button>
            </el-form-item>
          ...
          <el-form-item prop="copyOption">
            ...
            <el-checkbox-group v-model="form.copyOption">
              ...
              <el-checkbox label="isNotice">公告</el-checkbox>
              <el-checkbox label="isSetting">直播间设置</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
    ...
    <div slot="footer" class="exportScriptFooter">
      ...
      <el-button size="small" :loading="loading" @click="visible = false">
        取消
      </el-button>
    </div>
  </AppDialog>
...
import { getLiveRoomInfo, copyLiveRoomApi } from '@/api/liveRoom/index'
      ...
      Dialog.open({
        ...
        success: res => {
          if (res) {
            ...
            this.form.scriptId = res.scriptId
            this.form.scriptName = res.scriptName
            Dialog.close()
          }
        }
      })
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          const data = {
            ...
            name: this.form.name,
            startTime: starTime,
            scriptId: this.form.scriptId || undefined,
            videoId: this.form.videoId || undefined,
            copyOption: {
              ...
              isNotice: this.form.copyOption.includes('isNotice') ? 1 : 0,
              isSetting: this.form.copyOption.includes('isSetting') ? 1 : 0
            },
            teacherIdList: tutorTeacherUserIds
          }
          copyLiveRoomApi(data)
            .then(res => {
              this.loading = false
