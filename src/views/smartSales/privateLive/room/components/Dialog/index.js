...
const requireComponent = require.context(
  // 其组件目录的相对路径
  '@/views/smartSales/privateLive/room/components/Dialog',
  // 是否查询其子目录
  true,
  // 匹配基础组件文件名的正则表达式
  /\w+\.vue$/
...
export default {
  open(options = {}) {
    ...
    if (instance && (!instance.visible || instance.type === options.type)) {
      instance.$el?.parentNode && document.body.removeChild(instance.$el)
    }
    // if (!instance) {
    // }
