  ...
  <div v-loading="pageloading" class="registerSettings-page">
    <div>
      <registerMobile
        :img-list="imgList"
        :live-info="detail"
        :cover-img-url="form.coverImgUrl"
        :buttom-title-obj="{
          notApplyViewText: form.notApplyViewText,
          applyViewText: form.applyViewText
        }"
      />
    </div>
    <div class="registerSettings-form">
      ...
      >
        <el-form-item label="允许进入直播间" prop="model">
          <el-radio-group :value="modelValue">
            <el-radio
        ...
        <el-form-item v-if="modelValue === 2">
        ...
        <el-form-item label="直播封面" prop="coverImgUrl" class="zb-cover">
          <template>
            <!-- <upload
              key="MUpload"
              ref="MUpload"
        ...
        <el-form-item label="需要填写手机号" prop="applyPhoneFlag">
          <el-switch
        ...
        <el-form-item label="底部按钮文字">
          <div class="row-i-bot" style="margin-bottom: 20px">
            <span>未报名用户，显示</span>
            <el-input
              v-model="form.notApplyViewText"
              :disabled="isEdit"
              maxlength="10"
              class="row-i-bot-input"
              placeholder=""
            />
          </div>
          <!-- <div class="row-i-bot">
        ...
        >
          ...
          <div
            v-if="[1, 2].includes(form.applyOfficialFlag)"
            class="officialFlag-container marginTop_8"
          >
            <el-form-item
              label="选择公众号"
            ...
            >
              <el-select
                v-model="form.applyOfficialId"
                clearable
                :disabled="isEdit"
                @change="officialListChange"
              >
                <el-option
              ...
              <img
                v-if="form.applyOfficialId"
                width="88px"
                height="88px"
                class="officialFlag-img"
                :src="selectQrcodeUrl"
              />
            </el-form-item>
          </div>
        ...
        <el-form-item label="消息通知">
          ...
          >
            <el-checkbox
              label="1"
              :disabled="disablemsgType.includes('1')"
            >短信通知</el-checkbox>
            <el-checkbox
              label="2"
              :disabled="msgTypeApplyOfficialDisable"
            >公众号通知</el-checkbox>
          </el-checkbox-group>
          <div v-if="msgType.length" class="officialFlag-container">
            <applyMsgs
              ...
              :key="item"
              :ref="`applyMsgsObj${item}`"
              :is-edit="isEdit"
              :config="applyMsgsObj[item]"
              :apply-official-id="form.applyOfficialId"
              :class="{ 'applyMsgs-line': msgType.length === 2 }"
            />
          </div>
        </el-form-item>
        <el-form-item label="下一场直播预告">
          ...
          <div
            v-if="form.liveRoomTrailers.length > 0"
            class="labelLiveRoomList"
          >
            <el-tag
        ...
        <el-form-item label="">
          <div style="margin-top: 20px">
            ...
            <el-button size="small" :disabled="isEdit" @click="clear">
              重置
            </el-button>
          </div>
        </el-form-item>
...
<script>
import {
  applyConfigDetailAjax,
  editApplyConfigAjax,
  officialListAjax
} from '@/api/liveRoom/register'
...
// import upload from './coverUpload/upload.vue'
...
export default {
  ...
  computed: {
    msgTypeApplyOfficialDisable() {
      return (
        ...
        [0].includes(this.form.applyOfficialFlag) ||
        this.disablemsgType.includes('2') ||
        this.form.applyPhoneFlag === 0
      )
    }
  },
  watch: {
    'form.applyPhoneFlag'(val) {
      if (val === 0) {
        !this.disablemsgType.includes('2') &&
          (this.msgType = this.msgType.filter(item => item !== '2')) // 清空勾选的公众号通知
      }
    },
  ...
  methods: {
    ...
    getLiveRoomUserLimitInfo() {
      getLiveRoomUserLimit({
        liveRoomId: this.$route.params.id
      })
        .then(res => {
          this.modelValue = res.data?.model || 1
          console.log(this.modelValue, 'vvvvvvvvvvvvvvv')
        })
        .catch(() => {
          this.loading = false
        })
    ...
    onChange(type) {
      ...
      this.$delModal({
        ...
        fail: () => {
          this.modelValue = this.modelValue === 1 ? 1 : 2
        }
      })
    },
    onApplyPageFlagChange(val) {
      if (val === 0) {
        this.form.applyLiveEnd = false
      }
    },
    // 获取报名设置详情
    applyConfigDetail() {
      ...
      applyConfigDetailAjax({ liveRoomId })
        .then(async res => {
          if (res.code === 1) {
            const { applyImgUrl, applyMsgs, applyOfficialId, coverImgUrl } =
              res.data
            this.form = { ...res.data }
            this.defaultNotApplyViewText = res.data.notApplyViewText
            // 回显报名详情页的图片
            if (applyImgUrl) {
    ...
    sure() {
      ...
      Promise.all(promiseList).then(() => {
        ...
        params.liveRoomTrailers = this.form.liveRoomTrailers.map(
          item => item.liveRoomId
        )
        ...
        editApplyConfigAjax(params).then(res => {
          if (res.code === 1 && res.success === true) {
            ...
            this.applyConfigDetail()
            // 通知顶部封面更新
            this.$emit('enrollSettingcoverImgUrl')
            this.$message({
              message: '保存成功',
              type: 'success',
              duration: 1500
    ...
    clear() {
      this.form = {
        ...
        applyMsgs: [],
        model: 1,
        notApplyViewText: this.defaultNotApplyViewText
      }

      this.msgType = this.disablemsgType
      this.applyMsgsObj =
        Object.keys(this.disabledMsgsObj).length > 0
          ? this.disabledMsgsObj
          : {
            1: {
              msgType: 1,
              sendMsgFlag: 1,
    ...
    handleSureApplyMsgs() {
      ...
      const list = []
      applyMsgs1 &&
        applyMsgs1.applyMsgs.reduce((pre, next) => {
          list.push({
      ...
      applyMsgs2 &&
        applyMsgs2.applyMsgs.reduce((pre, next) => {
          list.push({
            timeMinute: next.timeMinute || '',
            timeType: next.timeType,
            msgTemplateId: next.msgTemplateId || '',
    ...
    officialListChange(e) {
      ...
      this.selectQrcodeUrl = this.officialList.find(
        item => item.officialId === e
      )?.qrcodeUrl
    },
    // 消息通知change
    msgTypeChange(e) {},
...
::v-deep {
  ...
  .enroll-setting-MUpload .uploadItem .image_poster .uploadShow .maskdShow {
    background: linear-gradient(
      180deg,
      rgba(38, 38, 38, 0) -17.19%,
      rgba(22, 22, 22, 0.43) 23.44%,
      #000 100%
    );
  }
}
