  ...
  <div v-loading="loading">
    ...
    >
      ...
      <el-form-item label="直播类型" prop="feesType">
        <div class="flex flex-middle" style="height: 32px">
          <el-radio-group
            v-model="form.feesType"
            :disabled="form.status !== 0 || goodsList"
      ...
      <el-form-item v-if="form.feesType === 2" prop="goods">
        ...
        <goodsList
          v-if="form.goods.length > 0"
          :data="form.goods"
          :is-del="form.status === 0"
          style="width: 663px"
          :type="goodsList ? 'copy' : 'create'"
          @del="onhandleDelInfoGoods"
        />
        <el-form-item
          v-show="form.goods.length && !goodsList"
          label="商品订单支付时间"
          label-width="140px"
        ...
        >
          <el-date-picker
            ...
            :picker-options="paymentPickerOptions"
          />
        </el-form-item>
      </el-form-item>
      ...
      <el-form-item label="直播场景" prop="type">
        <div class="flex flex-middle" style="height: 32px">
          <el-radio-group v-model="form.type" :disabled="form.status === 9">
          ...
          <el-tooltip
            class="item"
            effect="dark"
            content="测试直播间创建后可以删除，测试数据不纳入数据看板统计，测试直播间如果是真人直播可随时开播。。正式直播间开播后不可以删除。"
            placement="right"
          >
            <img
              class="wenhao-icon"
      ...
      <el-form-item label="暖场素材">
        <div class="flex flex-middle" style="height: 32px">
          <el-radio-group
            v-model="form.warmUp"
            :disabled="[1, 2, 3, 9].includes(form.status)"
          >
      ...
      <el-form-item label="直播模式" prop="liveModel">
        ...
        <div class="tipsError">注：请谨慎选择直播模式，一旦确定不可修改。</div>
      </el-form-item>
      ...
      >
        ...
        <AppSelect
          ...
          :popover-width="245"
          :disabled="[1, 2, 3, 9].includes(form.status) || ifLtr5min()"
          :value="form.videoId"
      ...
      <el-form-item label="展示模式" prop="displayMode" class="fi-bold">
        <el-radio-group
          v-model="form.displayMode"
          :disabled="
            [1, 2, 3, 9].includes(form.status) ||
              ifLtr5min() ||
      ...
      >
        <div class="flex flex-middle" style="height: 32px">
          ...
          <el-tooltip
            class="item"
            effect="dark"
            content="若选择为不允许，那么使用这个视频的所有直播间，同一个用户不能观看；反之，同一个用户则被允许重复观看。"
            placement="right"
          >
            <img
              class="wenhao-icon"
      ...
      <el-form-item label="开播时间" prop="startTime" class="fi-bold">
        <el-date-picker
      ...
      >
        <course-instructor
          ...
          :form="form"
          @select="handleSelectCourseInstructor"
        />
      </el-form-item>
      <el-form-item label="运营助教" prop="tutorTeacherUser" class="fi-bold">
        <el-button
          type="primary"
          :disabled="form.status === 9"
          @click="addAssistant"
        >
          添加助教({{ form.tutorTeacherUser.length }}/100)
        </el-button>
        <div>
      ...
      <el-form-item label="直播人数" prop="isShow" class="fi-bold">
        <el-radio-group v-model="form.isShow" :disabled="form.status === 9">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        realityTeacherUser: [{ required: true, message: '请选择讲师' }],
        goods: [
          {
            type: 'array',
            required: true,
  ...
  mounted() {
    ...
    getLiveRoomInfo(params)
      .then(res => {
        ...
        this.checkGoodsList =
          (res.data.materialGoodsDetail &&
            res.data.materialGoodsDetail?.map(m => m.goodsId)) ||
          []
        res.data.taskList &&
        ...
        const orederTimePays = res.data.materialGoodsDetail.length
          ? res.data.materialGoodsDetail[0]
          : undefined
        let paymentTime = []
        if (
          orederTimePays &&
          orederTimePays.payStartTime &&
          orederTimePays.payEndTime
        ) {
          paymentTime = [
            moment(orederTimePays.payStartTime).format('YYYY-MM-DD'),
            moment(orederTimePays.payEndTime).format('YYYY-MM-DD')
          ]
        }
        this.form = {
        ...
        this.$nextTick(() => {
          if (this.$refs.courseInstructorRef) {
            this.$refs.courseInstructorRef.emceeUserId =
              res.data.liveRoomPersonInfo.roundTableMasterId
            this.$refs.courseInstructorRef.userIds = res.data.teacherInfoList
              .filter(f => f.teacherRole === 0)
              .map(m => m.teacherId)
            const ls = realityTeacherUser.map(m => ({
              ...m,
              userName: m.nickName,
              isCheckedHost:
                m.userId === res.data.liveRoomPersonInfo.roundTableMasterId
            }))
            this.$refs.courseInstructorRef.realityTeacherUser = [...ls]
          }
          this.$refs.MUpload &&
  ...
  methods: {
    feesTypeChange(val) {
      ...
      if (val === 2) {
        this.form.paymentTime = [
          moment().days(-365).format('YYYY-MM-DD'),
    ...
    handleSelectCourseInstructor(data) {
      this.form.roundTableMasterId = data.emceeUserId
      const ids = []

      data.realityTeacherUserIds.map(m => {
        if (ids.findIndex(n => n.userId === m) === -1) {
          ids.push({
            userId: m
          })
        }
      ...
      this.form.realityTeacherUser = ids
    },
    // 点击切换直播模式
    changeliveModel(val) {},
    // 获取课件是否有更新
    ...
    handleChangeDate() {
      ...
      const isNotToday = this.validateStartTime2(this.form.startTime1)
    ...
    sure() {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          const payStartTime =
            this.form.paymentTime && this.form.paymentTime[0]
              ? moment(this.form.paymentTime[0]).format('YYYY-MM-DD 00:00:00')
              : undefined
          const payEndTime =
            this.form.paymentTime && this.form.paymentTime[1]
              ? moment(this.form.paymentTime[1]).format('YYYY-MM-DD 23:59:59')
              : undefined
          ...
          const params = {
            ...
            startTime: starTime,
            tutorTeacherUserIds,
            emceeUserId: this.form.roundTableMasterId,
          ...
          params.liveRoomId = this.form.id
          this.loading = true
          const apiFn = editLiveRoomV2
    ...
    addAssistant() {
      ...
      if ([1, 2, 3].includes(this.form.status)) {
        disabledUser = disabledUser.concat(this.defaultDisabledUser)
      }
      this.$AddressBook({
        visible: true,
    ...
    onhandleDelInfoGoods(row) {
      console.log(row, 'vvv')
      console.log(this.checkGoodsLis, this.form.goods)
      const index = this.form.goods.findIndex(i => i.goodsId === row.goodsId)
      this.form.goods.splice(index, 1)
      const index2 = this.form.goods.findIndex(i => i.goodsId === row.goodsId)
      this.checkGoodsList.splice(index2, 1)
    },
    onhandleSelectGoods() {
      this.$SelectGoods({
        visible: true,
