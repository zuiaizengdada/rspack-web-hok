...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        coursewareName: [
          { required: true, message: '请输入备课内容名称', trigger: 'blur' }
        ],
        refType: [
          { required: true, message: '请输入关联类型', trigger: 'blur' }
        ],
        refVideoId: [
          { required: true, message: '请选择关联视频课件', trigger: 'blur' }
        ]
      },
      loading: false,
      videoList: [],
    ...
    'config.visible': {
      handler(val) {
        ...
        if (this.config.type === 2) {
          this.loading = true
          coursewareDetail(this.config.row.coursewareId)
            .then(res => {
              if (res.data.operateWhiteList.length === 0) {
                res.data.operateWhiteList = [
                  {
                    userId: this.createUserInfo.userId,
    ...
    onAdd() {
      ...
      addCourseware(data)
        .then(res => {
          ...
          window.open(`/#/live/prepareLessons/${res.data}?isNewLiveRoom=true&videoId==${this.query.videoId}&videoName=${this.query.videoName}`)
          this.$emit('success')
          this.loading = false
        })
        .catch(() => {
