...
export default {
  ...
  mounted() {
    ...
    erd.listenTo(this.$refs.content, () => {
      this.$nextTick(() => {
        if (this.$el) this.screenHeight = this.$el.clientHeight
        this.updateItemsSize()
      })
    })
  },
  ...
  methods: {
    ...
    // 获取结束索引
    getEndIndex() {
      let index = this.start + this.visibleCount
    ...
    updateItemsSize() {
      ...
      this.$nextTick(() => {
        nodes.forEach((node) => {
          ...
          const oldHeight = this.positions[index]?.height || this.estimatedItemSize
          const dValue = oldHeight - height
          // 存在差值
          if (dValue && this.positions[index]) {
            this.positions[index].top = this.positions[index].top - dValue
            this.positions[index].bottom = this.positions[index].bottom - dValue
            this.positions[index].height = height
            for (let k = index + 1; k < this.positions.length; k++) {
              this.positions[k].top = this.positions[k - 1].bottom
              this.positions[k].bottom = this.positions[k].bottom - dValue
              this.positions[k].height = this.positions[k].bottom - this.positions[k].top
            }
          }
        })
      })
    ...
    scrollEvent(evt) {
      ...
      this.timer = setInterval(this.isScrollEnd, 10)
    ...
    scrollTop(index, fn) {
      const el = document.getElementById(`${this.idName}_${index}`)
      if (!el) {
        console.log(index, '对应元素不存在', `${this.idName}_${index}`)
        return
      }
      if (index === this.length - 1) {
        console.log('最后一个')
        this.ifScrollToIndex = ''
        this.timerNum = 0
        clearInterval(this.timer)
      ...
      this.timerNum++
      console.log(this.isInViewPort(el), '是否在试图内')
      if (!this.isInViewPort(el)) {
        console.log(el, 'el')
        const { top } = el.getBoundingClientRect()
        console.log(top)
        const idName = `${this.idName}_scriptConfiguration`
        const el2 = document.getElementById(idName)
        console.log(top - 207)
        if (top - 207 > 0) {
          el2.scrollTop += 30
        } else {
    ...
    isInViewPort(el) {
      ...
      console.log(top, bottom, '是否在试图内', top >= 207 && top + height <= viewHeight - 84)
      return top >= 207 && top + height <= viewHeight - 84
    },
    clearTime() {
      if (this.timer) {
        this.ifScroll = false
    ...
    isScrollEnd() {
      ...
      if (this.t2 === this.t1) {
        clearInterval(this.timer)
        this.ifScroll = false
        console.log('滚动结束了', this.ifScrollToIndex)
        if (this.ifScrollToIndex !== '') {
          this.$nextTick(() => {
            this.scrollTop(this.ifScrollToIndex)
          })
        }
      }
    }
