    ...
    <div class="filterTop">
      ...
      >
        <el-form-item label="" prop="userInfo">
          ...
          <selectTelEmail v-model="search.userInfo" :source-list="sourceList" class="m-r-10" size="small" @onChange="searchFn" @clearValue="clearValidate" />
          ...
          <div v-if="value.childrenOpen">
            <div v-for="(info, i) in value.dataList" :key="i" class="goodsList">
              <el-descriptions :column="3">
                ...
                <el-descriptions-item label="首次进入时间" :span="3">{{
                  info.firstEntryTime | renderTime('MM-DD HH:mm:ss')
                }}</el-descriptions-item>
                <el-descriptions-item label="直播观看时长">{{
                  info.liveWatchTime | filtersListen
                }}</el-descriptions-item>
                <el-descriptions-item label="回放观看时长">{{
                  info.playbackWatchTime | filtersListen
                }}</el-descriptions-item>
                <el-descriptions-item label="总观看时长">{{
                  info.totalWatchTime | filtersListen
                }}</el-descriptions-item>
              </el-descriptions>
            </div>
          </div>
      ...
      <div v-show="ifError || total === 0" class="noData">
        <template v-if="ifError">
          ...
          <div class="noDataText">
            {{
              `请输入用户准确的${
                { tel: '手机号', Email: '邮箱' }[search.userInfo.label]
              }进行查询！`
            }}
          </div>
        </template>
  ...
  data() {
    var checkUserInfo = (rule, value, callback) => {
      ...
      if (this.search.userInfo.label === 'tel') {
        ...
        if (
          !this.search.userInfo.value.Email ||
          !regex.test(this.search.userInfo.value.Email)
        ) {
  ...
  methods: {
    ...
    getObj(arr) {
      ...
      for (const key in obj) {
        obj[key].sort((a, b) => {
          return (
            moment(b.liveStartTime).valueOf() -
            moment(a.liveStartTime).valueOf()
          )
        })
    ...
    getList() {
      ...
      const params = {
        ...
        pageSize: this.size,
        phoneNumber:
          this.search.userInfo.label === 'tel'
            ? this.search.userInfo.value.tel
            : '',
        email:
          this.search.userInfo.label === 'Email'
            ? this.search.userInfo.value.Email
            : ''
      }
      console.log(params)
      getWatchHistoryByPhone(params)
        .then(res => {
          console.log(res, 'res')
...
.viewingRecord {
  @include responsive-height(125px);
  width: 100%;
  display: flex;
