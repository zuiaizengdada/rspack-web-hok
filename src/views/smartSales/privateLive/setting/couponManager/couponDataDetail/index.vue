<template>
  <div class="entry-box">
      ...
      <el-form ref="form" :model="form" label-width="80px" class="form-box">
        ...
        <el-row>
          <el-col :span="24" class="add-goods-row">
            <span v-if="chooseGoodsArray.length > 0" class="tips-gs"
              >已选择<em>{{ chooseGoodsArray.length }}</em
              >个商品</span
            >
          </el-col>
        </el-row>
        ...
        >
          <el-form-item class="submit-row-cls-submit">
            <el-button
              ...
              v-permission="[
                form.auditStatus === 1
                  ? 'btn:CouponDataDetails:ok'
                  : 'btn:CouponDataDetails:recheckOk',
                permsList
              ]"
              >通过</el-button
            >
            <el-button
              ...
              v-permission="[
                form.auditStatus === 1
                  ? 'btn:CouponDataDetails:down'
                  : 'btn:CouponDataDetails:recheckDown',
                permsList
              ]"
              >驳回</el-button
            >
    ...
    >
      ...
      />
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="small" @click="sureToBohui"
          >确 定</el-button
        >
    ...
    sureToBohui() {
      ...
      if (this.form.auditStatus === 1) {
        ...
        reexaminationRejection(params).then(res => {
          if (res.code === 1) {
            console.log(444444444, prevPath)
            this.$message.success('已驳回')
            this.cancelSubmit = false
            this.$router.push({
              path: prevPath,
