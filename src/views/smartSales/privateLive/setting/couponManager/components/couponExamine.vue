  ...
  <div v-if="pageShow" class="order-list">
    ...
    <div class="coupon-data-row flex justify-between">
      <div class="data-row-btn">
        ...
        >复审优惠券</span>
        <span
          v-permission="['web:couponManager:down', permsList]"
          :class="auditStatus === 3 ? 'active' : ''"
          @click="changeAuditStatus(3)"
        >审核驳回</span>
        <span
          v-permission="['web:couponManager:pass', permsList]"
      ...
      <div>
        <el-button
          v-permission="['btn:couponManager:auditor', permsList]"
          type="primary"
          size="small"
    ...
    >
      ...
      <template v-slot:operate="{ scope }">
        <el-button
          v-permission="[authorityMap[auditStatus], permsList]"
          type="text"
...
export default {
  ...
  data() {
    return {
      pageShow: true,
      authorityMap: {
        1: 'btn:couponManager:Preliminary:details',
        2: 'btn:couponManager:Reexamine:details',
        3: 'btn:couponManager:down:details',
        5: 'btn:couponManager:pass:details'
      },
      orderTimeArray: [],
      columns,
    ...
    initAuthority() {
      const permsList =
        getRoutesPermsList('setting').find(item => item.path === 'couponMg')
          ?.meta?.permsList || []
      if (permsList.includes('web:couponManager:Preliminary')) {
        this.auditStatus = 1
        return
      }
      if (permsList.includes('web:couponManager:Reexamine')) {
        this.auditStatus = 2
        return
      }
      if (permsList.includes('web:couponManager:down')) {
        this.auditStatus = 3
        return
      }
      if (permsList.includes('web:couponManager:pass')) {
        this.auditStatus = 5
        return
      }
      this.pageShow = false
    ...
    toDetail(row) {
      ...
      this.$router.push({
        ...
        query: {
          ...
          search: forSearch,
          prevPath: '/privateLive/setting/couponMg'
        }
      })
    },
    loadUserNoPage() {
