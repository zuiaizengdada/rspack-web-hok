<template>
  <div v-if="pageShow" class="order-list">
      ...
      <template v-slot:operate="{ scope }">
        <el-button
          v-if="
            authorityBtn(
              couponType === 0
                ? 'btn:couponMg:mySetUp:details'
                : 'btn:couponMg:myManagement:details'
            )
          "
          type="text"
        ...
        <el-button
          v-if="
            authorityBtn(
              couponType === 0
                ? 'btn:couponMg:mySetUp:copy'
                : 'btn:couponMg:myManagement:copy'
            )
          "
          type="text"
        ...
        <el-button
          ...
          "
          v-permission="['btn:couponMg:mySetUp:edit', permsList]"
          type="text"
        ...
        <el-button
          v-show="
            authorityBtn(
              couponType === 0
                ? 'btn:couponMg:mySetUp:cancellation'
                : 'btn:couponMg:myManagement:cancellation'
            )
          "
          v-if="
...
export default {
  ...
  components: { HokSearchForm, ZBTable },
  data() {
    return {
      keyIndex: 1,
      pageShow: true,
      ...
      typeList: [
        {
          name: '我创建的',
          permission: 'web:couponManager:MyCoupon',
          value: 0
        },
        {
          name: '我管理的',
          permission: 'web:couponManager:MyManager',
          value: 1
        }
      ]
    }
  ...
  methods: {
    authorityBtn(value) {
      const permsList =
        getRoutesPermsList('setting').find(item => item.path === 'couponMg')
          ?.meta?.permsList || []
      return permsList.includes(value)
    },
    initAuthority() {
      const permsList =
        getRoutesPermsList('setting').find(item => item.path === 'couponMg')
          ?.meta?.permsList || []
      if (permsList.includes('web:couponManager:MyCoupon')) {
        this.couponType = 0
        return
      }
      if (permsList.includes('web:couponManager:MyManager')) {
        this.couponType = 1
        return
      }
      if (permsList.includes('web:couponManager:MyUse')) {
        this.couponType = 2
        return
      }
      this.pageShow = false
    ...
    toDetail(row) {
      ...
      this.$router.push({
        path: `/privateLive/setting/CouponDataDetails`,
        query: {
    ...
    toCopy(row) {
      ...
      this.$router.push({
        path: `/privateLive/setting/addCoupon`,
        query: {
          couponNo: row.couponNo,
          fromTab: 'couponManager',
          search: paramsString,
