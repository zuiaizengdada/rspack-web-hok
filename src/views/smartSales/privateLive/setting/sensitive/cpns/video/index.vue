    ...
    >
      ...
      <template v-slot:operate="{ scope }">
        <el-button
          ...
          v-permission="['btn:sensitive:VideoTp:edit', permsList]"
        >
          编辑
        </el-button>
        <el-button
          ...
          class="red"
          @click="del(scope.row)"
          v-permission="['btn:sensitive:VideoTp:delete', permsList]"
        >
          删除
        </el-button>
      </template>
      <template #footer>
        <el-button
          size="small"
          @click="allChange(0)"
          v-permission="['btn:sensitive:VideoTp:deletes', permsList]"
          >批量删除</el-button
        >
      </template>
    </ZBTable>
...
import { videoConfig } from '../../data.js'
...
export default {
      ...
      formOpts: {
        ...
        operatorList: [
          ...
          },
          {
            ...
            bind: { plain: true },
            className: ['right'],
            permission: 'btn:sensitive:VideoTp:lead'
          },
          {
            label: '新增敏感词',
            fn: this.add,
            className: ['right'],
            permission: 'btn:sensitive:VideoTp:add'
          }
        ]
      },
      tableConfig: {
  ...
  methods: {
    ...
    add() {
      this.$refs.AddDialogRef.openDialog({ type: 'add', sceneType: 2 })
    },
    edit(row) {
    ...
    searchFn(isClear, isResetPageIndex = true) {
      ...
      if (isClear) {
        ...
        this.formOpts.formData = { sceneType: 2 }
      }
      this.$refs.zbTableRef.init(isResetPageIndex)
    },
    clearFn() {
      this.formOpts.ref.resetFields()
      console.log('videoConfig.form.formData-------', videoConfig.form.formData)
      this.formOpts.formData = cloneDeep(videoConfig.form.formData)
    ...
    importBtn() {
      this.$refs.ImportDialogRef.openDialog({ sceneType: 2 })
    },
    couponTypeChange() {
