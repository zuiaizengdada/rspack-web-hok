    ...
    <div v-if="replySetting.ifReply" class="message_reply">
      <div class="messageReply text_hidden_ellipsis">
        ...
        <div class="text_hidden_ellipsis">
          <span>回复 </span>
          {{ replySetting.data.messageReply.messageSenderName || replySetting.data.messageReply.messageSender }}
          :
...
export default {
  ...
  watch: {
    value: {
      handler(val) {
        if (this.isBlur) {
          this.$refs[`divEditor_${this.id}`] && (this.$refs[`divEditor_${this.id}`].innerHTML = val)
        }
  ...
  methods: {
    ...
    insertContent(value, id = '') {
      ...
      sel.addRange(range)
      this.$nextTick(() => {
        if (/@<span name="at"/.test(this.$refs[`divEditor_${this.id}`].innerHTML)) {
          // 使用正则替换，将已经输入的@替换掉
          // 如果直接赋值修改innerHTML，则光标默认会回到开头。因此需要额外处理光标
          this.$refs[`divEditor_${this.id}`].innerHTML = this.$refs[`divEditor_${this.id}`].innerHTML.replace(/@<span name="at"/, '<span name="at"')
    ...
    // 输入框清空
    clear() {
      this.$refs[`divEditor_${this.id}`] && (this.$refs[`divEditor_${this.id}`].innerHTML = '')
      this.$refs[`divEditor_${this.id}`].focus()
    },
    // 点击删除回复
