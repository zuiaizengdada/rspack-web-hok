  ...
  >
    <div class="sign">
      <div v-if="!ifCountdown" class="pageWrapForm">
        <!-- 签到选择页面 -->
        <el-form ref="form" v-loading="loading" :model="form" label-width="80px" size="small" :rules="rules" @submit.native.prevent>
          <el-form-item label="签到方式" prop="signType">
            <el-select v-model="form.signType" style="width: 245px" :disabled="!form.ifEdit" @change="onhandleChangeSignType">
          ...
          <el-form-item label="签到引导语1" prop="signGuideWord">
            <el-input
          ...
          <el-form-item class="m-t-48">
            <el-button v-if="form.signId && form.ifEdit" :loading="loading" type="primary" @click="updateLiveRoomSign">保存</el-button>
            <el-button v-if="form.signId && !form.ifEdit" type="primary" @click="onhandleEditSign">编辑</el-button>
            <el-button v-if="form.signId" class="btn_primary" @click="cancalLiveRoomSign(form.signId)">取消签到</el-button>
    ...
    <div slot="footer" />
    <div slot="title" class="signUserInfo">
      <span v-if="signForm.ifSignStart === 0" @click="openSignLogDialog">签到记录</span>
      <span v-if="signForm.ifSignStart === 2" @click="openDialog">签到结果</span>
    </div>
  </AppDialog>
</template>
...
export default {
  ...
  mounted() {
    this.top = Math.abs(window.innerHeight - 514) / 2 + 'px'
  },
  methods: {
    // 获取签到详情
    getSignDetail() {
