      ...
      <div class="addDialog overflowOuto">
        <el-form ref="form" v-loading="loading" :model="form" label-width="auto" size="small" :rules="rules">
          ...
          <el-form-item label="标签">
            <el-input v-model="currentTagValue" placeholder="请输入标签名称" @keyup.native.enter="handleChange" />
            <el-tag v-for="item in form.tags" :key="item" closable style="margin-right:6px;" @close="handleCloseTag(item)">{{ item }} </el-tag>
...
export default {
  ...
  methods: {
    ...
    handleChange() {
      if (this.form.tags.includes(this.currentTagValue)) {
        this.$message.warning('该标记已存在！')
      } else if (this.form.tags.length >= 10) {
        this.$message.error('最多只能添加 10 个标签')
      } else {
        this.form.tags.push(this.currentTagValue)
      }
