<template>
  <MCard class="PageWrap">
    <MBreadRow :title="['广告管理', '广告设置']" @onClick="onBreadRowClick" />
...
import { adInfoList, getManagerDetail } from '@/api/shop/advertising.js'
export default {
  components: {
    settingDetail,
    list,
  ...
  methods: {
    ...
    // 顶部点击跳转
    onBreadRowClick(row) {
      row === 0 && (this.$router.replace({ name: 'shopAdvertisingList' }))
    },
    // 获取广告位详情数据
    getDetail() {
      const params = {
