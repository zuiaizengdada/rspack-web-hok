...
<script>
import {
  finishVideoCreateUserListApi,
  finishVideoIpListApi,
  finishVideoCpOrgListApi,
  finishSupplierOrgListApi
...
export default {
  ...
  methods: {
    ...
    init() {
      // 合作机构
      finishVideoCpOrgListApi().then(res => {
        this.cpOrgList = res.data || []
      })
      // 分发人
      finishVideoCreateUserListApi().then(res => {
        this.createUserList = res.data || []
      })
      // IP
      finishVideoIpListApi().then(res => {
        this.ipList = res.data || []
      })
      // 供应机构
      finishSupplierOrgListApi().then(res => {
