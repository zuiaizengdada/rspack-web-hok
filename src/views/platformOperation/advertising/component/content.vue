<template>
  <div class="contentForm">
    <!-- <div>123</div> -->
    <el-form ref="ruleForm" size="small" :model="form" :rules="rules" label-width="78px" class="demo-ruleForm">
      <el-form-item label="广告标题" prop="adTitle">
        <el-input v-model.trim="form.adTitle" style="width: 486px" :maxlength="detail.type === 5 ? 5 : 20" show-word-limit />
      </el-form-item>
      <el-form-item prop="url" label-width="0px">
        ...
        <upload
          ...
          :tips="detail.remark"
          :max-length="1"
          oss-teacher-folder="hok_platformAdvertising"
          @change="fileChange"
        />
      </el-form-item>
      <el-form-item label="是否跳转" prop="skip">
        <el-radio-group v-model="form.skip" @click="form.skipText = ''" @input="skipChange">
          <el-radio :label="1">跳转</el-radio>
          <el-radio :label="0">不跳转</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item v-if="form.skip === 1" label="跳转类型" prop="skipType">
        <el-select v-model="form.skipType" placeholder="请选择" class="w-200" @change="skipTypeChange">
          <el-option v-for="(item, key) in jumpTypeOption" :key="item" :label="item" :value="Number(key)" />
        </el-select>
      </el-form-item>
      <el-form-item v-if="form.skip === 1 && form.skipType !== 6 && form.skipType" label="跳转对象" prop="jumpArr">
        <jumpObj ref="jumpObj" :type="form.skipType" :jump-arr="jumpArr" @change="changeSkipText" />
      </el-form-item>
      ...
      <el-form-item label="显示终端" prop="mobileTerminal">
        <el-checkbox
          ...
          :true-label="1"
          :false-label="0"
          @change="$refs.ruleForm && $refs.ruleForm.validateField('mobileTerminal')"
        >移动网页</el-checkbox>
        <el-checkbox
    ...
    <div class="m-t-18">
      <el-button class="contentFootBtn" :loading="loading || myloading" size="small" type="primary" @click="save">保存</el-button>
      <el-button v-if="!ifAdd" class="contentFootBtn delBtn" :loading="loading || myloading" size="small" @click="onhandleRemove">移除</el-button>
    </div>
  </div>
</template>
...
import { adInfoCreate, adInfoUpdate, adInfoDetail, adInfoRemove, platformGoodsType } from '@/api/platform/advertising'
...
const defaultForm = {
  ...
  url: '', // 资源地址
  skip: 1, // 是否跳转 0否 1是
  skipType: '', // 跳转类型 1.课程详情 2.机构分类 3.网页 4.微信小程序 5.抖音小程序 6.推广页 7.专题页 8.平台分类
  skipText: '', // 跳转内容
  goodsType: '', // 课程分类
  displayTime: '', // 开始时间
...
export default {
  ...
  data() {
    ...
    var checkEndTime = (rule, value, callback) => {
      if (this.form.longTerm === 0 && !this.form.fixedTime) {
        callback('请选择结束时间')
      } else if (this.form.longTerm === 0 && this.form.displayTime && this.form.fixedTime < this.form.displayTime) {
    ...
    var checkJumpObj = (rule, value, callback) => {
      if (this.form.skipType === 1 && !this.form.skipText) {
        ...
        callback('请选择机构分类')
      } else if (this.form.skipType === 8 && !this.form.skipText) {
        callback('请选择机平台分类')
    ...
    var checkMobile_terminal = (rule, value, callback) => {
      if (!this.form.mobileTerminal && !this.form.appTerminal && !this.form.douAppleTerminal && !this.form.wxAppleTerminal) {
        callback('至少选择一个显示终端')
      } else {
        callback()
      }
    ...
    return {
      ...
      rules: {
        ...
        jumpArr: [
          { validator: checkJumpObj, required: true, trigger: 'blur' }
        ],
        time: [
      ...
      pickerOptions1: {
        disabledDate: time => {
          if (this.form.displayTime) {
            return time.getTime() < new Date(moment(this.form.displayTime).format('YYYY-MM-DD 00:00:00')).getTime()
          } else {
            return false
          }
        }
      ...
      pickerOptions: {
        disabledDate: time => {
          if (this.form.fixedTime) {
            return time.getTime() > new Date(moment(this.form.fixedTime).format('YYYY-MM-DD')).getTime()
          } else {
            return false
          }
        }
  ...
  created() {
    Promise.all([this.getCatalogueList(), this.getCatalogueList1()])
  },
  methods: {
    beforeUpload(file) {
    ...
    getAdDetail(id) {
      ...
      return adInfoDetail(params).then(res => {
        res.data.fixedTime = res.data.fixedTime ? moment(res.data.fixedTime).format('YYYY-MM-DD HH:mm') : ''
        res.data.displayTime = res.data.displayTime ? moment(res.data.displayTime).format('YYYY-MM-DD HH:mm') : ''
        this.form = {
        ...
        if (this.form.skip === 1 && this.form.skipType) {
          if (this.form.skipType === 3) {
            ...
            if (arr) {
              this.jumpArr[3].value = arr[3]
              this.jumpArr[3].optionValue = arr[1] + arr[2]
            }
            ...
            this.jumpArr[4].value = this.form.skipText
          } else if (this.form.skipType === 8) {
            this.jumpArr[2].value = `${this.form.tenantId}-${this.form.skipText}`
          } else if (this.form.skipType === 2) {
            this.jumpArr[1].value = `${this.form.tenantId}-${this.form.skipText}`
          } else {
        ...
        this.$nextTick(() => {
          if (this.form.skipType && this.form.skipType === 1 && this.jumpArr[0].value && this.jumpArr[0].detail.goodsType) {
            this.$refs.jumpObj && this.$refs.jumpObj.getGoodsDetail(this.jumpArr[0].value)
          }
        })
    ...
    // 跳转对象改变触发
    changeSkipText(obj) {
      const { skipText, goodsType, tenantId } = obj
      this.form.skipText = skipText
      goodsType && (this.form.goodsType = goodsType)
      tenantId && (this.form.tenantId = tenantId)
    },
    // 上传文件对象改变
    fileChange(res) {
    ...
    removeChildrenField(arr) {
      return arr.map(item => {
        // 使用对象解构来复制当前对象，然后删除children字段
        const { children, catalogueId, tenantId, ...rest } = item
        if (Array.isArray(children) && children.length > 0) {
          // 如果有子数组且不为空，递归处理子数组
          rest.children = this.removeChildrenField(children)
        }
        return { ...rest, catalogueId, tenantId, newId: `${tenantId}-${catalogueId}` }
      })
    },
    // 获取机构分类
    getCatalogueList() {
      return platformGoodsType({ type: 2 }).then(res => {
        this.jumpArr[1].option = this.removeChildrenField(res.data)
      })
    },
    // 平台分类
    getCatalogueList1() {
      return platformGoodsType({ type: 1 }).then(res => {
        this.jumpArr[2].option = this.removeChildrenField(res.data)
      })
    },
    // 跳转类型改变触发
    skipTypeChange(res) {
      this.$refs.ruleForm && this.$refs.ruleForm.clearValidate('jumpArr')
      if (res) {
        this.form.skipText = this.jumpArr[res - 1] ? this.jumpArr[res - 1].value : ''
      } else {
        this.form.skipText = ''
      }
    },
    ...
    ifShowTerminal(key) {
      ...
      // 暂时只有H5有专题页
      // if (this.form.skipType === 5 && key !== 'mobileTerminal') {
      //   return true
      // }
      // 网页 小程序不可选
      if (this.form.skipType === 3 && ['douAppleTerminal', 'wxAppleTerminal'].includes(key)) {
        this.form.douAppleTerminal = 0
        this.form.wxAppleTerminal = 0
        return true
      }
      // 开屏 只能选app
      if (this.detail.type === 3 && key !== 'appTerminal') {
        return true
      }
      if (this.defaultForm[key] === 1) {
        ...
        // 不做数量的判断，由接口控制，测试提出
        // return this.detail.maxNum <= this.terminalAdNumber[key] && this.form[key] !== 1
        return false
      }
    },
    // 调整类型改变触发
