    ...
    <div class="editModel overflowOuto">
      <el-form ref="form" :model="form" label-width="92px" :inline="true" size="small" :rules="rules">
        ...
        <el-form-item v-if="form.type === 6" label="分类" label-width="92px" prop="bannerType">
          ...
          <el-input v-model="bannerTypeName" disabled />
        </el-form-item>
...
import { getManagerDetail, managerUpdate, goodTypeNameById } from '@/api/platform/advertising'
export default {
  components: {
    AppDialog
  },
  ...
  mounted() {},
  methods: {
    async getGoodTypeNameById() {
      const res = await goodTypeNameById(this.form.bannerType)
      this.bannerTypeName = res.data.catalogueName
    },
    delNoNumber(e, key) {
    ...
    getDetail() {
      ...
      getManagerDetail(params).then(res => {
        ...
        this.loading = false
        if (this.form.bannerType) {
          this.getGoodTypeNameById()
        }
      }).catch(() => {
        this.loading = false
      })
