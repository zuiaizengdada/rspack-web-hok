...
export default {
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      if (!isLtX) {
        this.$message.error(
          `上传文件大小不能超过 ${this.filterSize(this.detail.fileSize)}!`
        )
      }
      return isLtX
    },
        ...
        .then(res => {
          ...
          if (this.form.skip === 1 && this.form.skipType) {
            if (this.form.skipType === 3) {
              ...
              this.jumpArr[2].value = `${this.form.tenantId || null}-${
                this.form.skipText
              }`
    ...
    onhandleAdd(data) {
      ...
      const params = {
        ...
        pid: { banner: 23, navView: 22 }[this.currentType]
      }
      adInfoCreate(params)
        .then(res => {
          this.myloading = false
