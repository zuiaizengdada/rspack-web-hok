<template>
  <el-dialog
    ...
    :visible.sync="videoDetailFlag"
    :close-on-click-modal="false"
    :before-close="beforeClose"
  >
    <div slot="title">
      <p class="p-title">{{ detailObject.name }}</p>
      <span class="title-mess">上传者：<em>{{ detailObject.uploadUserName }}</em></span>
      <span class="title-mess">上传时间：<em>{{ detailObject.uploadTime }}</em></span>
    </div>
    <div class="video-detail-cont">
      <div class="vdc-left">
        <span>视频ID:<em>{{ detailObject.id }}</em></span>
        ...
        <span>视频类型:<em>{{ detailObject.videoType || '--' }}</em></span>
        <span>IP:<em>{{ detailObject.teacherName || '--' }}</em></span>
        <!-- <span>是否在保护周期内:<em>{{ detailObject.isWithProtectPeriod?'是':'否' }}</em></span> -->
        <span class="kfqx-cls">开放权限:<em :title="detailObject.sharedUserName">{{ detailObject.sharedUserName?detailObject.sharedUserName:'不开放' }}</em></span>
        <span><p style="float: left;">字幕处理:</p><em style="width: 200px; display: inline-block;"><StatusShow :subtitlestatus="detailObject.subtitleStatus" :videotype="3" :semi-video-id="semiVideoId" @reloadDatasg="loadData" /></em></span>
        <span><p style="float: left;">敏感词识别结果:</p>
          <em style="width: 228px; display: inline-block;">
            <mgcStatusShowVue :sensitivewordstatus="detailObject.sensitiveWordStatus" :videotype="3" :refvideoid="semiVideoId" @reloadDatasg="loadData" />
      ...
      <div class="vdc-right">
        <div v-show="videoDetailFlag" id="videoDetailPlay" class="video_play" />
        <div class="zimu_ls">
          <span>{{ nowWzData }}</span>
        </div>
      </div>
...
// import mgcStatusShowVue from './mgcStatusShow.vue'
import { getPlayDuration } from '@/utils/index'
import { urlVodExportZip } from '@/utils/zip'
import { getVideoDetailData } from '@/api/videoManagement/vedioSetting'
...
export default {
  ...
  methods: {
    ...
    loadData() {
      getVideoDetailData(this.semiVideoId).then(res => {
        if (res.success) {
    ...
    show(row) {
      this.semiVideoId = row.semiVideoId
      getVideoDetailData(row.semiVideoId).then(res => {
        if (res.success) {
          this.detailObject = res.data
          this.detailObject.sensitiveString = ''
          this.detailObject.sensitives.some(item => {
          ...
          this.detailObject.sensitiveString = this.detailObject.sensitiveString.substring(0, (this.detailObject.sensitiveString.length - 1))
          this.play(this.detailObject)
          this.videoDetailFlag = true
        }
      })
    },
    beforeClose() {
      this.destroy()
      this.videoDetailFlag = false
    },
