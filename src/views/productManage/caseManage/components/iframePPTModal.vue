...
export default {
  ...
  data() {
    return {
      toPPTUrl: process.env.VUE_APP_PPTSITE_URL,
      pptData: '',
      isShowIframe: false,
      iframeWin: '',
      oss: {}
  ...
  mounted() {
    ...
    window.addEventListener('message', (event) => {
      if (event.origin !== this.toPPTUrl) return
  ...
  methods: {
    ...
    async open({ data, type, titleName, userForppt }) {
      ...
      const params = {
        ...
        data,
        titleName,
        userForppt: JSON.stringify(userForppt),
        oss: JSON.stringify(this.oss)
      }
      const that = this
      setTimeout(function() {
        const iframeWin = that.$refs.iframeId.contentWindow
        iframeWin.postMessage(params, that.toPPTUrl)
      }, 1000)
    },
    inserts() {
      this.isShowIframe = true
      ...
      this.iframeWin.postMessage(params, this.toPPTUrl)
    },
    toEdit() {
      this.isShowIframe = true
      const data = [
