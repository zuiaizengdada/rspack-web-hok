  ...
  <MCard class="page_wrap">
    ...
    >
        ...
        <div class="m-t-20 p-b-20 searchTop">
          <el-form ref="form" :model="search" label-width="auto" size="small" :inline="true">
            <el-form-item label="备课内容名称" class="m-r-0">
              <el-input v-model="search.coursewareName" clearable placeholder="请输入" class="w-200" @keyup.enter.native="searchFn" @clear="searchFn" />
            </el-form-item>
            ...
            <el-form-item label="创建人" class="m-r-0">
              <el-select v-model="search.createUserId" class="w-200" filterable placeholder="请选择创建人" clearable @change="searchFn">
                <el-option v-for="item in userArray" :key="item.userId" :label="item.nickName" :value="item.userId" />
              </el-select>
            </el-form-item>
            <el-form-item class="m-r-0">
              <el-button type="primary" class="m-l-20" @click="searchFn">查询</el-button>
              <el-button @click="onClear">重置</el-button>
            </el-form-item>
          </el-form>
          <el-button size="small" type="primary" @click="onhandleAdd"><i class="el-icon-plus" />新增备课</el-button>
        </div>
      </template>
      <template slot="action" slot-scope="{ row }">
        ...
        >预览</el-button> -->
        <el-button type="text" style="margin-right: 10px; margin-left: 0px" :disabled="canEdit(row)" @click="onhandleConfiguration(row)">配置</el-button>
        <el-button type="text" style="margin-right: 10px; margin-left: 0px" @click="onhandleCopy(row)">复制</el-button>
        <el-button type="text" style="margin-right: 10px; margin-left: 0px" :disabled="userInfo.userId !== row.createUserId" @click="onhandleEdit(row)">编辑</el-button>
        <el-button type="text" style="margin-right: 10px; margin-left: 0px" class="c_F53F3F" :disabled="userInfo.userId !== row.createUserId" @click="onhandleDel(row)">删除</el-button>
      </template>
      <template slot="status" slot-scope="{ row }">
        <div class="statusView">
          ...
          <el-switch v-model="row.status" v-loading="row.editLoading" :disabled="!(enterLiveRoom || !(userInfo.userId !== row.createUserId))" element-loading-spinner="el-icon-loading" :active-value="1" :inactive-value="2" @change="onhandleStatusChange(row)" />
        </div>
      </template>
    </MPageLayout>
    <addDialog :config="addConfig" :create-user-info="createUserInfo" @success="clsoeAddDialog" />
  </MCard>
</template>
  ...
  methods: {
    canEdit(row) {
      ...
      if (row.operateWhiteList && row.operateWhiteList.length > 0) {
        const index = row.operateWhiteList.findIndex(v => v.userId === this.userInfo.userId)
        if (index > -1) {
          return false
        }
      }
    ...
    async onhandleConfiguration(row) {
      ...
      const { href } = this.$router.resolve({
        path: `/live/prepareLessons/${row.coursewareId}`
      })
