    ...
    <div class="addDialog overflowOuto">
      <el-form ref="form" v-loading="loading" :model="form" label-width="auto" size="small" :rules="rules" @submit.native.prevent>
        ...
        <el-form-item label="用户身份" prop="userIdentity">
          {{ {1: '学员', 2: '助教', 3: '讲师'}[form.userIdentity] }}
        </el-form-item>
...
import { editRobots } from '@/api/liveRoom/robot'
  ...
  methods: {
    init() {
      ...
      this.form = {
        ...
        userName: this.config.row.robotNickName,
        userIdentity: this.config.row.userIdentity,
        avatar: this.config.row.robotHeadPic || ''
      }
      if (this.$refs.MUpload && this.form.avatar) {
