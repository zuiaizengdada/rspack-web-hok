<template>
  <div class="userList" :class="{current: item.memberId === chatUser.userId}" @dblclick="onhandleDblclick({ item, message: item.message, type: 1 })">
    <div class="user_avatar" @click.stop="onhandleClick">
      <el-badge :value="item.message.unreadCount <= 99 ? item.message.unreadCount : '99+'" class="item" :hidden="!item.message.unreadCount">
        <AppAvatar :avatar="item.message.userProfile.avatar || item.memberPic" :class="{leave: !item.isOnline}" :name="item.memberName || item.message.userProfile.nick || item.message.userProfile.userID" :width="48" class="m-r-8" :no-name="true" />
      </el-badge>
      <img v-if="item.isV" src="~@/assets/image/liveRoom/vip_icon.png" alt="" class="tips_vip userStateIcon" />
    </div>

    <div class="user_info_box" @click="onhandleDblclick({ item, message: item.message, type: 1 })">
      <div class="user_info" @contextmenu.prevent.stop="openMenu($event,item)">
        <div class="user_info_nick">
          <textShowMore :line-clamp="1" :text="renderUserName(item)" :style="item.phoneNumber ? 'max-width : 80px' : 'flex: 1 0 0'" />
          <template v-if="item.phoneNumber">
            <!-- 手机号 -->
            <div class="user_info_userPhone">
              <template v-if="isHead() || !item.sysUserName || item.sysUserId === userInfo.userId">
                <watchPhoneNumber :phone-area-code="item.phoneAreaCode" :default-value="item.phoneNumber" :params="item" :user-id="item.memberId" :get-api="operationLogsWrite" :copy="true" />
              </template>
              <template v-else>
                <template v-if="item.phoneAreaCode">+{{ item.phoneAreaCode }}</template>
                {{ item.phoneNumber }}
              </template>
            </div>
          </template>
          <template v-else-if="item.email">
            ...
            <div class="user_info_userEmail">
              ...
              <template v-if="isHead() || !item.sysUserName || item.sysUserId === userInfo.userId">
                <watchPhoneNumber :default-value="item.email" :params="item" :user-id="item.memberId" :get-api="operationLogsWriteEmail" />
              </template>
              <template v-else>
                {{ item.email }}
              </template>
          ...
          <!-- 右侧播放时间 -->
          <el-popover :value="popoverShow" placement="right" trigger="manual" width="506" popper-class="p-0 border_none ov-h" @show="show = true" @hide="show = false">
            <liveLog :user-id="item.memberId" :show="show" @close="popoverLock = false" @open="popoverLock = true" />
            <div slot="reference" class="user_liveinfo" @click.stop="popoverShowFn">
              ...
              <div class="user_liveinfo_time">
                <img src="~@/assets/image/liveRoom/xuexicishu.png" class="renshutiaozhengIcon" />
                {{ item.listenDuration | filterformLabel }}
              </div>
            </div>
          </el-popover>
        </div>
        <div class="user_info_inviter"><span>邀请人：</span><span>{{ item.sysUserName || '暂无' }}</span></div>
        <div class="user_info_tips">
          <!-- tag标签 -->
          <div class="user_info_markers">
            <div v-if="item.speechTimes" class="user_info_markers_border mark_b_E0F9FF">发言 x{{ item.speechTimes }}</div>
            <div v-if="item.receiveCouponNum" class="user_info_markers_border mark_b_FFEEDD">领券 x{{ item.receiveCouponNum }}</div>
            <div v-if="item.waitOrderCount" class="user_info_markers_border mark_b_FFE4E0">待支付 x{{ item.waitOrderCount }}</div>
            <div v-if="item.goodsClickNum" class="user_info_markers_border mark_b_DFF6EF">查看商品 x{{ item.goodsClickNum }}</div>
          </div>
          <!-- 拨打 -->
          <div class="outbound">
            <outbound :item="item" :can-call="item.sysUserId === userInfo.userId" />
          </div>
        </div>
      </div>
    </div>
      ...
      contextmenu.open({
        ...
        onClickFn: (el, res) => {
          ...
          if (el === '发私信') {
            ...
            console.log('用户解除了禁言')
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 2, traineeId: item.memberId }).then(() => { this.item.isShutUp = false; this.$emit('refresh') })
          } else if (el === '禁言') {
            console.log('禁言')
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 1, traineeId: item.memberId }).then(() => { this.item.isShutUp = true; this.$emit('refresh') })
          } else if (el === '拉黑') {
            console.log('拉黑')
            ...
            console.log('自嗨')
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 5, traineeId: item.memberId }).then(() => { this.item.isAmuse = true; this.$emit('refresh') })
          } else if (el === '解除自嗨') {
            console.log('解除自嗨')
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 6, traineeId: item.memberId }).then(() => { this.item.isAmuse = false; this.$emit('refresh') })
          } else if (el === '标记颜色') {
