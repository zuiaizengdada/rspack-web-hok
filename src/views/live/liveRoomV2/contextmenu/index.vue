      ...
      <div v-for="(el, i) in item" :key="i" class="contextmenuLi">
        ...
        <li v-else @click="onhandleClick(el)">
          ...
          <div v-else class="iconMenuText">
            <svg-icon v-if="el.icon" :icon-class="el.icon" class-name="icon" />
...
<script>
import { markColor } from '@/views/live/columns.js'
export default {
  components: {},
  data() {
    return {
...
.contextmenu {
  ...
  line-height: 20px;
  width: 120px;
  box-shadow: 1px 1px 2px 0 #525a660a, 2px 2px 8px 0 #525a6614;
  border-radius: 6px;
