    ...
    <template v-else-if="!item.isMine">
      ...
      <div class="chatConten m-l-10">
        <div v-if="chatUserType === 2" class="chatUserInfo">
          ...
          <div class="userName text_hidden_ellipsis" @contextmenu.prevent.stop="openMenu($event,item)">
...
<script>
import chatComment from '@/views/live/liveRoomV2/p3/chatRoom/chatComment'
    ...
    'leftList': {
      handler(val) {
        ...
        if (this.cloudCustomDataParse && this.cloudCustomDataParse.messageReply) {
          this.replayMessage = this.messageData.find(v => v.message.ID === this.cloudCustomDataParse.messageReply.messageID)
        } else {
      ...
      contextmenu.open({
        ...
        onClickFn: (el, res) => {
          if (el.text === '复制') {
            ...
            console.log('这里触发了', this.onfocus)
            this.onfocus()
          } else if (el.text === '删除') {
            console.log('删除', item)
            if (this.$store.state.im.chatUser.type !== 2) {
