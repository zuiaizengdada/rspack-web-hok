...
export default {
  ...
  computed: {
    ...
    ...mapState({
      'ifCanScrollBottom': state => state.im.privateChatView.ifCanScrollBottom
    })
  },
  mounted() {
  ...
  methods: {
    ...
    scrollEl(el) {
      const $scrollWrapper = this.scrollWrapper
      $scrollWrapper.scrollTop = el.offsetTop - $scrollWrapper.offsetTop
    },
    scrollBottom() {
      setTimeout(() => {
        // console.log('能否滚动到底部', this.ifCanScrollBottom)
        if (this.ifCanScrollBottom) {
          var wrap = this.$refs.scrollContainer.$refs.wrap
