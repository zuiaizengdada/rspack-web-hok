    ...
    <template v-if="type === 'text'">
      ...
      <div v-else class="message-container">
        <el-image
          ...
          class="message-image"
        />
        <span class="p-10 chatComment_text" v-html="filterContent(content.text)" />
      </div>
    </template>
    ...
    filterContent(value) {
      if (!value) {
        ...
        if (!tmp) {
          ...
          const imgMatch = value.match(imgReg)

          if (imgMatch) {
            // 获取图片位置
            const imgIndex = value.indexOf(imgMatch[0])

            // 分割图片前后的文字
            const beforeText = value.substring(0, imgIndex).trim()
            const afterText = value.substring(imgIndex + imgMatch[0].length).trim()
        ...
        if (imgMatch) {
          ...
          // 根据文字位置组织布局 flex-direction: column;
          let result = '<div style="gap: 8px;">'
