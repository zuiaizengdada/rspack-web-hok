...
export default {
  ...
  methods: {
    ...
    openMenu(event, item) {
      ...
      //   ]
      //   markHeight = 184
      // } else if (item.isShutUp) {
      //   tools = [
      //     ['私聊Ta', '修改备注'],
      //     [`${item.isShutUp ? '解除禁言' : '禁言'}`, '拉黑并踢出'],
      //     ['标记颜色']
      //   ]
      //   markHeight = 268
      // } else {
      //   tools = [
      //     ['私聊Ta', '修改备注'],
      //     [`${item.isShutUp ? '解除禁言' : '禁言'}`, '拉黑并踢出', `${item.isAmuse ? '解除自嗨' : '自嗨'}`],
      //     ['标记颜色']
      //   ]
      //   markHeight = 310
      // }
        ...
        onClickFn: (el, res) => {
          ...
          if (el === '私聊Ta') {
            ...
            Dialog.open({
              ...
              success: (val) => {
                this.$store.dispatch('im/updateUserRemarks', { remarksInfo: val, traineeId: item.memberId }).then(() => { this.$emit('refresh') })
              }
            })
          } else if (el === '解除禁言') {
            console.log('用户解除了禁言')
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 2, traineeId: item.memberId }).then(() => { this.$emit('refresh') })
          } else if (el === '禁言') {
            console.log('禁言')
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 1, traineeId: item.memberId }).then(() => { this.$emit('refresh') })
          } else if (el === '拉黑并踢出') {
            console.log('拉黑并踢出')
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 3, traineeId: item.memberId }).then(() => { this.$emit('refresh') })
          } else if (el === '自嗨') {
            console.log('自嗨')
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 5, traineeId: item.memberId }).then(() => { this.$emit('refresh') })
          } else if (el === '解除自嗨') {
            console.log('解除自嗨')
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 6, traineeId: item.memberId }).then(() => { this.$emit('refresh') })
          } else if (el === '标记颜色') {
            this.$store.dispatch('im/markUserColor', { color: item.colorMark === res.value ? 0 : res.value, traineeId: item.memberId }).then(() => { this.$emit('refresh') })
          } else if (el === '解除拉黑') {
            this.$store.dispatch('im/operateUserTrainee', { groupId: this.groupID, operateType: 4, traineeId: item.memberId }).then(() => { this.$emit('refresh') })
          }
        }
      })
    },
    ...
    renderUserName(item) {
      ...
      if (!obj || !obj.traineeRemark) {
        ...
        return filtersTextPhone(obj.traineeRemark || '')
      }
    },
