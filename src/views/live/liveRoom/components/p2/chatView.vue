    ...
    <template v-else-if="!item.isMine">
      <div class="avatar notIsming">
        ...
        <div v-if="cloudCustomDataParse.identity && cloudCustomDataParse.identity.includes('VIP')" class="tips_vip">
          <img src="~@/assets/image/liveRoom/vip.png" alt="" class="toolsIcon" />
        </div>
      </div>
      <div class="chatConten m-l-10">
        <div v-if="chatUserType === 2" class="chatUserInfo">
          ...
          >
            ...
            {{ item.traineeRemark || filtersTextPhone(item.memberName) || filtersTextPhone(item.message.from) }}
          </div>
        ...
        <div class="comment">
          ...
          <div v-if="cloudCustomDataParse && cloudCustomDataParse.isAmuse" class="msgIsAmuse">
            <img src="~@/assets/image/liveRoom/zihai.png" alt="" class="msgIsAmuseIcon" />
            <span>自嗨</span>
          </div>
        </div>
    ...
    openMenu(event, item) {
      ...
      getLiveRoomMemberInfo(data).then((res) => {
        ...
        if (this.userInfo.isBlackList) {
          ...
          const arr = [`${this.userInfo.isShutUp ? '解除禁言' : '禁言'}`, '拉黑并踢出']
          !this.userInfo.isShutUp && arr.push(`${this.userInfo.isAmuse ? '解除自嗨' : '自嗨'}`)
          tools.unshift(arr)
          markHeight += 53 + 42 + 42
        }
        if (this.userInfo.isBlackList) {
          tools.unshift(['查看资料', '修改备注'])
          markHeight += 53 + 42
        } else if (!this.userInfo.isBlackList) {
          tools.unshift(['查看资料', '私聊Ta', '修改备注'])
          markHeight += 53 + 42 + 42
        } else {
          tools.unshift(['查看资料', '私聊Ta'])
          markHeight += 53 + 42
        }
