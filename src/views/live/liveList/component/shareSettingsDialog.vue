<template>
  <!-- 分享设置弹窗 -->
  <el-dialog custom-class="dialog-body" :visible.sync="config.visible" width="866px" append-to-body :show-close="true" title="分享设置" class="share-settings-dialog">
    <div id="posterDialog" class="dialog-body-content flex">
      <div class="shareFormLeft">
...
export default {
  ...
  methods: {
    ...
    getDetail() {
      this.loading = true
      const data = {
        liveRoomId: this.$route.params.id || this.config?.liveRoomId
      }
      getH5shareSetting(data).then(res => {
        console.log(res, '分享设置')
        this.form = {
