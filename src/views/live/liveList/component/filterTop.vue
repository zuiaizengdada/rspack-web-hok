  ...
  <div class="filter-module">
    ...
    <el-form ref="form" :model="search" label-width="auto" size="small" :inline="true">
      ...
      <el-row>
        ...
        <el-form-item label="直播状态">
          <el-select v-model="search.status" class="w-200" placeholder="请选择" style="width: 200px" clearable @change="handleClickSearch">
            <el-option v-for="(item, key) in liveStateOption.filter(v => v.value !== '-1')" :key="key" :value="item.value" :label="item.label" />
          </el-select>
        </el-form-item>
      ...
      <el-row class="m-b-20">
        ...
        <el-button v-permission="['web:liveRoomlist:addLiveRoom', permsList]" size="small" class="fr" type="primary" @click="$emit('onhandleAdd')">新增直播间</el-button>
      </el-row>
    </el-form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleClickCreateLiveRoom() {
      this.$router.push('/createNewLiveRoom')
    }
  }
}
</script>
