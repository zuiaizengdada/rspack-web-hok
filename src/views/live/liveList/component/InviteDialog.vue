<template>
  ...
  >
    <div v-loading="loading" class="InviteDialog overflowOuto">
      <el-form ref="ruleForm" :model="form" size="small" label-width="100px" class="demo-ruleForm">
        <el-tabs v-model="activeName" type="border-card">
          <el-tab-pane v-if="havBtnOld('web:liveRoom:linkH5')" label="H5网页" name="1">
            <el-form-item label="直播模式：" prop="mode">{{ form.liveModel | getOptionsValue(liveModeOption) }}</el-form-item>
            <el-form-item label="直播名称：" prop="mode">{{ form.name }}</el-form-item>
            <el-form-item label="开播时间：" prop="mode">{{ moment(form.startTime).format('YYYY-MM-DD HH:mm:ss') }}</el-form-item>
            <el-form-item label="邀请人：" prop="mode">{{ userInfo.nickName }}</el-form-item>
            <template v-if="!ifLink">
              <el-form-item label="域名地址：" prop="domainUrl">
                <el-select v-model="form.domainUrl" placeholder="请选择">
                  <el-option v-for="(item, index) in urlList" :key="index" :label="item.label" :value="item.value" />
                </el-select>
                <i v-if="refreshLoading" class="el-icon-loading m-l-10" />
                <el-tooltip v-else class="item" effect="dark" content="获取最新的域名地址" placement="top">
                  <i class="el-icon-refresh-right m-l-10 pointer" style="color: #0C6FFF" @click="getLiveRoomOrgConfig" />
                </el-tooltip>
              </el-form-item>
            </template>
            <el-form-item v-if="ifLink" label="生成链接：" prop="link">
              <el-input v-model="form.link" style="width: 300px" disabled />
              <el-tooltip class="item" effect="dark" content="点击复制" placement="top-start">
                <i style="height: 23px;line-height: 25px;" class="el-icon-document-copy a_link m-l-10" @click="copyText(form.link)" />
              </el-tooltip>
            </el-form-item>
            <el-form-item v-if="showShortLinkInviteUrl" label="短链接：" prop="shortLinkInviteUrl">
              <el-input v-model="form.shortLinkInviteUrl" style="width: 300px" disabled />
              <el-tooltip class="item" effect="dark" content="点击复制" placement="top-start">
                <i style="height: 23px;line-height: 25px;" class="el-icon-document-copy a_link m-l-10" @click="copyText(form.shortLinkInviteUrl)" />
              </el-tooltip>
            </el-form-item>
            <!-- // 如果点的老的就用长链接 -->
            ...
            <template v-else>
              ...
              <el-form-item v-if="ifLink" label="H5二维码：">
                <span class="liveLinkCode">
                  <AppQrcode :src="form.shortLinkInviteUrl" :size="200" :margin="10" />
                </span>
              </el-form-item>
            </template>
          </el-tab-pane>

          <el-tab-pane v-if="havBtnOld('web:liveRoom:linkWechat')" label="微信小程序" name="2">
            <el-form-item label="直播模式：" prop="mode">{{ form.liveModel | getOptionsValue(liveModeOption) }}</el-form-item>
            <el-form-item label="直播名称：" prop="mode">{{ form.name }}</el-form-item>
            <el-form-item label="开播时间：" prop="mode">{{ moment(form.startTime).format('YYYY-MM-DD HH:mm:ss') }}</el-form-item>
            <el-form-item label="邀请人：" prop="mode">{{ userInfo.nickName }}</el-form-item>
            ...
            <el-form-item v-if="ifLink" label="小程序链接：" prop="miniAppUrl">
              <el-input v-model="form.miniAppUrl" style="width: 300px" disabled />
              <el-tooltip class="item" effect="dark" content="点击复制" placement="top-start">
                <i style="height: 23px;line-height: 25px;" class="el-icon-document-copy a_link m-l-10" @click="copyText(form.miniAppUrl)" />
              </el-tooltip>
            </el-form-item>
          ...
          <el-tab-pane v-if="havBtnOld('web:liveRoom:linkApp')" label="获客APP" name="3">
            <el-form-item label="直播模式：" prop="mode">{{ form.liveModel | getOptionsValue(liveModeOption) }}</el-form-item>
            <el-form-item label="直播名称：" prop="mode">{{ form.name }}</el-form-item>
            <el-form-item label="开播时间：" prop="mode">{{ moment(form.startTime).format('YYYY-MM-DD HH:mm:ss') }}</el-form-item>
            <el-form-item label="邀请人：" prop="mode">{{ userInfo.nickName }}</el-form-item>
            ...
            <el-form-item v-if="ifLink" label="APP链接：" prop="hokAppUrl">
              <el-input v-model="form.hokAppUrl" style="width: 300px" disabled />
              <el-tooltip class="item" effect="dark" content="点击复制" placement="top-start">
                <i style="height: 23px;line-height: 25px;" class="el-icon-document-copy a_link m-l-10" @click="copyText(form.hokAppUrl)" />
              </el-tooltip>
            </el-form-item>
          ...
          <el-tab-pane v-if="havBtnOld('web:liveRoom:linkDou')" label="抖音小程序" name="4">
            <el-form-item label="直播模式：" prop="mode">{{ form.liveModel | getOptionsValue(liveModeOption) }}</el-form-item>
            <el-form-item label="直播名称：" prop="mode">{{ form.name }}</el-form-item>
            <el-form-item label="开播时间：" prop="mode">{{ moment(form.startTime).format('YYYY-MM-DD HH:mm:ss') }}</el-form-item>
            <el-form-item label="邀请人：" prop="mode">{{ userInfo.nickName }}</el-form-item>
            ...
            <el-form-item v-if="ifLink" label="小程序链接：" prop="dyMiniAppUrl">
              <el-input v-model="form.dyMiniAppUrl" style="width: 300px" disabled />
              <el-tooltip class="item" effect="dark" content="点击复制" placement="top-start">
                <i style="height: 23px;line-height: 25px;" class="el-icon-document-copy a_link m-l-10" @click="copyText(form.dyMiniAppUrl)" />
              </el-tooltip>
            </el-form-item>
    ...
    <div slot="footer" class="ss-material-box-bottom">
      <div>
        <el-button v-if="ifLink" type="text" @click="onhandlePosterSetting">海报设置</el-button>
        <el-button type="text" @click="onClickShareLink">分享设置</el-button>
      </div>

      <div>
        <el-button v-if="showShortLinkInviteUrl" type="primary" size="small" class="m-l-20" :loading="loading" @click="copyUrl">复制链接</el-button>
        <el-button v-if="ifLink" size="small" :loading="loading" @click="onhandleCreatePosters">生成海报</el-button>
        <template v-else>
          <el-button v-if="havBtnOld('web:liveRoom:inviteNew')" size="small" type="primary" :loading="loading" @click="generateLink">生成链接</el-button>
          <el-button v-if="havBtnOld('web:liveRoom:inviteOld')" size="small" type="primary" :loading="loading" @click="generateLinkOld">生成链接（旧）</el-button>
        </template>
        <el-button size="small" @click="$emit('close')">取消</el-button>
      </div>
    </div>
    <posterDialog :config="posterConfig" :src="posterConfig.src" :nick-name="userInfo.nickName" :star-time="form.startTime" :live-room-name="form.name" />
...
export default {
  ...
  watch: {
    'config.visible': {
      handler(val) {
        ...
        if (val) {
          this.form = { ...JSON.parse(JSON.stringify(this.config.row)), shortLinkInviteUrl: '', domainUrl: '' }
  ...
  methods: {
    ...
    generateLinkOld() {
      ...
      buildInviteUrlAjax({ liveRoomId }).then(res => {
        if (res.code === 1) {
    ...
    generateLink() {
      ...
      buildInviteUrlNew(data).then(res => {
        if (res.code === 1) {
          if (typeof res.data === 'string') {
            this.form.link = res.data
            this.ifLink = true
    ...
    getLiveRoomOrgConfig() {
      ...
      getAllMainConfigByLiveRoomId(data)
        .then(res => {
          let domainUrl = ''
    ...
    havBtnOld(key) {
      ...
      if (obj && obj.children && obj.children.length > 0) {
        ...
        if (o && o.children && o.children.length > 0) {
          const p = o.children.find(v => v.path === '/live/liveRoom')
          // console.log(p, 'p')
