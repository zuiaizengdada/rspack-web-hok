  ...
  <div class="registerMobile-page">
    <div class="liveRoomStartScale">
      <div class="page_wrap overflowOuto">
        ...
        <!-- 提示层 -->
        <div v-if="!(liveInfo.status === 0 && timeShow.times < 0)" class="registerMobile-tips">
          ...
          <template v-else-if="[1,2,3].includes(liveInfo.status)">
            <div class="registerMobile-tips-liveing" :class="`liveRoomLiveState${liveInfo.status}`">
              <svg-icon class-name="liveTypeicon" :icon-class="{0: 'livenostart', 1: 'liveIng', 2: 'liveSuspend',3: 'liveError', 9: 'audit_info'}[liveInfo.status]" />
              <span style="margin-left: 12px">{{ liveInfo.status | getOptionsValue(liveStateOption) }}</span>
            </div>
          </template>
          <template v-else-if="[9].includes(liveInfo.status)">
            <div class="registerMobile-tips-liveEnd">本场直播已结束 请联系班主任学习下一场</div>
          </template>
        </div>
        <!-- 标题栏 -->
        <div class="registerMobile-title">
          ...
          <div class="registerMobile-startTime">
            <template v-if="liveInfo.status !== 9">
              开播时间
              <span class="m-l-10">{{ liveInfo.startTime ? moment(liveInfo.startTime).format('YYYY-MM-DD HH:mm') : '' }}</span>
            </template>
            <template v-else>
              <svg-icon class-name="size-icon" icon-class="time_grey" />
              <span style="margin-left: 5px">{{ liveInfo.endTime ? moment(liveInfo.endTime).format('YYYY-MM-DD HH:mm') : '' }}</span>
        ...
        <div class="registerMobile-main">
          <template v-if="imgList.length === 0">
            <img src="~@/assets/image/live/registerSettingsNodata.png" alt="" style="width: 245px;margin: auto;">
          </template>
          <template v-else>
      ...
      <div v-if="liveInfo.status !== 9" class="registerMobile-footer">
        <div class="footer-btn">{{ liveInfo.status === 0 ? '立即报名' : '观看直播' }}</div>
      </div>
    </div>
    <!-- <div class="registerMobile-main">
      <img v-for="(item,index) in curImgList" :key="index" width="100%" :src="item">
  ...
  watch: {
    'liveInfo.startTime'(val) {
      if (val) {
        ...
        this.liveInfo.status === 0 && (this.timer = setInterval(() => {
