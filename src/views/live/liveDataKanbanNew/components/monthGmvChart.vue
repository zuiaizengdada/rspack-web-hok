<template>
  <div class="revenue-chart">
    <div id="monthGmvChart" class="revenue-chart-im" />
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      getCMVChart(params).then(res => {
        if (res.code === 1 && res.data) {
          ...
          res.data.some(item => {
            ...
            // this.xArray.push(item.userName)
            // this.yDataArray.push(item.gmv ? this.filterPrice(item.gmv) : 0)
            this.yDataArray.push(item.gmv ? item.gmv / 100 : 0)
          })
        }
        this.loadData()
      })
    ...
    loadData: function() {
      this.chart = echarts.init(document.getElementById('monthGmvChart'))
        ...
        dataZoom: [
          {
            ...
            end: (!this.xArray?.length || this.xArray?.length < 12) ? 100 :Math.floor((11 / this.xArray?.length) * 100)-1,  // 可以通过滑动来改变这个选择范围
            zoomLock: true, // 禁用缩放
            textStyle: false,
            // // 设置选中范围的填充颜色
