...
import { getGmvRankingBoard } from '@/api/liveRoom/kanban.js'
...
export default {
  ...
  methods: {
    ...
    init(data) {
      ...
      getGmvRankingBoard(params).then(res => {
        this.xAxisData = []
        this.xTearcherArray = []
        this.arrayString = []
        this.yArray = []
        if (res.code === 1 && res.data) {
          res.data.some(item => {
            ...
            this.yArray.push(
              // item.gmvAmount ? this.filterPrice(item.gmvAmount) : 0
              item.gmvAmount ? item.gmvAmount / 100 : 0
            )
          })
        }
        ...
        dataZoom: [
          {
            ...
            end: (!this.xAxisData?.length || this.xAxisData?.length < 12) ? 100 :  Math.floor((11/ this.xAxisData?.length) * 100)-1, // 可以通过滑动来改变这个选择范围
            zoomLock: true,   // 禁用缩放
            textStyle: false,
