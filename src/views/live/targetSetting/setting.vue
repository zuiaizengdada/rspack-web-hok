<template>
  <MCard class="page_wrap">
    ...
    <div class="filterHeader">
      <div class="fl">
        ...
        <el-cascader
          ...
          @change="searchFn"
        />
        <el-button size="small" type="primary" @click="onhanldeAdd">新增</el-button>
      </div>
    </div>
    ...
    >
      ...
      <el-table-column prop="deptName" label="所在部门" min-width="120" align="center" />
      <el-table-column prop="userName" label="员工" min-width="120" align="center" />
      <el-table-column prop="targetValue" label="目标值" min-width="120" align="center">
        ...
        <template slot-scope="{ row }">
          ...
          <el-input v-else v-model="row.targetValue" size="small" @input="(e) => delNumber(e, row, 'targetValue')" />
        </template>
      </el-table-column>
      <el-table-column prop="updateUserName" label="更新人" min-width="120" align="center" />
      <el-table-column prop="updateTime" label="更新时间" min-width="120" align="center" />
      <el-table-column label="操作" min-width="120" align="center">
        <template slot-scope="{ row }">
          <el-button v-if="currentMothYear <= search.mothYear" type="text" class="c_F53F3F" @click="onhandleDel(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="m-t-20">
      <el-button v-if="!ifEdit" :disabled="currentMothYear > search.mothYear" size="small" type="primary" @click="onhandleEdit">编辑</el-button>
      <el-button v-else size="small" type="primary" @click="onhandleSave">保存</el-button>
    </div>
  </MCard>
</template>
...
import { getTargetSetupList, delTargetSetup, saveTargetSetup } from '@/api/liveRoom/target.js'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getTargetSetupList(data).then(res => {
