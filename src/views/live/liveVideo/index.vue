<template>
  <MCard class="page_wrap">
    ...
    >
      ...
      <template slot="videoSource" slot-scope="{ row }">
        <span> {{ $getdictName('VIDEO_SOURCE', row.videoSource) }}</span>
      </template>
      <template slot="duration" slot-scope="{ row }">
        <span>{{ secondIsLiveDuration(row.duration) }}</span>
      </template>
      <template slot="covertStatus" slot-scope="{ row }">
        <span v-if="row.covertStatus === 1" style="color: #ff7d00">
          <el-progress :percentage="row.progress" :format="row.progress" style="width: 100px" color="#ff7d00" text-color="#ff7d00" />
        </span>
      ...
      <template slot="status" slot-scope="{ row }">
        <div class="statusBox">
          <p :style="`background:${row.status == 1 ? '#00B42A' : '#F53F3F'}`" />
          <span> {{ $getdictName('VIDEO_TYPE_SOURCE', row.status) }}</span>
          <el-switch
            v-model="row.status"
            :disabled="userInfo.userId!==row.createUserId || row.covertStatus !== 2"
            active-color="#0C6FFF"
            :active-value="1"
            :inactive-value="2"
      ...
      <template slot="teacherName" slot-scope="{ row }">
        <div class="text_hidden_1">
          {{ row.teacherName || '——' }}
        </div>
      </template>
      <template slot="action" slot-scope="{ row }">
        <el-button type="text" :disabled="userInfo.userId!==row.createUserId" @click="onhandleEdit(row)">编辑</el-button>
        <el-button type="text" :disabled="userInfo.userId!==row.createUserId" @click="onhandleDel(row)">删除</el-button>
        <el-button type="text" @click="onhandleDetail(row)">详情</el-button>
      </template>
    </MPageLayout> -->
    <!-- <columns-customer /> -->
    <filterTop
    ...
    >
      <!-- <el-table-column label="视频课件ID" prop="videoId" width="200px" /> -->
      <el-table-column
        label="视频ID"
        prop="videoId"
        width="200"
      ...
      <el-table-column label="视频分辨率" prop="code" width="150">
        <template slot-scope="{ row }">
      ...
      <el-table-column label="视频时长" prop="duration" width="150">
        <template slot-scope="{ row }">
      ...
      <el-table-column label="视频转码状态" prop="covertStatus" width="150">
        <template slot-scope="{ row }">
          <span v-if="row.covertStatus === 1" style="color: #ff7d00">
            <el-progress
      ...
      <el-table-column
        ...
        show-overflow-tooltip
      />
      <el-table-column label="上传时间" prop="createTime" width="200px" />
      <el-table-column
        label="关联脚本数"
...
export default {
  ...
  methods: {
    ...
    onhandleClickToCurriculum(data) {
      const url =
        '/live/script?videoId=' + data.videoId + '&videoName=' + data.videoName
