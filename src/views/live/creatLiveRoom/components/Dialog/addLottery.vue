    ...
    <div class="addLottery overflowOuto">
      <el-form ref="form" v-loading="loading" :model="form" label-width="auto" size="small" :rules="rules" @submit.native.prevent>
        <el-form-item label="抽奖名称" prop="prizeDrawName">
          <el-input ref="userInput" v-model.trim="form.prizeDrawName" style="width: 450px" clearable placeholder="请输入" maxlength="20" show-word-limit />
        </el-form-item>
        <el-form-item label="参与条件" prop="participationCondition">
          <el-radio-group v-model="form.participationCondition" @change="participationConditionChange">
            <el-radio :label="1">用户在线</el-radio>
            <!-- <el-radio :label="2">参与任意评论</el-radio> -->
            <el-radio :label="3">参与指定评论</el-radio>
          </el-radio-group>

          <el-input v-if="form.participationCondition === 3" v-model.trim="form.assignedComment" placeholder="点击输入指定的评论" class="m-t-10" maxlength="50" show-word-limit />
        </el-form-item>
        <el-form-item label="奖品设置" prop="prizeDetail">
          ...
          >
            ...
            <el-table-column prop="prizeName" label="奖品名称" width="130">
              <template slot-scope="{row}">
                <el-input v-model.trim="row.prizeName" placeholder="例如：一等奖" maxlength="6" />
              </template>
            </el-table-column>
            <el-table-column prop="amount" label="份数" width="74">
              <template slot-scope="{row}">
                <el-input v-model.trim="row.amount" @input="(e) => delNumber(e, row)" />
              </template>
            </el-table-column>
            <el-table-column label="" width="50" align="center">
              <template slot-scope="{row}">
                <i class="el-icon-delete cursorPoninter" style="color: #0C6FFF;font-size: 14px" @click="onhandleDel(row)" />
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-form-item label="规则设置" prop="showParticipantsAmount">
          <el-checkbox v-model="form.showParticipantsAmount" :true-label="1" :false-label="0">用户端将展示抽奖参与人数</el-checkbox>
        </el-form-item>
        <el-form-item label="抽奖说明" prop="prizeInstructions">
          <el-input v-model.trim="form.prizeInstructions" type="textarea" maxlength="500" show-word-limit :autosize="{minRows: 2, maxRows: 6}" />
        </el-form-item>
      </el-form>
    </div>
...
export default {
  ...
  methods: {
    ...
    onhandleDel(row) {
      ...
      this.form.prizeDetail.map((v, i) => { v.sort = i + 1 })
    },
      ...
      if (tbody) {
        _this.sortable = Sortable.create(tbody, {
          async onEnd({ newIndex, oldIndex }) {
            ...
            _this.form.prizeDetail.map((v, i) => { v.sort = i + 1 })
          }
        })
      }
    },
