  ...
  >
    ...
    <div class="selectTask">
      <div class="filterHeader">
        ...
        <el-input v-model.trim="search.taskName" size="small" placeholder="请输入任务名称" style="width: 200px" class="m-r-10" clearable @keyup.enter.native="searchFn" @clear="searchFn" />
        <span class="formLabel">任务创建时间</span>
        <el-date-picker v-model="search.time.value" size="small" class="input-with-time m-r-10" type="daterange" value-format="yyyy-MM-dd" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :unlink-panels="true" @change="handleClickSearch" />
      ...
      <div v-loading="loading" class="myTable">
        <el-table height="485" :header-cell-style="{ background: '#F3F3F3' }" :data="list" @row-click="rowClick">
          <el-table-column width="80" align="center">
            <template slot-scope="{ row }">
              <el-checkbox :value="selectIdsList.includes(row.id)" @change="rowClick(row)" />
            </template>
          </el-table-column>
          <el-table-column label="任务名称" prop="taskName" min-width="190px" show-overflow-tooltip />
        ...
        <div class="pagination">
          <div class="tipsError">
            ...
            <span v-if="selectList.length > 50" class="m-l-10">您关联的任务超过50个，请重新选择！</span>
          </div>
          <el-pagination :current-page="currentPage" :page-sizes="[10, 20, 50, 100]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total" :pager-count="5" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
        </div>
      </div>
    </div>
...
export default {
  ...
  watch: {
    visible: {
      handler(val) {
        if (val) {
          this.search.time.value = [moment().subtract(3, 'month').format('YYYY-MM-DD'), moment().format('YYYY-MM-DD')]
          val && this.getList()
