  ...
  >
    <div v-loading="loading" class="exportScript overflowOuto">
      ...
      <template v-if="active === 1">
        ...
        <div class="m-t-20 tips m-b-16">2.选择需要导入的数据文件<span class="remake">注：导入文件为Excle文件，最大导入为1万条。请确保导入手机号为11位。</span></div>
        <upload
          ref="MUpload"
          width="160"
          height="120"
...
import { importLiveRoomUserLimit } from '@/api/liveRoom/detail.js'
...
export default {
  ...
  methods: {
    ...
    onhandleStrtExport() {
      ...
      importLiveRoomUserLimit(formData,
        (progressEvent) => {
    ...
    getImportFileUrl() {
      ...
      newWin.location.href = 'https://oss.hokkj.cn/edata-live/%E7%9B%B4%E6%92%AD%E9%97%B4%E6%8C%87%E5%AE%9A%E4%BA%BA%E5%91%98%E5%AF%BC%E5%85%A5.xls' // 成功 重定向到目标页面
