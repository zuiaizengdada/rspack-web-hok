<template>
  <AppDialog v-model="visible" :title="title" width="1377px" height="593px" top="10vh" @success="successFn">
    <div v-loading="loading" class="choose_resource">
      <div class="window-search">
        <span v-if="['0', '1', '2', '3', '4', '5', '51', '7'].includes(currentId) && tearchSearch" class="text">IP</span>
        <TeacherSelect v-if="['0', '1', '2', '3', '4', '5', '51', '7'].includes(currentId) && tearchSearch" ref="teacherSelect" class="m-r-20" placeholder="请选择IP" :default-tearch-id="tearch.lecturerId" :value="tearch.lecturerName" @selected="teacherSelect" />
        <m-select v-if="['0', '1', '2', '3', '4', '5', '51', '7'].includes(currentId) && tearchSearch" v-model="singleLecture" :clearable="false" dict-key="singleLectureType" style="width: 140px" size="small" class="m-r-20" />
        ...
        <div class="order-time-box">
          <el-select v-model="searchType" placeholder="请选择" size="small" class="input-with-select input-with-select-chang">
            <el-option label="商品名称" :value="1" />
            <el-option label="商品ID" :value="2" />
          </el-select>
          <el-input v-if="searchType === 1" v-model.trim="search" class="filter_teacherSelect m-r-20" style="width: 160px" placeholder="请输入商品名称" clearable size="small" @keyup.enter.native="searchFn" @clear="searchFn" />
          <el-input v-else v-model.trim="searchGoodsId" class="filter_teacherSelect m-r-20" style="width: 160px" placeholder="请输入商品id" clearable size="small" @keyup.enter.native="searchFn" @clear="searchFn" />
        </div>

        <div v-if="isSearchPrice && ['0', '1', '2', '3', '4', '5', '7'].includes(currentId)" class="moneyBox m-r-20">
          <span class="text">价格</span>
          <el-input-number v-model="starSalePrice" size="small" placeholder="请输入" style="width: 139px" :controls="false" />
          <span class="m-l-10 m-r-10">-</span>
          <el-input-number v-model="endSalePrice" size="small" placeholder="请输入" style="width: 139px" :controls="false" />
        </div>
        <template v-if="['A7'].includes(currentId) && showCourseType">
          <span class="text">课程标签</span>
          <el-select v-model="courseType" clearable placeholder="请选择" size="small" class="m-r-20" @clear="searchFn">
            <el-option label="视频课" :value="0" />
            <el-option label="直播课" :value="1" />
          </el-select>
        </template>
        <el-button size="small" type="primary" class="search_btn search_paramy" @click="searchFn">查询</el-button>
        <el-button size="small" class="search_btn" @click="searchReset">重置</el-button>
      </div>
      <div class="window-tab">
        ...
        <div class="tab-slort">
          <span>排序：</span>
          <m-select v-model="slort" dict-key="COMMODITY_SORT" size="small" @change="handleSlortChage" />
        </div>
      </div>
      <div class="window-list">
        <div class="resource-item-wrapper">
          <template v-if="['0', '1', '2', '3', '4', '5', '6'].includes(currentId)">
            <div class="resource-item-wrapper-box" :class="{ noMultiple: !multiple }">
              <el-checkbox-group v-model="checkList">
                <el-checkbox v-for="(item, index) in tableData" :key="item.goodsId" :label="item.goodsId" :disabled="check.includes(item.goodsId) || disabledFn(item)" @change="res => checkChange(res, item, index)">
                  <div class="resource-item-wrapper-box-group">
                    <div class="item-wrapper">
                      ...
                      <div class="item-info">
                        ...
                        <div style="display: flex">
                          <span v-if="isPlaform" style="color: #333;line-height: 24px;font-weight: 600;" class="m-r-10">{{ item.tenantVO.tenantName + ' ' }}</span>
                          ...
                          <AppTag v-if="[0,1,2].includes(item.shelveFlag)" :bg-color="{0: '#E6F7EA', 1: '#FEECEC', 2: '#FFF2E6'}[item.shelveFlag]">
                            <template slot="icon">
                              <i :class="{0: 'el-icon-success', 1: 'el-icon-error', 2: 'el-icon-time'}[item.shelveFlag]" :style="{color: {0: '#00b42a', 1: '#F53F3F', 2: '#FF7D00'}[item.shelveFlag]}" />
                            </template>
                            <template slot="text">
                              <span :style="{color: {0: '#00b42a', 1: '#F53F3F', 2: '#FF7D00'}[item.shelveFlag]}">{{ { 0: '已上架', 1: '已下架', 2: '待上架' }[item.shelveFlag] }}</span>
                            </template>
                          </AppTag>
                        </div>
                      </div>
                    ...
                    <div v-if="packagePrice" class="packageInputView">
                      ...
                      <el-input-number v-model="item.packagePrice" :min="0" size="small" placeholder="请输入" style="width: 262px;text-align: left;" :controls="false" />
                    </div>
                  </div>
                </el-checkbox>
              </el-checkbox-group>

              <div v-if="tableData && tableData.length === 0" class="resource-item-wrapper-box-noData">
          ...
          <template v-else-if="['8', '9'].includes(currentId)">
            <div class="ss-material-main-tree">
              <div class="ss-material-main-tree-left">
                <AppTree v-model="classlist" v-loading="loading" style="width: 200px" :loading="treeLoading" :nobtn="true" @node-click="treeClick" />
              </div>
              <div class="ss-material-main-tree-right">
                <el-checkbox-group v-model="checkList">
                  <el-checkbox v-for="(item, index) in tableData" :key="item.materialId" :label="item.materialId" :disabled="check.includes(item.materialId)" @change="res => checkChange(res, item, index)">
                    <div class="resource-item-wrapper-box-group" style="width: 1067px;">
                      <div class="item-wrapper">
                        <img v-if="currentId === '8'" :src="item.materialFrameUrl" alt="" style="width: 60px; height: 60px" />
                        <svg-icon v-if="currentId === '9'" class-name="size-icon" style="width: 60px; height: 60px;font-size: 60px" icon-class="audio" @click="play(row.materialUrl)" />
                        <div class="item-info">
          ...
          <template v-else-if="['51'].includes(currentId)">
            <div class="resource-item-wrapper-box" :class="{ noMultiple: !multiple }">
              <el-checkbox-group v-model="checkList">
                <el-checkbox v-for="(item, index) in tableData" :key="item.specId" :label="item.specId" :disabled="check.includes(item.specId) || disabledFn(item)" @change="res => checkChange(res, item, index)">
                  <div class="item-wrapper">
                    <ImagePreviewer class="image-previewer" :src=" item.contentUrl ? item.contentUrl.split(',')[0] : ''" alt="" />
                    <div class="item-info">
                      <div class="item-name">{{ item.goodsName }}</div>
                      <div class="item-price">
                        <el-tag size="mini">{{ item.specName }}</el-tag>
                      </div>
                      <div class="item-price">￥{{ item.salePrice | filtersMoney }}</div>
                    </div>
                  </div>
                </el-checkbox>
              </el-checkbox-group>
          ...
          <template v-else-if="['7'].includes(currentId)">
            <div ref="resourceItemWrapperBox" class="resource-item-wrapper-box" :class="{ noMultiple: !multiple }">
              <div v-for="(item, index) in tableData" :id="'packageCoure-check-view-group' + item.goodsId" :key="item.goodsId" class="packageCoure-check-view-group" :label="item.goodsId">
                <div class="packageCoure-item-wrapper-box-group" @click="onClickPackageCourse(item.goodsId, item, index)">
                  <div class="m-r-10">
                    <el-checkbox :value="checkList.includes(item.goodsId)" :disabled="check.includes(item.goodsId) || disabledFn(item)" @change="onClickPackageCourse(item.goodsId, item, index)" />
                  </div>
                  <div class="packageCoure-item-wrapper">
                    ...
                    <div class="packageCoure-item-info">
                      <div class="item-name">{{ item.contentName }}</div>
                      <div style="display: flex">
                        <span v-if="isPlaform" style="color: rgb(51, 51, 51); line-height: 24px; font-weight: 600;" class="m-r-10">{{ item.tenantVO.tenantName + ' ' }}</span>
                        <div v-if="item.saleType === 0" class="item-price m-r-10">免费</div>
                        <div v-if="item.saleType === 2" class="item-price m-r-10">加密</div>
                        <div v-if="item.saleType === 1" class="item-price m-r-10">￥{{ item.salePrice | filtersMoney }}</div>
                        <AppTag v-if="[0,1,2].includes(item.shelveFlag)" :bg-color="{0: '#E6F7EA', 1: '#FEECEC', 2: '#FFF2E6'}[item.shelveFlag]">
                  ...
                  <div v-if="item.children && item.children.length > 0" class="packageCoure-item-wrapper-left" @click.stop="onhandleOpen(item)">
                ...
                <div v-if="item.childrenOpen" class="packageCoure-item-children">
                  <div v-for="(c) in item.children" :key="c.goodsId" class="packageCoure-item-wrapper-box-group" style="padding: 18px 0px 0px 50px;">
                    <div class="packageCoure-item-wrapper">
                      ...
                      <div class="packageCoure-item-info">
                        <div class="item-chidren-name">
                          ...
                          <span v-if="(c.goodsType === 3 || c.goodsType === 4) && c.productCategory === 2" style="margin-left: 15px" class="item-chidren-type">线下课</span>
              ...
              <div v-for="(item, index) in tableData" :id="'packageCoure-check-view-group' + item.goodsId" :key="item.goodsId" class="packageCoure-check-view-group" :label="item.goodsId">
                <div class="packageCoure-item-wrapper-box-group">
                  <div class="m-r-10" :class="{ noMultiple: !multiple }">
                    <el-checkbox
                      :indeterminate="packageCoure_checkList[item.goodsId] && packageCoure_checkList[item.goodsId].length > 0 && packageCoure_checkList[item.goodsId].length !== item.liveCourseNum"
                      :value="ifCheckAll(item)"
                      :disabled="item.liveCourseNum === 0 || disabledFn(item)"
                      @change="(res) => onClickPackageA7Course(item.goodsId, item, index, res)"
                    />
                  </div>
                  <div class="packageCoure-item-wrapper" @click.stop="onClickPackageA7Course(item.goodsId, item, index, !ifCheckAll(item))">
                    <ImagePreviewer class="image-previewer" :src="item.coverVerticalUrl" alt="" fit="contain" />
                    <div class="packageCoure-item-info">
                      <div class="item-name">{{ item.contentName }}</div>
                ...
                <div v-if="item.childrenOpen" class="packageCoure-item-children">
                  <div v-for="(c) in item.children" :key="c._goodsId" class="packageCoure-item-wrapper-box-group" style="padding: 18px 0px 0px 25px;">
                    <div class="packageCoure-item-wrapper">
                      <div class="m-r-10">
                        <el-checkbox
                          :value="checkList.includes(c._goodsId)"
                          :disabled="!((c.goodsType === 3 || c.goodsType === 4) && c.productCategory === 1) || (check.includes(c._goodsId) || disabledFn(c))"
                          @change="onClickA7PackageCourse(c._goodsId, c, index, item)"
                        />
                      </div>
                      <ImagePreviewer class="image-previewer" :src="c.coverVerticalUrl" alt="" fit="contain" />
                      <div class="packageCoure-item-info" @click.stop="onClickA7PackageCourse(c._goodsId, c, index, item)">
                        <div class="item-chidren-name">
                          <span class="item-name">{{ c.contentName }}</span>
                          <span v-if="(c.goodsType === 3 || c.goodsType === 4) && c.productCategory === 0" style="margin-left: 15px" class="item-chidren-type">录播课</span>
    ...
    <div slot="footer" class="dialogFooter">
      <div>
        <el-pagination v-if="!['A7'].includes(currentId)" layout="jumper, prev, pager, next, sizes, total" :total="total" :page-size="size" :current-page="current" @current-change="handleCurrentChange" @size-change="handleSizeChange" />
      </div>
      <div class="dialogFooterRight">
        <el-button type="primary" class="search_btn search_paramy" @click="successFn()">确定</el-button>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      } else if (['0', '1', '2', '3', '4', '5'].includes(this.currentId)) {
        this.getGoodsList()
      ...
      getGoodsList(params)
        .then(res => {
          if (this.packagePrice) {
            res.data.records.map(v => { v.packagePrice = v.salePrice / 100 })
          }
          console.log(res, 'res这是一个商品数据', this.packageChildren)
          res.data.records.map(v => {
            const index = this.packageChildren.findIndex(e => e.associateGoodsId === v.goodsId)
            index > -1 && (v.packagePrice = (this.packageChildren[index].packagePrice) / 100)
          })
      ...
      getGoodsList(params)
        .then(res => {
          ...
          res.data.records.map(v => {
            ...
            v.children && v.children.map(e => {
              ((e.goodsType === 3 || e.goodsType === 4) && e.productCategory === 1) && v.liveCourseNum++
              e._goodsId = v.goodsId + '-' + e.goodsId
              return
            })
            const index = this.packageChildren.findIndex(e => e.goodsId === v.goodsUd)
            index > -1 && (v.packagePrice = (this.packageChildren[index.packagePrice]) / 100)
          })
          this.tableData = res.data.records
    ...
    successFn() {
      // 选了相同老师课程
      const res = this.success(this.checkListInfo, this.checkList)
    ...
    checkChange(res, item, index) {
      if (this.multiple) {
        ...
        if (res) {
          ...
          const i = this.checkListInfo.findIndex(v => {
            if (['0', '1', '2', '3', '4', '5', '6', '7'].includes(this.currentId)) {
      ...
      console.log(this.checkListInfo)
      this.loading = false
    },
    // 点击搜索
    searchFn() {
    ...
    getPlatformGoodsList() {
      ...
      platformGoodsList(params)
        .then(res => {
          res.data.records.map(v => { v.childrenOpen = false })
          this.tableData = res.data.records
          this.total = res.data.total
          this.loading = false
        })
    ...
    onClickA7PackageCourse(value, c_item, index, item) {
      if (!((c_item.goodsType === 3 || c_item.goodsType === 4) && c_item.productCategory === 1) || this.check.includes(c_item._goodsId) || this.disabledFn(c_item)) {
        return
      }
      if (this.changeLoading) return
      this.changeLoading = true
      const i = this.checkList.findIndex(v => v === value)
      const c_index = this.checkListInfo.findIndex(v => v.goodsId === item.goodsId)

      if (i > -1) {
        // 删除
        this.checkList.splice(i, 1)
        if (this.checkListInfo[c_index] && this.checkListInfo[c_index].children) {
          const c_index_chidren = this.checkListInfo[c_index].children.findIndex(v => v._goodsId === c_item._goodsId)
          c_index_chidren > -1 && this.checkListInfo[c_index].children.splice(c_index_chidren, 1)
        }
      } else {
        // 添加
        this.checkList.push(value)
        if (c_index > -1) {
          this.checkListInfo[c_index] && this.checkListInfo[c_index].children.push(c_item)
        } else {
    ...
    onClickPackageA7Course(value, item, index, res) {
      ...
      if (obj && obj.children) {
        ...
        const childrenArr = []
        obj.children.map(v => {
          if (((v.goodsType === 3 || v.goodsType === 4) && v.productCategory === 1)) {
            ids.push(v._goodsId)
            childrenArr.push(v)
          }
        })
        ...
        if (res) {
          ...
          index > -1 ? this.checkListInfo[index] = childrenArr : this.checkListInfo.push({ ...obj, children: childrenArr })
        } else {
          // 全取消
          this.checkList = this.checkList.filter(element => !ids.includes(element))
    ...
    onhandleOpen(item) {
      ...
      if (item.childrenOpen) {
        this.$nextTick(() => {
          const element = document.getElementById(`packageCoure-check-view-group${item.goodsId}`)
    ...
    ifCheckAll(item) {
      if (Object.getOwnPropertyNames(this.packageCoure_checkList).length === 0) {
        return false
      }
      if (!this.packageCoure_checkList[item.goodsId]) {
        return false
      }
      if (this.packageCoure_checkList[item.goodsId] && this.packageCoure_checkList[item.goodsId].length === 0) {
        return false
      }
      if (this.packageCoure_checkList[item.goodsId] && this.packageCoure_checkList[item.goodsId].length !== item.liveCourseNum) {
        return false
      }
      return true
    }
