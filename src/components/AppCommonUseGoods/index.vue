  ...
  >
    <div v-loading="loading" class="choose_resource">
      <div class="window-search">
        <span
          v-if="
            ['0', '1', '2', '3', '4', '5', '51', '7'].includes(currentId) &&
              tearchSearch
          "
          class="text"
        >IP</span>
        ...
        <div
          v-if="
            isSearchPrice &&
              ['0', '1', '2', '3', '4', '5', '7'].includes(currentId)
          "
      ...
      <div class="window-list">
        <div class="resource-item-wrapper">
          <template
            v-if="['-1','0', '1', '2', '3', '4', '5', '6'].includes(currentId)"
          >
            <div
            ...
            >
              ...
              >
                ...
                <div class="collect" :class="{'active' : item.commonUse===1}" @click.stop="handleCollect(item,index)"><i class="el-icon-star-on" /></div>
              </div>
              <div
...
export default {
  ...
  methods: {
    async handleCollect(item, index) {
      console.log('收藏')
      await commonUseGoods({ goodsId: item.goodsId,
        goodsName: item.goodsName }
      )
      if (this.currentId === '-1') {
        this.tableData.splice(index, 1)
      } else {
        item.commonUse = item.commonUse === 1 ? 0 : 1
      }
    },
    ...
    getList() {
      ...
      } else if (['-1', '0', '1', '2', '3', '4', '5'].includes(this.currentId)) {
        this.getGoodsList()
    ...
    getGoodsList() {
      ...
      const params = {
        ...
        depositFlag: 0, // 查预存
        commonUse: this.currentId === '-1' ? 1 : 0, // 是否只查常用商品：0否，1是
        goodsType: this.currentId !== '-1' ? this.currentId : '', // 商品类型：3普通专栏，4大专栏，5线下课
      ...
      if (this.multiple) {
        ...
        if (res) {
          ...
          const i = this.checkListInfo.findIndex(v => {
            if (
              ['0', '1', '2', '3', '4', '5', '6', '7'].includes(this.currentId)
            ) {
