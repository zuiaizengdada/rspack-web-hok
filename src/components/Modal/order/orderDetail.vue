...
<template>
  <div v-loading="loading" :class="`group${type == 2 ? 'Dialog' : 'Drawer'}`">
    <div class="form">
      <div class="page_wrap">
        <!-- <div class="header">
          <span class="pointer" @click="$router.push({path: '/business/order'})">订单管理</span>
          > 订单详情
        </div> -->
        ...
        <div class="order-detail">
          <div>
            <div class="deliver-info">
              <div>
                <i v-if="detail.orderStatus === 2" class="el-icon-success" style="color: #21c11f; font-size: 18px" :style="{color: orderStatus[detail.orderStatus || 0].color}" />
                <span class="m-l-10 deliverTips" :style="{color: orderStatus[detail.orderStatus || 0].color}">
          ...
          <div class="module-item">
            ...
            <div class="module-content">
              <div class="single-line">
                <div class="info-item">订单号: <span>{{ detail.orderNo || '--' }}</span></div>
                <div class="info-item">支付方式: <span>{{ detail.paymentType | getOptionsValue(paymentType_options) }}</span></div>
                <div class="info-item">下单时间: <span>{{ detail.createTime || '--' }}</span></div>
              </div>
              <hr>
              <div class="single-line">
                <div class="info-item">优惠信息: <span>{{ (detail.discountsFee || 0) | filtersMoney }}元</span></div>
                <div class="info-item">渠道来源: <span>{{ detail.userTerminal | getOptionsValue(userTerminal_options) }}</span></div>
                <div class="info-item">支付时间: <span>{{ detail.updateTime || '--' }}</span></div>
              </div>
              <div class="single-line">
          ...
          <div class="module-item">
            ...
            <div class="module-content">
              <div class="single-line">
                <div class="info-item">交易单号: <span>{{ detail.transactionId|| '--' }}</span></div>
                <div class="info-item">商户单号: <span>{{ detail.paymentNo || '--' }}</span></div>
              </div>
            </div>
          </div>
          ...
          <div class="module-item">
            ...
            <div class="module-content">
              <div class="single-line">
                <div class="info-item">用户ID: <span class="a_link" @click="gotoUserDetails(detail.userId)">{{ detail.userId || '--' }}</span></div>
                <div class="info-item">买家昵称: <span>{{ detail.userName || '--' }}</span></div>
                <div class="info-item">买家手机号: <span>{{ detail.userPhone || '--' }}<i v-if="detail.userPhone.indexOf('xxxx') > 0 || detail.userPhone.indexOf('XXXX') > 0" class="pointer el-icon-view" @click="toShowPhoneNumber" /></span></div>
              </div>
            </div>
          </div>
        ...
        <div class="module-content">
          ...
          <div class="module-item">
            ...
            <el-table :data="detail.list" style="width: 100%" border :header-cell-style="{background: '#f5f5f5'}">
              ...
              <el-table-column label="操作">
                <template slot-scope="{ row }">
                  <span class="a_link" @click="applyAfterSalesCard(row)">开通售后卡</span>
                </template>
              </el-table-column>
            </el-table>
          </div>
          <div class="clear-fix page_footer">
            <div class="price-summary clear-fix">
              <div class="summary-item clear-fix">
                ...
                <span class="content">￥ {{ (detail.discountsFee || 0) | filtersMoney }}</span>
              </div>
              <div class="summary-item total clear-fix">
                <span class="title">订单合计{{ detail.list.length }}件商品，实收:</span>
                <span class="content">￥ {{ (detail.payerTotal) | filtersMoney }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="module-item">
            ...
            <el-table :data="afterSaleList" style="width: 100%" border :header-cell-style="{background: '#f5f5f5'}">
              <el-table-column label="商品名称" prop="goodsName">
                <template slot-scope="{ row }">
              ...
              <el-table-column label="退款状态" prop="status">
                <template slot-scope="{ row }">
                  {{ { 0: '未处理', 1: '退款中', 2: '已退款', 3: '已关闭', 4: '退款异常', 5: '处理中', 6: '商家拒绝退款', 7: '部分退款' }[row.status] }}
                </template>
              </el-table-column>
              <el-table-column label="退款类型" prop="afterSaleMode">
                <template slot-scope="{ row }">
                  {{ {1: '仅退款', 3: '财务操作退款'}[row.afterSaleMode] }}
                </template>
              </el-table-column>
              ...
              <el-table-column label="操作人" prop="operationName">
                <template slot-scope="{row}">
                  ...
                  <el-button class="m-l-20" type="text" @click="gotoDetailDetail(row)">售后详情</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
          ...
          <div class="module-item">
            ...
            <div><el-button type="primary" :loading="modal.loading" @click="openAddModal">新增</el-button></div>
...
<script>
import { getOrderDetail, goodsAfterSaleList, goodsAfterSaleCard } from '@/api/business'
...
import { queryPhoneNumberByUid } from '@/api/user/member'
export default {
  components: {
    AppDialog,
    Modal,
  ...
  methods: {
    toShowPhoneNumber() {
      const params = {
        uid: this.detail.userId,
        operateModule: 23
      }
      queryPhoneNumberByUid(params).then(res => {
        if (res.code === 1) {
          this.detail.userPhone = res.data
    ...
    getGoodsAfterSaleList() {
      goodsAfterSaleList({ orderNo: this.orderNo }).then(res => {
    ...
    applyAfterSalesCard(row) {
      ...
      this.$delModal({
        ...
        success: () => {
          this.loading = true
          goodsAfterSaleCard(row.id).then(res => {
            console.log(res, 'res')
            this.$message({
              showClose: true,
              duration: 0,
