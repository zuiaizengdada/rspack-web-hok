    ...
    <div class="form">
      <div v-loading="loading" class="page_wrap">
        ...
        >
          ...
          <div class="m-t-20">
            <el-form-item label="视频名称" prop="contentName">
              <el-input
            ...
            <el-form-item label="商品主图" prop="coverUrlList">
              <ImageUpload
                ...
                :limit="8"
                :tips="['方形缩略图是指在商品列表展示的图片。','建议尺寸750*750，小于5MB的JPG、PNG格式图片']"
                @change="getCoverUrlList"
              />
            </el-form-item>

            <el-form-item label="视频上传" prop="materialVo">
              <div class="fl videoUpload">
                <div class="w-200 m-r-30">
                  <div
                    v-if="form.materialVo.materialFrameUrl"
                    class="video-preview"
                  >
                    <el-popover
                      ...
                      width="400"
                      trigger="hover"
                      :content="`${form.materialVo.materialName} ${form.materialVo.createTime}`"
                    >
                      <div slot="reference">
                        <el-image
                          ...
                          fit="contain"
                          class="pointer"
                          :src="form.materialVo.materialFrameUrl"
                          @click="playVideo(form.materialVo)"
                        />
                        <!-- <div class="preview__mask" /> -->
                        <!-- <div class="preview__text">{{ form.materialVo.materialName }}</div> -->
                      </div>
                    </el-popover>
                  </div>
                  <div
                ...
                <div>
                  ...
                  <div class="tips m-t-20">
                    <div>
                      支持mp4，avi，wmv，flv，rmvb，3gp，m4v，mkv格式；文件最大不超过5G。
                    </div>
                    <div>当前店铺版本最大支持1080超清转码</div>
                  </div>
                </div>
              </div>
            </el-form-item>
            ...
            <el-form-item label="视频封面图" prop="patchUrl">
              <div>
                <div
                  v-if="!form.patchUrl"
                  class="image_poster"
                ...
                <div class="tips m-t-20">
                  <div>
                    视频封面图是指视频播放器在播放前展示的图片，建议尺寸750*422或比例16：9，
                  </div>
            ...
            <template v-if="form.goodsForm == 2">
              ...
              <template v-if="form.giftsType === 1">
                ...
                <el-form-item label="赠品数量" prop="giftsNum">
                  <el-input-number v-model="form.giftsNum" :controls="false" :min="1" :max="8" />
                </el-form-item>
                <el-form-item label="赠品退还策略" prop="policyType">
                  <el-select v-model="form.policyType" placeholder="请选择赠品退还策略" @change="changePolicyType">
                    <el-option
                      label="退款不需要退回"
                      :value="1"
                    />
            ...
            <el-form-item label="相关资料">
              ...
              <!-- 资料列表 -->
              <div
                v-show="form.informationPicReqList.length > 0"
                class="ZiliaoTable"
              >
                <el-table
                  class="m-t-20"
              ...
              <!-- PPT列表 -->
              <div
                v-show="form.informationPptReqList.length > 0"
                class="PPTTable"
              >
                <el-table
                  :header-cell-style="{ background: '#f5f5f5', padding: '0' }"
            ...
            <el-form-item label="上架设置">
              <el-radio-group v-model="form.shelveFlag">
                <el-radio
                  ...
                  class="m-t-10"
                >立即上架</el-radio>
                <el-radio v-if="!isDou" :label="2" class="m-t-10">
                  <span>定时上架</span>
                  <el-date-picker
              ...
              <div v-if="!isDou" class="EC1 m-t-20 m-b-10">更多设置</div>
              <div v-if="!isDou">
                <el-checkbox
              ...
              <el-divider v-if="!isDou" />
              <div v-if="!isDou" class="tips">
                <i class="el-icon-warning" />
                根据您上传的视频大小需要0-30分钟不等的转码时间，转码过程中该商品将隐藏不可显示，转码完成后会根据您的设置进行显示/隐藏。
              </div>
            </el-form-item>
          </div>

          <div class="header_tips">防录屏跑马灯</div>

          <div class="m-t-20">
            <el-form-item label="防录屏跑马灯">
              <el-radio-group v-model="form.lampFlag">
                <el-radio :label="1">开启</el-radio>
                <el-radio :label="0">关闭</el-radio>
              </el-radio-group>
              <div class="tips">
                开启后，学员观看课程会出现个人ID信息，可有效防止录屏。
              </div>
            </el-form-item>
          </div>
  ...
  data() {
    ...
    return {
      ...
      form: {
        contentName: '', // 名称
        materialId: '', // 素材Id
        materialUrl: '', // 素材存储路径(视频，音频)
        materialFrameUrl: '', // 视频首帧图片url
        ...
        coverUrl: '', // 横版封面存储路径
        patchUrl: '', // 视频贴片存储路径，goods_type=2生效
        ...
        goodsId: '', // 视频音频图文id
        goodsType: 2, // 0:图片,1:音频，2视频,3专栏，4大专栏
      ...
      rules: {
        ...
        giftsImgList: [
          { required: true, message: '请选择赠品图片', type: 'array', trigger: 'change' }
        ],
        goodsForm: [
          {
            required: true,
        ...
        patchUrl: [
          { required: true, message: '请选择视频封面图', trigger: 'change' }
        ],
        sale: [{ required: true, validator: validateSale, trigger: 'change' }],
        content: [
          { required: true, message: '请输入视频详情', trigger: 'blur' }
        ],
        materialVo: [
          {
            required: true,
    ...
    init() {
      ...
      axios
        ...
        .then(res => {
          ...
          //   res[2].data.materialSaleVo.salePrice = (res[2].data.materialSaleVo.salePrice || 0) / 100
          //   if (!res[2].data.materialSaleVo.marketPrice) {
          //     res[2].data.materialSaleVo.marketPrice = (res[2].data.materialSaleVo.marketPrice || 0) / 100
          //   } else {
          ...
          this.form = {
            ...
            giftsType: materialGiftsVo.giftsType || 1,
