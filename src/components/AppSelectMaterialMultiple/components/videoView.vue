<template>
  <div>
    <div class="ss-material-picture">
      <div class="left-wrap">
        <el-button class="m-r-20" @click.native="uploadVideo">上传视频</el-button>
        <span class="ss-material-button" @click="gotoMaterial()">前往素材中心</span>
      </div>
      <div class="right-wrap">
        <i class="el-icon-s-fold" :class="{ active: listType === 'list' }" @click="changeListType('list')" />
        <i class="el-icon-menu" :class="{ active: listType === 'card' }" @click="changeListType('card')" />
        <el-input
          v-model.trim="search"
          placeholder="视频名称"
      ...
      <div v-loading="loading" class="ss-material-main-right">
        <div class="content-wrap">
          ...
          <div v-show="listType === 'card'" class="card-pattern-list-wrap">
            ...
            <el-checkbox-group v-model="valueIds" @change="handleCheckedChange">
              <div v-for="item in tableData" :key="item.materialId" class="picture-card">
                <div class="content">
                  <img :src="item.materialFrameUrl" :alt="item.materialName" @click="playVideo(item)">
                ...
                <div class="detail">
                  <div class="title-wrap">
                    <el-checkbox :label="item" @change="(res) => handleCheckedChange(res, item)">
                      <span class="title">
                        {{ item.materialName }}
                      </span>
                    </el-checkbox>
                  ...
                  <div class="common-row">{{ item.materialSize | filterSize }}</div>
                </div>
              </div>
            </el-checkbox-group>
          </div>
          ...
          <div v-show="listType === 'list'" class="table-wrap">
            <div class="header-wrap">
              ...
              >
                ...
                <el-table-column prop="materialName" label="视频">
                  <template slot-scope="{ row }">
                    <div v-if="type === '2'" class="fl fl_ai_c">
                      <div class="videoImg">
                        <img :src="row.materialFrameUrl" alt="" @click="playVideo(row)">
                      </div>
                      <span class="m-l-10 text_hidden" :title="row.materialName">
                        {{ row.materialName }}
                      </span>
                    </div>
                  </template>
  ...
  methods: {
    ...
    handleCheckedChange(value, res) {
      ...
      if (value) {
        ...
        const arr = this.value.filter((item) => item.materialId !== res.materialId)
        this.$emit('input', arr)
      }
