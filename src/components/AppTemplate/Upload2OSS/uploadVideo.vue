<template>
  <div class="dialog-body">
    <div slot="title" class="ss-material-box-header" v-if="title">
      <div>{{ title }}</div>
    </div>
          ...
          <el-table-column label="操作" width="180">
            <template slot-scope="scope">
              ...
              <el-button
                ...
                type="text"
                style="color: #e6a23c"
                @click="cancelRequest(scope.row, scope.$index)"
                >取消上传</el-button
              >
              ...
              <el-button
                ...
                type="text"
                style="color: #e6a23c"
                @click="againUpload(scope.row, scope.$index)"
                >重新上传</el-button
              >
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    beforeUploads(file) {
      ...
      if(!isLt5M){
        this.$message.error(`上传视频大小不能超过${this.maxM}MB!`)
        this.$refs.upload && this.$refs.upload.abort()
      }
      return isLt5M
    },
    beforUpload(file){
      console.log(this.beforeUploads(file))
