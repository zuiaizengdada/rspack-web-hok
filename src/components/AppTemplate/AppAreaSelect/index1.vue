...
<script>
import { getFilterAreaCodeList, getAreaCode } from '@/api/user/member'
export default {
  components: {},
  props: {
    // 是否可以选择任意一项
  ...
  methods: {
    getList() {
      const params = [getFilterAreaCodeList({
        level: 0
      }), getFilterAreaCodeList({
        level: 1
      })]
      if (this.maxlevel === 2) {
        params.push(getFilterAreaCodeList({
          level: 2
        }))
      }
    ...
    lazyLoad(node, resolve) {
      ...
      let fn = getFilterAreaCodeList
      let params
      if (level === 0) {
        fn = getFilterAreaCodeList
        params = {
          level: 0
        }
      } else {
